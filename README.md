...
Phase 210 exposes
`route_approved_fresh_workflow_bounded(workflow, state_path, events_path, bootstrap_context, continuation_contexts)`.
This is an intentional public-contract narrowing: the route no longer accepts
`fresh_start_function` or `bounded_continuation_function` owner-substitution
keywords. It is the explicit top-level entry for a fresh Phase-208 step-1
start followed by at most one Phase-192 bounded continuation handoff. The
caller supplies the frozen Phase-208 bootstrap context and a finite built-in
tuple of exact frozen `ApprovedWorkflowContinuationContext` values.

Before Phase 208, the boundary performs only shallow structure checks: the
continuation container must be an exact built-in `tuple`, and every element
must be an exact `ApprovedWorkflowContinuationContext`. The canonical Phase
208 and Phase 192 owners are used directly; approval, employee, tool, API-key,
execution-approval, and transport semantics remain owned by Phase 208 and
Phase 192/190. No context, approval, employee, tool, key, or transport is
generated or looked up by Phase 210.
