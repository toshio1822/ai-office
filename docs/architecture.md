...
- OpenAI Responses JSON Serializerは、JSON互換Python辞書を決定的なJSON文字列へ変換する。compact形式は不要な空白なし、pretty形式は2 space indentとし、入力dictの挿入順序を維持して`ensure_ascii=False`でUnicodeを保持する。HTTP request builderがendpoint、headers、request bodyを構成し、future runtimeがHTTP通信、API呼び出し、response処理を担当する。
