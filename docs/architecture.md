...
Phase 79 accepts exactly one Phase 78 `WorkflowExecutionPersistenceResult`, `workflow_complete`, or `persisted_failure`. A valid persistence result is checked against the exact terminal state/history and byte counts, then delegated exactly once to Phase 72 with identical argument identity. Only the exact matching `PersistedExecutionOutcome` is returned unchanged. The terminal event `request_id` keeps the existing built-in str semantics, with the Issue #373 compatibility relaxation allowing `None` only when all of `succeeded` state, built-in int `current_step_index >= 6`, `request_id is None`, and built-in str `provider == "openai"` hold simultaneously. Completion and persisted failure are strict zero-call read-only stop routes. Dependency errors, malformed returns, mutations, and rollback failures are classified safely and compensated with both restoration attempts where possible; safe error identity is preserved and retry is never performed. Phase 79 does not select employees, resolve tools, create credentials or approvals, invoke providers or tools, create runtime results, persist transitions, duplicate classification logic, route classified outcomes, decide progression, prepare next steps, retry, continue automatically, finalize, schedule, loop, run in parallel, or add paid CLI/GUI behavior.
...
Phase 86 accepts exactly one Phase 85 `WorkflowExecutionPersistenceResult`, `workflow_complete`, or `persisted_failure`. A persistence result requires exact target identity, terminal state/history, byte counts, and final event validation, then delegates exactly once to the injected Phase 79 function with canonical identity and order `(result, workflow, state_path, events_path)`, returning the exact `PersistedExecutionOutcome`. The terminal event `request_id` keeps the existing built-in str semantics, with the Issue #373 compatibility relaxation allowing `None` only when all of `succeeded` state, built-in int `current_step_index >= 6`, `request_id is None`, and built-in str `provider == "openai"` hold simultaneously. Completion and persisted failure are strict zero-call read-only stop routes. Dependency errors, malformed outcomes, mutations, and rollback failures are classified safely and compensated with both restoration attempts where possible; safe error identity is preserved and retry is never performed. Phase 86 does not execute providers/tools, create runtime results, persist transitions, call Phase 72 directly, duplicate classification logic, route classified outcomes, decide progression, prepare steps, retry, continue automatically, finalize, schedule, loop, run in parallel, or add paid CLI/GUI behavior.
...
`route_persisted_running_execution_cycle_handoff_chain_reentry_continuation_boundary()` accepts exactly one Phase 125 `RunningStatePersistenceResult`, `WorkflowProgressionDecision(workflow_complete)`, or `PersistedExecutionOutcome(persisted_failure)`. On the continuation route, the exact Phase 125 result is revalidated with its exact `PreparedStepExecutionStart`, workflow, employee, regular state/event targets, resolved tools, credential, approval, and transport. Continuations require `PreparedStepExecutionStart.running_state.current_step_index >= 2` and the complete succeeded predecessor history; for step 2 this is the exact persisted running state plus exactly succeeded step 1. The boundary delegates directly to the public Phase 119 boundary exactly once in canonical `(result, start, workflow, employee, state_path, events_path, resolved_tools, api_key, approval, transport)` order and returns the identical exact `StepRuntimeExecutionSuccess` or `StepRuntimeExecutionFailure` object. State and event targets remain byte-for-byte unchanged.
...
Phase 131は、Phase 130 continuationからの正確な`PreparedWorkflowStep`、`WorkflowProgressionDecision(workflow_complete)`、または`PersistedExecutionOutcome(persisted_failure)`を受けるread-only bridgeである。prepared-step routeでは`step_index >= 2`を要求し、exact workflow/step/employee、regular state/event targets、succeeded predecessor terminal state/history、completed-step prefix、terminal eventのruntime linkageとprovider `"openai"`を再検証する。検証後、公開Phase 124 `route_prepared_step_start_cycle_handoff_chain_reentry_continuation_boundary()`へ、supplied object identityを保持した`(result, workflow, employee, state_path, events_path)`のcanonical five-argument orderで正確に一度だけ委譲し、exact valid `PreparedStepExecutionStart`を返す。
...
Phase 132は、Phase 131 continuationからの正確な`PreparedStepExecutionStart`、`WorkflowProgressionDecision(workflow_complete)`、または`PersistedExecutionOutcome(persisted_failure)`を受けるbridgeである。prepared-start routeでは`current_step_index >= 2`、exact workflow/step/employee、nested request/running-state、regular state/event targets、succeeded predecessor terminal state/history（predecessor index = `current_step_index - 1`; step 2 continuationではexact succeeded step 1）、completed-step prefix、terminal eventのruntime linkageとprovider=`"openai"`を再検証する。検証後、公開Phase 125 `route_prepared_start_persistence_cycle_handoff_chain_reentry_continuation_boundary()`へ、supplied object identityを保持した`(result, workflow, employee, state_path, events_path)`のcanonical five-argument orderで正確に一度だけ委譲する。
...
Phase 133は、Phase 132 continuationからの正確な`RunningStatePersistenceResult`、`WorkflowProgressionDecision(workflow_complete)`、または`PersistedExecutionOutcome(persisted_failure)`を受けるouter bridgeである。実行routeでは`PreparedStepExecutionStart.running_state.current_step_index >= 2`、exact workflow/start/request/running-state/employee/tools/credential/approval、regular targets、Phase 132のpersisted running state bytes、succeeded predecessor history、直前のsucceeded terminal eventのprovider=`"openai"`を再検証する。step 2ではpredecessor historyはexact succeeded step 1である。直前より前のpredecessor providerはPhase 132の契約を超えて制限しない。
...
Phase 138は、Phase 137のexact `PreparedWorkflowStep`と`EmployeeDefinition`を受けるprepared-start outer bridgeである。`step_index >= 2`、workflow/step/employee linkage、prepared predecessor state/history、completed prefix、直前predecessor provider=`"openai"`、terminal response/request/outputのexact contractを検証し、exact built-in empty success `output_text`も許容する。
...
Phase 139は、Phase 138のexact `PreparedStepExecutionStart`と`EmployeeDefinition`を受けるouter persistence bridgeである。`running_state.current_step_index >= 2`、exact nested request/running-state、workflow/step/employee linkage、Phase 138 predecessor terminal state/history（predecessor index = `running_state.current_step_index - 1`; step 2 continuationではexact succeeded step 1）、immediate predecessor provider=`"openai"`、terminal provider/response/request/outputのexact contractを検証し、exact built-in empty success `output_text`も許容する。
...
Phase 141は、Phase 139のexact `RunningStatePersistenceResult`、`WorkflowProgressionDecision(workflow_complete)`、または`PersistedExecutionOutcome(persisted_failure)`を受けるouter boundaryである。実行routeでは`PreparedStepExecutionStart.running_state.current_step_index >= 2`、exact workflow/start/request/running-state/employee/tools/credential/approval/transport、regular targets、Phase 139のpersisted running state bytes、succeeded predecessor history、immediate predecessor provider=`"openai"`を再検証する。step 2ではexact persisted running stateとexact succeeded step-1 predecessor historyを使用する。exact built-in empty success `output_text`はPhase 140の非final契約どおり許容する。
...
Phase 146は、Phase 145のexact `PreparedWorkflowStep`、`WorkflowProgressionDecision(workflow_complete)`、または`PersistedExecutionOutcome(persisted_failure)`を受けるouter-chain boundaryである。prepared-step routeでは、exact `PreparedWorkflowStep`、exact `WorkflowDefinition`/`WorkflowStepDefinition`、exact `EmployeeDefinition`、regular targets、exact built-in `int step_index >= 2`（generic continuation domain; historical Phase-position lower bound removed; Phase 145 provenance/compatibility thresholds are preserved）、workflow/step/index/employee linkage、employee ID/instructions/model/allowed-tools linkage、exact built-in tuple `allowed_tool_names`、persisted predecessor terminal succeeded state（`prepared.step_index - 1`、step 2 continuationでは exact succeeded step 1がpredecessor）、complete ordered succeeded predecessor/terminal history、succeeded completed-step prefix、`last_failure_category is None`、Phase 145 provenance（全historyがexact `RuntimeStepEvent`、earlier predecessorはexact `step_succeeded`・`running -> succeeded`・provider non-empty str・immediate predecessor provider=`"openai"`・still earlier providerはvalid non-`openai`・response_id/request_id non-empty・`output_text`はexact built-in `str`でemptyも許容・failure_category/messageは`None`、terminal eventはprovider=`"openai"`・request_id `None`またはnon-empty・response_id non-empty・`output_text`はexact built-in `str`でemptyも許容）を再検証する。検証後、公開Phase 138 `route_prepared_step_start_cycle_handoff_chain_bridge_outer_reentry_continuation_boundary()`へcanonical five-argument order `(result, workflow, employee, state_path, events_path)`でexactly once委譲し、返却されたexact `PreparedStepExecutionStart`（exact nested `ModelInvocationRequest`/`WorkflowExecutionState`、request linkage、running stateはstatus=`"running"`・`current_step_index >= 2`・completed-step prefix継承・`last_failure_category is None`）を再検証して返す。正常経路では両targetをbyte-for-byte不変に保つ。
...
Phase 165は、outcome-classification segment（**実Phase 79 → 実Phase 72 → 実Phase 65 → Phase 58**）がPhase-155 provenance persisted outcomeを正しく受け渡せるようにするstaged compatibility repairである。Phase 164で修復したPhase 100 → 93 → 86セグメントの直後にあるsegmentで、各Phaseはstrict loaderがPhase-155 provenance history（`current_step_index >= 6`、predecessorの空`output_text`）を拒否する場合にのみ、public `load_workflow_execution_history`（`WorkflowExecutionPersistenceTargets`）+ 限定されたPhase-155互換検証へフォールバックする。
...
各Phase（79 / 72 / 65）のproduction moduleに、**strict-first** のlocal bounded fallbackを実装する:
...
Phase 79 / 72 / 65の既存test moduleへ各**+6 cases**を追加する（fixtureはexact Phase-155 provenance: 6-step history "one"〜"six"、predecessor 1-5＝step_succeeded（step 2とstep 5は`output_text=""`、step 5は`provider="openai"` / `request_id=None`、他は`request_id`非空）、terminal step 6＝succeeded（`response_id` "response-six", `output_text` "output-six"）/ failed（`failure_category` "api_error", `message` "safe failure"/`""`））。
...
Phase 166は、persisted-outcome classificationの最終segment（**実Phase 58 → 実Phase 51 → 実Phase 44 → 実Phase 37**）がPhase-155 provenance persisted outcomeを正しく受け渡せるようにするstaged compatibility repairである。Phase 165で修復したPhase 79 → 72 → 65セグメントの直後にあるtailで、各Phaseはstrict loaderがPhase-155 provenance history（`current_step_index >= 6`、predecessorの空`output_text`）を拒否する場合にのみ、public `load_workflow_execution_history`（`WorkflowExecutionPersistenceTargets`）+ 限定されたPhase-155互換検証へフォールバックする。
...
各Phase（58 / 51）のproduction moduleに、**strict-first** のlocal bounded fallbackを実装する:
...
Phase 58 / 51の既存test moduleへ各**+6 cases**を追加する（fixtureはexact Phase-155 provenance: 6-step history "one"〜"six"、terminal "six" index 6、employee = step_id[0]、`events[4]`は`provider="openai"`・`request_id=None`・`output_text=""`）。
...
Phase 167は、classified persisted-outcome progressionのsegment（**実Phase 144 → 実Phase 136 → 実Phase 129**）が、Phase 155 provenance persisted outcome（`current_step_index >= 6`、predecessorの空`output_text`、immediate predecessorの`request_id=None`）を、次strict seamであるPhase 122へ正しく受け渡せるようにするstaged compatibility repairである。Phase 166で修復したclassification tailの直後にあるprogression segmentで、各Phaseはstrict loaderがPhase-155 provenance historyを拒否する場合にのみ、public `load_workflow_execution_history`（`WorkflowExecutionPersistenceTargets`）+ 限定されたPhase-155互換検証へフォールバックする。
...
Phase 168は、classified persisted-outcome progressionのsegment（**実Phase 122 → 実Phase 115 → 実Phase 108**）が、Phase 155 provenance persisted outcome（`current_step_index >= 6`、predecessorの空`output_text`、immediate predecessorの`request_id=None`・`provider="openai"`）を、次strict seamであるPhase 101へ正しく受け渡せるようにするstaged compatibility repairである。Phase 167で修復したPhase 144→136→129 segmentの直後にあるprogression segmentで、各Phaseはstrict loader（`load_strict_terminal_history`）がPhase-155 provenance historyを拒否する場合にのみ、public `load_workflow_execution_history`（`WorkflowExecutionPersistenceTargets`）+ 限定されたPhase-155互換検証へフォールバックする。
...
Phase 171は、Phase 155 provenance persisted outcome（`current_step_index >= 6`、predecessorの空`output_text`、immediate predecessorの`request_id=None`・`provider="openai"`）を、**実Phase 52 bridge → 実Phase 45 → 実Phase 38 → 実Phase 37 / 31 → 実Phase 25**の下流実チェーンが無変更で受容することを証明するfinal staged compatibility repairである。Phase 170まで「次strict seam」として変更しなかったPhase 52を、`strict-first + local bounded fallback`で修復し、`Phase 45 → 38 → 37 / 31 → 25`の実ルーティング tailが同一provenanceをそのまま受け渡せることを実チェーンregressionで固定する。
...
- **Phase 52のみ修復**: `_validate_terminal` のstrict loadを `try: ...` / `except TerminalHistoryContractError:` で包み、Phase-155互換ケース（exact `PersistedExecutionOutcome` + exact builtin int `current_step_index >= 6`）に限定した `_load_phase155_terminal_history` へフォールバックする。public loader使用（`WorkflowExecutionPersistenceTargets(state_path, events_path)`）、`_valid_phase155_terminal_history` / `_valid_phase155_predecessor` / `_valid_phase155_terminal_event` で検証。helperはPhase 52ローカルに複製し、他Phase（59/73/80）からimportしない
- **transient I/O失敗はfallbackの理由にしない**: `TerminalHistoryContractError` の `__cause__` が `WorkflowExecutionLoadError` の場合は `terminal_contract` のまま（strict-path readがtransient失敗してもretry read成功ではfallbackに入らない）。raw `OSError` もstrict経路のまま `terminal_contract`
...
```text
Phase 155 result (StepRuntimeExecutionSuccess / Failure, または stop)
    ↓ Phase 175 post-runtime → durable persistence → classification → progression → approval/preparation → prepared-step start
...
### stage ownership（Phase-175 no-destructive-outer-rollback）

- Phase 175 が durable runtime-result commit point と自身の downstream compensation を所有するため、Phase 176 は **pre-Phase175 rollback を行わない**
- Phase 175 stage が safe error を raise した場合は exact object identity で re-raise（Phase 147 呼び出し 0 回、target への write なし）
- 予期しない例外は `dependency_error` に sanitize、不正戻り値は `phase175_contract`（いずれも Phase 147 呼び出し 0 回・write なし）
- Phase 175 retry なし

### committed terminal snapshot / Phase-147 ownership

- 有効な Phase 175 結果を受け入れた時点の target bytes を post-Phase175 committed terminal snapshot とする
- Phase 147 stop route は両 target を committed snapshot のまま要求（exact identity・mutation 不可）
- Phase 147 prepared route は state target の変更のみ許可。成功した state-only persistence が **新しい durable running-state commit point** になり、state target は exact UTF-8 serialization（`phase175_result.running_state`）、event bytes は committed event bytes のまま、`state_bytes_written` は exact positive int == persisted byte length、public storage loader で running state を再現
- Phase 147 の safe error は復元成功時のみ exact identity で re-raise、予期しない例外は sanitize、不正戻り値は `phase147_contract`、有効戻り値 + target mutation は `committed_mutation`（いずれも committed terminal bytes への復元のみ、pre-Phase175 へは戻さない）
- committed への復元失敗は `rollback_failure`（両 target を 1 回ずつ試行、stage retry なし）

### thin 契約（下流検証の非重複）

- Phase 175 出力: stop は exact identity、runtime success は exact `PreparedStepExecutionStart`（ModelInvocationRequest の model / system・task instructions / allowed_tools、running `WorkflowExecutionState` の workflow / status / current step・index・employee / completed prefix / `last_failure_category is None`）、final-step success は exact `workflow_complete`（next 全 None・`last_step_succeeded`）、runtime failure は exact `persisted_failure`（failure category 一致）
- Phase 147 出力: prepared は exact `RunningStatePersistenceResult`（positive int `state_bytes_written`）、stop は exact identity
- approval / employee は Phase 175 前に prevalidate しない（Phase 175 内部の責務）。Phase 175 / 147 の terminal-history・persistence validator は重複実装しない
...
| `target_conflict` | state_path == events_path |
| `configuration` | 依存関数が callable でない |
| `phase175_contract` | Phase 175 の戻り値・stop identity の契約違反 |
| `phase147_contract` | Phase 147 の戻り値・stop identity・persistence フィールドの契約違反 |
| `dependency_error` | 予期しない例外（sanitize、detail-safe 固定メッセージ） |
| `committed_mutation` | Phase 147 有効戻り値後の target mutation を検出 |
| `rollback_failure` | 補償 restore 自体が失敗 |

### 依存と no-bypass

- orchestration は公開 Phase 175 / 公開 Phase 147 のみ。Phase 173 / 146 / 139 / 132 以下の lower boundary は直接 import / call しない（error type の import は Phase 175 / 147 が expose / re-raise する type のみ、call はしない）
- import は公開 data / error type（`WorkflowDefinition`、`EmployeeDefinition`、`WorkflowProgressionDecision`、`PersistedExecutionOutcome`、`StepRuntimeExecutionSuccess/Failure`、`PreparedStepExecutionStart`、`ModelInvocationRequest`、`WorkflowExecutionState`、`RunningStatePersistenceResult`、`Path`）と両境界の public function + error class のみ
- 依存注入は keyword-only の `phase175_function` / `phase147_function`（default は実公開関数の exact identity）
- workflow loop・stage retry・alternate bypass path なし

### real-default regression（4 cases）

- A: 7-step step-6 success + valid approval/matching employee → exact `RunningStatePersistenceResult`。step-6 は実チェーンで durable commit、final state は step-7 running（completed prefix steps 1–6）、event bytes は six-event committed step-6 history のまま、step-7 runtime event / provider / tool 実行なし（canonical step-5 `provider="openai"` / `output_text=""` / `request_id=None` 維持）
- B: 6-step success + `None` approval/employee → exact `workflow_complete` identity、committed terminal state/event bytes 不変
- C: 6-step failure + `None` approval/employee → exact `persisted_failure` identity、committed terminal state/event bytes 不変
- D: 7-step step-6 success + missing approval（employee は matching）→ Phase 175 が先に step-6 を durable commit、既存 safe approval error が identity で escape、Phase 147 呼び出し 0 回、committed step-6 bytes 維持、step 7 は persist / execute されない
...
### canonical provenance（prerequisite #367 解決済み）

実 A/B/C/D は、Issue #363 が指定する **canonical Phase-155 provenance**（直前 step-5 が `provider="openai"` / `output_text=""` / `request_id=None`）のまま public Phase 175 → public Phase 147 を合成して検証している。prerequisite #367 のマージにより public Phase 147 の prepared route は canonical immediate-None prepared start（step 7 以降）を受理するため、strict provenance の deviation はない。

### 変更しないもの

- Phase 175 / 147 / 139 / 132 または下流 prepared-start persistence production、Phase 146 / 138 / 131、Phase 173 / 172 / 161 またはその下流チェーン、`terminal_history_contract.py`、`storage/running_state_persistence.py`
- 既存テストの削除・rename・skip・xfail・parameter-collapse・弱体化
...
```text
finished current-step Phase-155 result
...
### stage ownership（Phase-176 no-destructive-outer-rollback）

- Phase 176 が durable running-state commit point と自身の downstream compensation を所有するため、Phase 177 は **pre-Phase176 rollback を行わない**
- Phase 176 stage が safe error を raise した場合は exact object identity で re-raise（Phase 155 呼び出し 0 回、target への write なし）
- 予期しない例外は `dependency_error` に sanitize、不正戻り値は `phase176_contract`（いずれも Phase 155 呼び出し 0 回・write なし）
- Phase 176 retry なし

### committed running-state snapshot / Phase-155 ownership

- 有効な Phase 176 結果を受け入れた時点の target bytes を post-Phase176 committed running-state snapshot とする
- Phase 176 stop route は exact stop object（workflow_complete / persisted_failure）を identity で返し、Phase 155 は zero calls・両 target は committed snapshot のまま
- Phase 176 prepared route は exact `RunningStatePersistenceResult`（positive int `state_bytes_written`）＋ capture-only delegating Phase-147 adapter が実 Phase 147 handoff から capture した **ちょうど 1 件の exact `PreparedStepExecutionStart`** を要求（複数・不正・unlinked・stop 時の capture は `phase176_contract`、代替 fabricate なし）
- Phase 155 は prepared route でのみちょうど 1 回呼ばれ、captured start を exact な 10 引数（persistence result / start / workflow / employee / state / events / tools / api key / execution approval / transport）で受ける
- Phase 155 の safe error は committed snapshot への復元成功時のみ exact identity で re-raise、予期しない例外は sanitize、不正戻り値は `phase155_contract`、有効戻り値 + target mutation は `committed_mutation`（いずれも committed snapshot への復元のみ、pre-Phase176 へは戻さない）
- committed への復元失敗は `rollback_failure`（両 target を 1 回ずつ試行、stage retry なし）

### thin 契約（下流検証の非重複）

- Phase 176 出力: stop は exact identity（stop 入力なら `value is result`）＋ captured に `PreparedStepExecutionStart` が無いこと。prepared は exact `RunningStatePersistenceResult`（positive int）＋ captured ちょうど 1 件の exact start（request の model / system・task instructions / allowed_tools、running state の workflow / status / current step・index・employee / completed prefix / `last_failure_category is None`）＋ persisted state bytes が `serialize_workflow_execution_state_json(running)` と一致し public storage loader で復元一致
...
| `target_conflict` | state_path == events_path |
| `configuration` | 依存関数が callable でない |
| `phase176_contract` | Phase 176 の戻り値・stop identity・capture 契約の違反 |
| `phase155_contract` | Phase 155 の戻り値契約の違反 |
| `dependency_error` | 予期しない例外（sanitize、detail-safe 固定メッセージ） |
| `committed_mutation` | Phase 155 有効戻り値後の target mutation を検出 |
| `rollback_failure` | 補償 restore 自体が失敗 |

### 依存と no-bypass

- orchestration は公開 Phase 176 / 公開 Phase 147（capture-only adapter 内のみ）/ 公開 Phase 155 のみ。Phase 175 / 173 / 172 / 161、Phase 139 / 132、Phase 141 / 133 / 126 以下の lower boundary は直接 import / call しない（error type の import は各境界が expose / re-raise する type のみ、call はしない）
- 依存注入は keyword-only の `phase176_function` / `phase147_function` / `phase155_function`（default は実公開関数の exact identity）
- workflow loop・stage retry・alternate bypass path なし

### real-default regression（4 cases）

- A: 7-step step-6 success + valid approval/matching employee → 実チェーンで step-6 を durable commit し step-7 running state を persist、captured start で Phase 155 が step-7 をちょうど 1 回実行し exact `StepRuntimeExecutionSuccess`。transport 1 回、state / event bytes は Phase 176 alone の committed bytes と不変（six-event history 維持）
- B: 7-step step-6 success + deterministic failure transport → exact `StepRuntimeExecutionFailure`（category `api_error`）、transport 1 回、committed bytes 不変
- C: 6-step terminal success / failure → exact `workflow_complete` / `persisted_failure` identity、Phase 155 zero calls、committed terminal state / event bytes 不変
- D: 7-step step-6 success + invalid execution approval（`None`）→ Phase 176 が先に step-6 と step-7 running state を durable commit、Phase 155 が既存 safe approval error を identity で escape、transport 0 回、committed running step-7 bytes / six-event history 維持
...
実 A/B/C/D は、Issue #363 が指定する **canonical Phase-155 provenance**（直前 step-5 が `provider="openai"` / `output_text=""` / `request_id=None`）のまま public Phase 176 → capture-only delegating Phase 147 → public Phase 155 を合成して検証している。step-7 実行は canonical immediate-None prepared start で行われ、strict provenance の deviation はない。

### 変更しないもの

- Phase 176 / 175 / 173 / 172 / 161、Phase 147 / 139 / 132 または下流 prepared-start persistence production、Phase 155 / 141 / 133 / 126 または下流 runtime execution production、`terminal_history_contract.py`、`storage/running_state_persistence.py`
- 既存テストの削除・rename・skip・xfail・parameter-collapse・弱体化
...
Issue #380は、Phase 178（Issue #377）の前提として、**7つの境界すべて**が accumulated aged-None compatibility の対象であるが、**適用ルートは境界ごとに限定**される。accumulated aged None の許容は、境界時点の current / terminal index が **7以上**（predecessor 履歴が6件以上 = `last_position >= 6`）かつ **predecessor position >= 5** の None request-ID に対して、以下のルートでのみ適用される:
...
16. `README.md` — 本 prerequisite の documentation
17. `docs/architecture.md` — 本節
...
Phase 300 recovery-resume operator decision. The caller supplies only the exact
Phase 295 `resume_intent_binding_path`. Phase 299 is the sole orchestration
dependency and is called exactly once; the Phase 300 decision path is derived
internally from the returned Phase 299 outcome digest.
...
Only an exact Phase 299 `DecisionRequired` model is accepted after Phase 301
reconstructs it through the public model constructor. A completed Phase 298
route is classified as `preparation_not_required` before any Phase 300
decision loader or digest call and creates no Phase 301 artifact. An exact
Phase 300 `stop` decision is classified as `preparation_not_authorized` before
the Phase 300 decision digest and creates no Phase 301 artifact. Only the exact
`authorize_resume_preparation` decision can proceed.

Phase 301 independently reconstructs the strict-loaded Phase 300 decision and
compares every Phase 299 → Phase 300 provenance field before computing the
decision digest: outcome, start authorization, binding, operation intent,
operation start, approval, plan, source operation, previous/current recovery
kind, operation, result kind/digest, and decided state. The digest is computed
exactly once from the original object returned by the Phase 300 loader and is
bound directly to the preparation. Phase 301 never recalculates the current
recovery reason. Thus both previous `already_acquired` → current
`reconciliation_mismatch` and previous `reconciliation_mismatch` → current
`already_acquired` are preserved exactly.

The Phase 301 frozen secret-free model has exactly seventeen fields and the
canonical JSON has exactly seventeen keys. It contains no raw paths, provider,
transport, credential, timestamp, UUID, randomness, hostname, PID, or
exception text. Preparation persistence uses exclusive create, full write,
flush, file fsync, safe close, and parent-directory fsync. Identical existing
canonical bytes are durable idempotent success and return the strict-loaded
object identity. Different, partial, or noncanonical occupied bytes fail
closed without overwrite, cleanup, retry, or rewrite. Ambiguous failures after
exclusive creation retain the artifact.
...
preparation. It is not intent existence, start authorization, start
acquisition, execution authority, retry permission, or automatic continuation.
Phase 294's preparation is not reused as authority. Phase 301 calls no Phase
294 or Phase 300 orchestration, no Phase 298/297/293/291/290 acquisition or
lower boundary, and no provider, transport, runtime request, network,
