"""Phase 77 cycle continuation boundary contract tests."""
...
def call(values_: dict[str, object], dependency: object) -> object:
    return route_persisted_running_execution_routing_phase_bridge_cycle_continuation(
        **values_, phase70_function=dependency
...
def test_execution_delegates_once_with_identity_and_returns_exact_result(
    ...
    def phase70(*args: object) -> object:
        calls.append(args)
        assert all(
    ...
    returned = call(supplied, phase70)
...
def test_terminal_routes_stop_without_phase70_and_preserve_identity(
...
def test_target_oserror_is_classified_before_phase70(
...
def test_safe_phase70_error_identity_is_preserved_and_not_retried(tmp_path: Path) -> None:
    supplied, calls = values(tmp_path), 0
    error = PersistedRunningExecutionRoutingPhaseBridgeContinuationError("private")

    def phase70(*_: object) -> object:
        nonlocal calls
        calls += 1
        raise error

    with pytest.raises(PersistedRunningExecutionRoutingPhaseBridgeContinuationError) as caught:
        call(supplied, phase70)
    assert caught.value is error and calls == 1


def test_unexpected_phase70_error_is_sanitized_and_not_retried(tmp_path: Path) -> None:
    supplied, calls = values(tmp_path), 0
