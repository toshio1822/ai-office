...
def test_phase155_success_delegates_to_phase101_exactly_once(tmp_path: Path) -> None:
    """Phase-155 persisted success delegates to Phase 101 exactly once with
    ...
    returned = route_classified_persisted_outcome_progression_cycle_continuation_boundary(
        result, definition, state, events, phase101_function=dependency
    )
    ...
    alt_returned = route_classified_persisted_outcome_progression_cycle_continuation_boundary(
        alt[0],
        alt[1],
        alt[2],
        alt[3],
        phase101_function=alt_dependency,
    )
    ...
    pin_calls = {"phase101": 0}

    def pin_dependency(*_: object) -> object:
        pin_calls["phase101"] += 1
    ...
    with pytest.raises(
        ...
        route_classified_persisted_outcome_progression_cycle_continuation_boundary(
            pin[0], pin[1], pin[2], pin[3], phase101_function=pin_dependency
        )
    assert classification(caught) == "terminal_contract"
    assert pin_calls["phase101"] == 0
    ...
    pin2_calls = {"phase101": 0}

    def pin2_dependency(*_: object) -> object:
        pin2_calls["phase101"] += 1
        raise AssertionError("must not be called")
    ...
    with pytest.raises(
        ...
        route_classified_persisted_outcome_progression_cycle_continuation_boundary(
            pin2[0], pin2[1], pin2[2], pin2[3], phase101_function=pin2_dependency
        )
    assert classification(caught) == "terminal_contract"
    assert pin2_calls["phase101"] == 0
    ...
    pin3_calls = {"phase101": 0}

    def pin3_dependency(*_: object) -> object:
        pin3_calls["phase101"] += 1
        raise AssertionError("must not be called")
    ...
    with pytest.raises(
        ...
        route_classified_persisted_outcome_progression_cycle_continuation_boundary(
            ...
            pin3[2],
            pin3[3],
            phase101_function=pin3_dependency,
        )
    assert classification(caught) == "terminal_contract"
    assert pin3_calls["phase101"] == 0
...
def test_phase155_failure_empty_message_returns_unchanged_zero_calls(
    ...
    supplied object with Phase 101 call count zero and unchanged targets."""
    ...
    returned = route_classified_persisted_outcome_progression_cycle_continuation_boundary(
        result, definition, state, events, phase101_function=forbidden
    )
    assert returned is result
    assert calls == {"phase101": 0}
    assert (state.read_bytes(), events.read_bytes()) == before
...
def test_phase155_success_multiple_earlier_empty_delegates_once(tmp_path: Path) -> None:
    ...
    2 and 3) delegates to Phase 101 exactly once with unchanged targets."""
    result, definition, state, events = phase155_setup(
...
def test_phase155_step2_output_none_rejected_before_phase101(tmp_path: Path) -> None:
    """Earlier predecessor step 2 output_text=None rejects with exact
    terminal_contract before Phase 101 with zero calls."""
    result, definition, state, events = phase155_setup(tmp_path, "succeeded")
    _phase155_replace_event(
        events,
        2,
...
def test_phase155_step5_output_non_string_rejected_before_phase101(
    tmp_path: Path,
) -> None:
    """Immediate predecessor step 5 output_text=1 (non-string) rejects with
    exact terminal_contract before Phase 101 with zero calls."""
    result, definition, state, events = phase155_setup(tmp_path, "succeeded")
    _phase155_replace_event(
        events,
        5,
