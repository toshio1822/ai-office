"""Focused Phase 117 prepared-step start cycle handoff reentry continuation tests."""
...
from ai_office.engine.prepared_step_start_cycle_reentry_continuation_boundary import (
...
def invoke(result: object, person: object | None, state: Path, events: Path, function=None):
    kwargs = {} if function is None else {"phase110_function": function}
    return route_prepared_step_start_cycle_handoff_reentry_continuation_boundary(result, workflow(), person, state, events, **kwargs)


def test_implementation_uses_only_public_phase110_dependency() -> None:
    source = Path(
        "src/ai_office/engine/"
        "prepared_step_start_cycle_handoff_reentry_continuation_boundary.py"
    ).read_text()
    assert "route_prepared_step_start_cycle_reentry_continuation_boundary" in source
    assert "phase103" not in source.lower()
    assert "prepared_step_start_cycle_continuation_boundary" not in source
    assert "._validate_" not in source
    assert "._raise" not in source

def test_public_signature_and_phase110_signature_are_canonical() -> None:
    parameters = tuple(inspect.signature(route_prepared_step_start_cycle_handoff_reentry_continuation_boundary).parameters)
    assert parameters[:5] == ("result", "workflow", "employee", "state_path", "events_path")
    assert tuple(inspect.signature(route_prepared_step_start_cycle_reentry_continuation_boundary).parameters)[:5] == parameters[:5]
...
def test_prepared_route_delegates_exact_five_arguments_once_and_returns_identity(tmp_path: Path) -> None:
    ...
    def phase110(*args: object) -> object:
        received.extend(args)
        return started()

    result = route_prepared_step_start_cycle_handoff_reentry_continuation_boundary(*supplied, phase110_function=phase110)
...
def test_stop_routes_are_zero_call_and_preserve_identity(tmp_path: Path, result: object, status: str, index: int) -> None:
    ...
    def phase110(*_: object) -> object:
        nonlocal calls
        calls += 1
        raise AssertionError

    assert invoke(result, None, state, events, phase110) is result and calls == 0
...
def test_stop_routes_reject_employee(tmp_path: Path, result: object) -> None:
    ...
    with pytest.raises(PreparedStepStartCycleHandoffReentryContinuationCompatibilityError) as caught:
...
def test_direct_success_and_terminal_predecessor_mismatch_are_rejected(tmp_path: Path) -> None:
    state, events = targets(tmp_path)
    with pytest.raises(PreparedStepStartCycleHandoffReentryContinuationCompatibilityError):
...
def test_safe_error_identity_and_compensation(tmp_path: Path, mutation: str) -> None:
    ...
    with pytest.raises(PreparedStepStartCycleHandoffReentryContinuationError) as caught:
        invoke(prepared(), employee(), state, events, phase110)
...
def test_missing_target_and_target_conflict_are_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(PreparedStepStartCycleHandoffReentryContinuationCompatibilityError) as caught:
        route_prepared_step_start_cycle_handoff_reentry_continuation_boundary(prepared(), workflow(), employee(), state, state)
...
def test_public_signature_parameter_kinds_and_dependency_contract() -> None:
    signature = inspect.signature(route_prepared_step_start_cycle_handoff_reentry_continuation_boundary)
    ...
    assert signature.parameters["phase110_function"].default is route_prepared_step_start_cycle_reentry_continuation_boundary
...
def test_non_callable_dependency_is_rejected_before_dispatch(tmp_path: Path, dependency: object) -> None:
    state, events = targets(tmp_path)
    with pytest.raises(PreparedStepStartCycleHandoffReentryContinuationCompatibilityError) as caught:
        route_prepared_step_start_cycle_handoff_reentry_continuation_boundary(prepared(), workflow(), employee(), state, events, phase110_function=dependency)
    assert caught.value.detail.classification == "start_contract"
...
def test_targets_require_exact_path_models(tmp_path: Path, state_value: object, events_value: object, classification: str) -> None:
    ...
    with pytest.raises(PreparedStepStartCycleHandoffReentryContinuationCompatibilityError) as caught:
        route_prepared_step_start_cycle_handoff_reentry_continuation_boundary(prepared(), workflow(), employee(), bad_state, bad_events)
...
def test_workflow_exact_model_and_every_field_is_prevalidated(tmp_path: Path, bad_workflow: object) -> None:
    state, events = targets(tmp_path)
    with pytest.raises(PreparedStepStartCycleHandoffReentryContinuationCompatibilityError) as caught:
        route_prepared_step_start_cycle_handoff_reentry_continuation_boundary(prepared(), bad_workflow, employee(), state, events)
...
def test_workflow_field_types_are_strict(tmp_path: Path, field: str, value: object) -> None:
    state, events = targets(tmp_path)
    with pytest.raises(PreparedStepStartCycleHandoffReentryContinuationCompatibilityError) as caught:
        route_prepared_step_start_cycle_handoff_reentry_continuation_boundary(prepared(), workflow().model_copy(update={field: value}), employee(), state, events)
    assert caught.value.detail.classification == "workflow_definition"
...
def test_path_identity_and_equal_but_distinct_paths_are_not_a_dependency_route(tmp_path: Path) -> None:
    ...
    assert route_prepared_step_start_cycle_handoff_reentry_continuation_boundary(prepared(), workflow(), employee(), equal_state, events, phase110_function=phase110).request.model == "model"
    assert received[3] is equal_state and received[4] is events
    with pytest.raises(PreparedStepStartCycleHandoffReentryContinuationCompatibilityError) as caught:
        route_prepared_step_start_cycle_handoff_reentry_continuation_boundary(prepared(), workflow(), employee(), state, Path(str(state)))
    assert caught.value.detail.classification == "target_conflict"
...
def test_completion_all_fields_are_strict_and_zero_call(tmp_path: Path, field: str, value: object) -> None:
    ...
    with pytest.raises(PreparedStepStartCycleHandoffReentryContinuationCompatibilityError):
        invoke(bad, None, state, events, phase110)
    assert calls == 0
...
def test_every_dependency_path_is_one_call_and_byte_for_byte_compensated(tmp_path: Path, kind: str, mutation: str) -> None:
    ...
    expected = PreparedStepStartCycleHandoffReentryContinuationError if kind == "safe" else PreparedStepStartCycleHandoffReentryContinuationCompatibilityError
...
def test_valid_start_with_dependency_mutation_is_restored_and_rejected(tmp_path: Path, mutation: str) -> None:
    state, events = targets(tmp_path)
    before = state.read_bytes(), events.read_bytes()
    calls = 0
...
def test_malformed_terminal_history_is_rejected_before_dependency(tmp_path: Path) -> None:
    state, events = targets(tmp_path)
    calls = 0
