"""Focused Phase 111 prepared-start persistence cycle reentry continuation tests."""
...
def reject_classification(callable_object, classification: str) -> None:
    with pytest.raises(PreparedStartPersistenceCycleReentryContinuationCompatibilityError) as caught:
        callable_object()
...
def invoke(result: object, employee: object | None, state: Path, events: Path, function=None):
    kwargs = {} if function is None else {"phase104_function": function}
    return route_prepared_start_persistence_cycle_reentry_continuation_boundary(result, definition(), employee, state, events, **kwargs)
...
def test_implementation_uses_only_public_phase104_dependency() -> None:
    ...
    assert "phase97" not in source.lower()
    assert "prepared_start_persistence_dispatch_continuation_boundary" not in source
    assert "._inputs" not in source
    assert "._persisted" not in source
    assert "._raise" not in source


def test_signature_is_canonical() -> None:
    signature = inspect.signature(route_prepared_start_persistence_cycle_reentry_continuation_boundary)
    assert tuple(signature.parameters)[:5] == ("result", "workflow", "employee", "state_path", "events_path")
    assert signature.parameters["phase104_function"].default is route_prepared_start_persistence_cycle_continuation_boundary


def test_prepared_route_calls_phase104_once_and_returns_identity(tmp_path: Path) -> None:
    ...
    actual = route_prepared_start_persistence_cycle_reentry_continuation_boundary(result, workflow, employee, state, events, phase104_function=dependency)
...
def test_workflow_and_employee_subclasses_are_rejected(tmp_path: Path) -> None:
    ...
    reject_classification(lambda: route_prepared_start_persistence_cycle_reentry_continuation_boundary(start(), workflow, person(), state, events), "workflow_definition")
...
def test_employee_and_workflow_mismatch_rejection(tmp_path: Path) -> None:
    ...
    reject_classification(lambda: route_prepared_start_persistence_cycle_reentry_continuation_boundary(start(), wrong_workflow, person(), state, events), "start_contract")
...
def test_stop_routes_reject_employee_and_prepared_requires_exact_employee(tmp_path: Path) -> None:
    ...
    with pytest.raises(PreparedStartPersistenceCycleReentryContinuationCompatibilityError):
...
def test_safe_error_compensates_each_target_shape(tmp_path: Path, changed: str) -> None:
    state, events = targets(tmp_path)
    original = state.read_bytes(), events.read_bytes()
    error = PreparedStartPersistenceCycleContinuationError("safe")
    ...
    with pytest.raises(PreparedStartPersistenceCycleContinuationError) as caught:
        invoke(start(), person(), state, events, dependency)
...
def test_safe_phase104_error_preserves_identity_after_compensation(tmp_path: Path) -> None:
    state, events = targets(tmp_path)
    error = PreparedStartPersistenceCycleContinuationError("safe")
...
def test_public_signature_kinds_default_identity_and_non_callable_dependency(tmp_path: Path) -> None:
    ...
    assert signature.parameters["phase104_function"].default is not None
    state, events = targets(tmp_path)
    for dependency in (None, 1, object()):
        reject_classification(lambda dependency=dependency: route_prepared_start_persistence_cycle_reentry_continuation_boundary(start(), definition(), person(), state, events, phase104_function=dependency), "persistence_contract")
...
def test_workflow_all_field_types_and_substitutes_are_rejected(tmp_path: Path, field: str, value: object) -> None:
    ...
    reject_classification(lambda: route_prepared_start_persistence_cycle_reentry_continuation_boundary(start(), bad, person(), state, events), "workflow_definition")
    substitute = SimpleNamespace(**definition().model_dump())
    reject_classification(lambda: route_prepared_start_persistence_cycle_reentry_continuation_boundary(start(), substitute, person(), state, events), "workflow_definition")
    child = WorkflowChild.model_validate(definition().model_dump())
    reject_classification(lambda: route_prepared_start_persistence_cycle_reentry_continuation_boundary(start(), child, person(), state, events), "workflow_definition")
...
def test_path_identity_and_conflict_are_strict(tmp_path: Path) -> None:
    ...
    assert route_prepared_start_persistence_cycle_reentry_continuation_boundary(start(), definition(), person(), equal_state, events, phase104_function=dependency) is expected
