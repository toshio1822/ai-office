...
def call(values_: dict[str, object], dependency: object) -> object:
    return route_persisted_running_execution_routing_phase_bridge_cycle_reentry_continuation(
        **values_, phase77_function=dependency
...
def test_execution_delegates_once_with_identity_and_returns_exact_result(
    ...
    def phase77(*args: object) -> object:
        calls.append(args)
        assert all(
    ...
    returned = call(supplied, phase77)
...
def test_terminal_routes_stop_without_phase77_and_preserve_identity(
...
def test_target_oserror_is_classified_before_phase77(
...
def test_safe_phase77_error_identity_is_preserved_and_not_retried(tmp_path: Path) -> None:
    supplied, calls = values(tmp_path), 0
    error = PersistedRunningExecutionRoutingPhaseBridgeCycleContinuationError("private")
    ...
    with pytest.raises(PersistedRunningExecutionRoutingPhaseBridgeCycleContinuationError) as caught:
        call(supplied, phase77)
    assert caught.value is error and calls == 1


def test_unexpected_phase77_error_is_sanitized_and_not_retried(tmp_path: Path) -> None:
    supplied, calls = values(tmp_path), 0
