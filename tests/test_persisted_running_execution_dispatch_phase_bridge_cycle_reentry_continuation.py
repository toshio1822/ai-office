"""Phase 84 cycle reentry continuation boundary contract tests."""
...
def call(values_: dict[str, object], dependency: object) -> object:
    return route_persisted_running_execution_dispatch_phase_bridge_cycle_reentry_continuation(
        **values_, phase84_function=dependency
...
def test_execution_delegates_once_with_identity_and_returns_exact_result(
    ...
    def phase84(*args: object) -> object:
        calls.append(args)
        assert all(
    ...
    returned = call(supplied, phase84)
...
def test_terminal_routes_stop_without_phase84_and_preserve_identity(
...
def test_target_oserror_is_classified_before_phase84(
...
def test_employee_all_fields_require_exact_builtin_types_and_zero_phase84_calls(
    tmp_path: Path, field: str, value: object
) -> None:
    supplied = values(tmp_path)
    ...
    def phase84(*_: object) -> object:
        nonlocal calls
        calls += 1
        raise AssertionError("Phase 84 must not be called")

    with pytest.raises(
        PersistedRunningExecutionDispatchPhaseBridgeCycleReentryContinuationCompatibilityError
    ) as caught:
        call(supplied, phase84)
...
def test_safe_phase84_error_identity_is_preserved_and_not_retried(tmp_path: Path) -> None:
    supplied, calls = values(tmp_path), 0
    error = PersistedRunningExecutionRoutingPhaseBridgeCycleReentryContinuationError("private")
    ...
    with pytest.raises(PersistedRunningExecutionRoutingPhaseBridgeCycleReentryContinuationError) as caught:
        call(supplied, phase84)
    assert caught.value is error and calls == 1


def test_unexpected_phase84_error_is_sanitized_and_not_retried(tmp_path: Path) -> None:
    supplied, calls = values(tmp_path), 0
