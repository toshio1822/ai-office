"""Phase 113 runtime-result transition persistence cycle reentry continuation tests."""
...
def call(
    ...
    return (
        route_runtime_result_transition_persistence_cycle_reentry_continuation_boundary(
            result, workflow_value, state, events, phase106_function=dependency
        )
...
def test_public_signature_is_canonical_and_keyword_only_dependency() -> None:
    ...
    assert tuple(signature.parameters) == (
        "result", "workflow", "state_path", "events_path", "phase106_function"
    )
    assert all(
    ...
    dependency = signature.parameters["phase106_function"]
    assert dependency.kind is inspect.Parameter.KEYWORD_ONLY
    from ai_office.engine.runtime_result_transition_persistence_cycle_continuation_boundary import (
        route_runtime_result_transition_persistence_cycle_continuation_boundary,
    )
    assert dependency.default is route_runtime_result_transition_persistence_cycle_continuation_boundary
def test_production_module_references_only_the_public_phase106_dependency() -> None:
    import ai_office.engine.runtime_result_transition_persistence_cycle_reentry_continuation_boundary as module
    source = inspect.getsource(module)
    assert "runtime_result_transition_persistence_cycle_continuation_boundary" in source
    assert "executed_result_transition_persistence_dispatch_continuation_boundary" not in source
    assert "phase99" not in source
    assert "runtime_result_transition_persistence_cycle_continuation_boundary._" not in source
...
def test_malformed_stop_and_unsupported_progression_are_zero_call(
    ...
    with pytest.raises(RuntimeResultTransitionPersistenceCycleReentryContinuationCompatibilityError) as caught:
        call(result, workflow(), state, events, lambda *_: pytest.fail("called"))
...
def test_safe_error_identity_and_unexpected_error_sanitization(tmp_path: Path) -> None:
    state, events = setup(tmp_path)
    safe = RuntimeResultTransitionPersistenceCycleContinuationError("private")
    with pytest.raises(
        RuntimeResultTransitionPersistenceCycleContinuationError
    ) as caught:
        call(
...
def test_safe_error_identity_is_preserved_after_successful_compensation(
    ...
    state, events = setup(tmp_path)
    before = state.read_bytes(), events.read_bytes()
    safe = RuntimeResultTransitionPersistenceCycleContinuationError(
        "private"
    )
...
def test_safe_error_compensates_each_mutation_shape_and_calls_once(
    ...
    with pytest.raises(RuntimeResultTransitionPersistenceCycleContinuationError) as caught:
        call(success(), workflow(), state, events, dependency)
    assert caught.value is safe
    assert calls == 1
    assert (state.read_bytes(), events.read_bytes()) == before
...
def test_earlier_predecessor_empty_output_delegates_once_to_phase106(
    tmp_path: Path,
) -> None:
    ...
    def phase106(*args: object) -> object:
        nonlocal expected
        calls.append(args)
        assert all(
            actual is wanted
    ...
    assert call(result, supplied_workflow, state, events, phase106) is expected
    assert len(calls) == 1


def test_immediate_predecessor_empty_output_delegates_once_to_phase106(
    tmp_path: Path,
) -> None:
    state, events = setup_multi_history(tmp_path, ("output", ""))
...
def test_multiple_earlier_and_immediate_empty_outputs_delegate_once_to_phase106(
    tmp_path: Path,
) -> None:
...
def test_predecessor_output_text_non_string_is_rejected_before_phase106(
    tmp_path: Path, value: object
) -> None:
