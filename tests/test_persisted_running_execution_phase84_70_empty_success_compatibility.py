"""Phase 152 segment regression: empty succeeded predecessor output through the real Phase 84 → 77 → 70 chain.

The synthetic Phase 63 seam is the only stubbed boundary, so this proves the three
...
execution. Phase 63 itself remains strict (next explicit seam, out of scope for
Phase 152).
"""
...
def _chain(seam: object) -> tuple[object, dict[str, list[tuple[object, ...]]]]:
    """Real Phase 84 → 77 → 70 chain with only the Phase 63 seam stubbed.
    ...
    def phase70_wrapper(*args: object, **kwargs: object) -> object:
        calls["phase70"].append(args)
        return route_persisted_running_execution_routing_phase_bridge_continuation(
            *args, **kwargs, phase63_function=seam_wrapper  # type: ignore[arg-type]
        )

    def phase77_wrapper(*args: object, **kwargs: object) -> object:
        calls["phase77"].append(args)
        return route_persisted_running_execution_routing_phase_bridge_cycle_continuation(
            *args, **kwargs, phase70_function=phase70_wrapper  # type: ignore[arg-type]
        )

    def route(*args: object, **kwargs: object) -> object:
        calls["phase84"].append(args)
        return route_persisted_running_execution_routing_phase_bridge_cycle_reentry_continuation(
            *args, **kwargs, phase77_function=phase77_wrapper  # type: ignore[arg-type]
        )
...
def _assert_exactly_once_canonical(
    ...
    for name in ("phase84", "phase77", "phase70", "seam"):
...
def test_non_string_predecessor_output_still_rejected_before_real_chain(
    ...
    except PersistedRunningExecutionRoutingPhaseBridgeCycleReentryContinuationCompatibilityError as caught:
    ...
    assert calls["phase77"] == [] and calls["phase70"] == [] and calls["seam"] == []
