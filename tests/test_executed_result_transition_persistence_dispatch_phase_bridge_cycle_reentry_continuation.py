"""Phase 92 cycle reentry continuation boundary contract tests."""
...
def call(
    ...
    return (
        route_executed_result_transition_persistence_dispatch_phase_bridge_cycle_reentry_continuation(
            result, workflow_value, state, events, phase85_function=dependency
        )
...
def test_invalid_predecessor_output_is_rejected_before_phase85(
    ...
    def dependency(*_: object) -> object:
        nonlocal calls
        calls += 1
        pytest.fail("Phase 85 must not be called")
...
def test_safe_error_identity_is_preserved_after_successful_compensation(
    ...
    state, events = setup(tmp_path)
    before = state.read_bytes(), events.read_bytes()
    safe = ExecutedResultTransitionPersistenceRoutingPhaseBridgeCycleReentryContinuationError(
        "private"
    )
