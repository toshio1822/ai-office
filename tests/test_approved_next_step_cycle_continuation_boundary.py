"""Focused Phase 102 approved-next-step cycle continuation tests."""
...
def call(*args: object, phase95_function=None) -> object:
    kwargs = {} if phase95_function is None else {"phase95_function": phase95_function}
    return route_approved_next_step_cycle_continuation_boundary(*args, **kwargs)
...
def test_implementation_uses_only_public_phase95_dependency() -> None:
    ...
    assert "route_approved_next_step_preparation_dispatch_phase_bridge_cycle_reentry_continuation" in source
...
def test_public_signature_and_exact_six_argument_identity(tmp_path: Path) -> None:
    assert tuple(inspect.signature(route_approved_next_step_cycle_continuation_boundary).parameters)[:6] == ("result", "workflow", "approval", "employee", "state_path", "events_path")
    ...
    assert call(*supplied, phase95_function=fake) is not None
...
def test_prepare_delegates_once_and_returns_exact_dependency_object(tmp_path: Path) -> None:
    ...
    assert call(decision(), wf(), approval(), employee(), state, events, phase95_function=fake) is value
    assert calls == 1
...
def test_completion_and_failure_are_identity_preserving_zero_call_stops(tmp_path: Path) -> None:
    ...
    assert call(complete, wf(), None, None, state, events, phase95_function=fake) is complete
    ...
    assert call(failed, wf(), None, None, state, events, phase95_function=fake) is failed
    assert calls == 0
...
def test_exact_models_and_stop_context_rejected(tmp_path: Path, bad: object) -> None:
    state, events = targets(tmp_path)
    with pytest.raises(ApprovedNextStepCycleContinuationCompatibilityError):
...
def test_safe_error_identity_and_unexpected_error_are_sanitized(tmp_path: Path) -> None:
    state, events = targets(tmp_path); safe = ApprovedNextStepPreparationDispatchPhaseBridgeCycleReentryContinuationError("safe")
    with pytest.raises(ApprovedNextStepPreparationDispatchPhaseBridgeCycleReentryContinuationError) as caught:
        call(decision(), wf(), approval(), employee(), state, events, phase95_function=lambda *_: (_ for _ in ()).throw(safe))
    assert caught.value is safe
    with pytest.raises(ApprovedNextStepCycleContinuationCompatibilityError) as caught:
        call(decision(), wf(), approval(), employee(), state, events, phase95_function=lambda *_: (_ for _ in ()).throw(RuntimeError("secret")))
...
def test_mutation_is_compensated_and_malformed_return_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(ApprovedNextStepCycleContinuationCompatibilityError):
        call(decision(), wf(), approval(), employee(), state, events, phase95_function=fake)
...
def test_signature_has_keyword_only_dependency() -> None:
    ...
    assert parameters["phase95_function"].kind is inspect.Parameter.KEYWORD_ONLY
    assert parameters["phase95_function"].default is route_approved_next_step_preparation_dispatch_phase_bridge_cycle_reentry_continuation
...
def test_prepared_return_field_matrix(tmp_path: Path, field: str) -> None:
    ...
    with pytest.raises(ApprovedNextStepCycleContinuationCompatibilityError):
        call(decision(), wf(), approval(), employee(), state, events,
             phase95_function=lambda *_: replace(prepared(), **{field: invalid[field]}))
...
def test_prepared_subclass_and_substitute_rejected(tmp_path: Path) -> None:
    ...
    for value in (child, object()):
        with pytest.raises(ApprovedNextStepCycleContinuationCompatibilityError):
            call(decision(), wf(), approval(), employee(), state, events, phase95_function=lambda *_: value)
...
def test_safe_error_identity_after_each_mutation(tmp_path: Path, target_kind: str) -> None:
    state, events = targets(tmp_path)
    safe = ApprovedNextStepPreparationDispatchPhaseBridgeCycleReentryContinuationError("safe")
    with pytest.raises(ApprovedNextStepPreparationDispatchPhaseBridgeCycleReentryContinuationError) as caught:
        call(decision(), wf(), approval(), employee(), state, events,
             phase95_function=_mutating_error_fake(state, events, target_kind, safe))
    assert caught.value is safe
