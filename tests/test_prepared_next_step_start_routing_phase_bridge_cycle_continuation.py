"""Focused Phase 75 boundary tests using injected Phase 68 fakes only."""
...
from ai_office.engine.prepared_next_step_start_routing_phase_bridge_continuation import (
    PreparedNextStepStartRoutingPhaseBridgeContinuationError as Phase68Error,
)
...
def test_prepared_route_delegates_once_with_exact_identity_and_returns_start(
    ...
    def phase68(*args: object) -> PreparedStepExecutionStart:
        nonlocal calls
        calls += 1
    ...
    result = route_prepared_next_step_start_routing_phase_bridge_cycle_continuation(
        supplied, definition, person, state, events, phase68_function=phase68
    )
...
def test_stop_routes_return_same_object_without_phase68(
    tmp_path: Path, result, status: str, index: int
) -> None:
    state, events = targets(tmp_path, status, index)
    before = state.read_bytes(), events.read_bytes()

    def phase68(*args: object) -> object:
        raise AssertionError("Phase 68 must not run")

    returned = route_prepared_next_step_start_routing_phase_bridge_cycle_continuation(
        result, workflow(), None, state, events, phase68_function=phase68
    )
...
def test_every_prepared_field_is_strict_and_prevalidated(
    ...
    with pytest.raises(
        ...
        route_prepared_next_step_start_routing_phase_bridge_cycle_continuation(
            bad, workflow(), employee(), state, events, phase68_function=phase68
        )
...
def test_returned_request_fields_are_strict(
    ...
    with pytest.raises(
        ...
        route_prepared_next_step_start_routing_phase_bridge_cycle_continuation(
            prepared(), workflow(), employee(), state, events, phase68_function=phase68
        )
...
def test_safe_error_identity_is_preserved_after_compensation(tmp_path: Path) -> None:
    state, events = targets(tmp_path)
    before = state.read_bytes(), events.read_bytes()
    safe = Phase68Error("safe")

    def phase68(*args: object):
        state.write_bytes(b"changed")
        raise safe

    with pytest.raises(Phase68Error) as error:
        route_prepared_next_step_start_routing_phase_bridge_cycle_continuation(
            prepared(), workflow(), employee(), state, events, phase68_function=phase68
        )
...
@pytest.mark.parametrize(
    "bad_result,bad_workflow,bad_employee,bad_state,bad_events,bad_dependency,classification",
    [
        (object(), workflow(), employee(), None, None, None, "result_type"),
        (prepared(), WorkflowSubclass.model_validate(workflow().model_dump()), employee(), None, None, None, "workflow_definition"),
        (prepared(), workflow(), EmployeeSubclass.model_validate(employee().model_dump()), None, None, None, "employee_contract"),
        (prepared(), workflow(), employee(), PathSubclass("state"), None, None, "state_target"),
        (prepared(), workflow(), employee(), None, PathSubclass("events"), None, "event_target"),
        (prepared(), workflow(), employee(), None, None, object(), "start_contract"),
    ],
)
def test_all_supplied_models_targets_and_dependency_are_exact(
    tmp_path: Path, bad_result, bad_workflow, bad_employee, bad_state, bad_events,
    bad_dependency, classification: str,
) -> None:
    state, events = targets(tmp_path)
    state_value = state if bad_state is None else bad_state
    events_value = events if bad_events is None else bad_events
    dependency = (lambda *args: started()) if bad_dependency is None else bad_dependency
    assert_classification(
        lambda: route_prepared_next_step_start_routing_phase_bridge_cycle_continuation(
            bad_result, bad_workflow, bad_employee, state_value, events_value,
            phase68_function=dependency,
        ),
        classification,
    )
...
@pytest.mark.parametrize("person", [employee(), object()])
def test_stop_routes_require_no_employee(tmp_path: Path, value, person) -> None:
    ...
    assert_classification(
        lambda: route_prepared_next_step_start_routing_phase_bridge_cycle_continuation(
            value, workflow(), person, state, events,
        ),
...
@pytest.mark.parametrize(
    "value",
    [
        PersistedExecutionOutcome("persisted_success", "workflow", "first", 1, "one", None),
        replace(completion(), decision="stopped_failed"),
        replace(completion(), decision="not_progressable"),
    ],
)
def test_direct_success_and_unsupported_decisions_are_rejected(tmp_path: Path, value) -> None:
...
def test_missing_non_regular_and_conflicting_targets_are_rejected(tmp_path: Path, which: str) -> None:
    state, events = targets(tmp_path)
...
def test_each_dependency_target_mutation_is_restored_without_retry(tmp_path: Path, mutation: str) -> None:
    state, events = targets(tmp_path)
    before = state.read_bytes(), events.read_bytes()
    calls = 0
...
def test_predecessor_terminal_history_fields_are_strict(tmp_path: Path) -> None:
    state, events = targets(tmp_path)
    payload = json.loads(state.read_text())
    payload["completed_step_ids"] = ["second"]
...
def test_safe_error_identity_is_preserved_without_target_mutation(tmp_path: Path) -> None:
    state, events = targets(tmp_path)
    safe = Phase68Error("safe")
