"""Phase 99 continuation boundary contract tests."""
...
def call(
    ...
    return (
        route_executed_result_transition_persistence_dispatch_continuation_boundary(
            result, workflow_value, state, events, phase92_function=dependency
        )
...
def test_invalid_predecessor_output_is_rejected_before_phase92(
...
def test_public_signature_is_canonical_and_keyword_only_dependency() -> None:
    ...
    assert tuple(signature.parameters) == (
        "result", "workflow", "state_path", "events_path", "phase92_function"
    )
    assert all(
    ...
    assert signature.parameters["phase92_function"].kind is inspect.Parameter.KEYWORD_ONLY
...
def test_safe_error_identity_and_unexpected_error_sanitization(tmp_path: Path) -> None:
    state, events = setup(tmp_path)
    safe = ExecutedResultTransitionPersistenceDispatchPhaseBridgeCycleReentryContinuationError("private")
    with pytest.raises(
        ExecutedResultTransitionPersistenceDispatchPhaseBridgeCycleReentryContinuationError
    ) as caught:
        call(
...
def test_safe_error_identity_is_preserved_after_successful_compensation(
    ...
    state, events = setup(tmp_path)
    before = state.read_bytes(), events.read_bytes()
    safe = ExecutedResultTransitionPersistenceDispatchPhaseBridgeCycleReentryContinuationError(
        "private"
    )
...
def test_safe_error_compensates_each_mutation_shape_and_calls_once(
    ...
    with pytest.raises(ExecutedResultTransitionPersistenceDispatchPhaseBridgeCycleReentryContinuationError) as caught:
        call(success(), workflow(), state, events, dependency)
    assert caught.value is safe
    assert calls == 1
    assert (state.read_bytes(), events.read_bytes()) == before
