...
def _six_step_workflow() -> WorkflowDefinition:
...
def _predecessor_event(step_id: str, position: int, *, provider: object = "other", request_id: object = _SENTINEL, output_text: object = "output") -> RuntimeStepEvent:
...
def _terminal_event(status: str, *, message: str = "safe failure") -> RuntimeStepEvent:
    if status == "succeeded":
        return RuntimeStepEvent(
            "step_succeeded", "w", "six", 6, "s", "running", "succeeded", "openai",
            None, "response-six", "request-six", "output-six", None,
        )
    return RuntimeStepEvent(
        "step_failed", "w", "six", 6, "s", "running", "failed", "openai",
        "api_error", None, "request-six", None, message,
    )


def _setup_six(tmp_path: Path, status: str, *, earlier_empty: tuple[int, ...] = (2,), message: str = "safe failure"):
    workflow = _six_step_workflow()
    state_model = WorkflowExecutionState(
        "w", status, "six", 6, "s",
    ...
    events = [
        _predecessor_event(step_id, position, output_text="" if position in earlier_empty else "output")
        for position, step_id in enumerate(_SIX_STEP_IDS[:5], 1)
    ]
    events[4] = _predecessor_event("five", 5, provider="openai", request_id=None, output_text="")
    events.append(_terminal_event(status, message=message))
    ...
    result = WorkflowExecutionPersistenceResult(state, events_path, len(state_bytes), len(terminal_bytes))
    return state, events_path, result, workflow, state_bytes, event_bytes


def _six_step_outcome(status: str) -> PersistedExecutionOutcome:
...
def test_phase155_succeeded_predecessor_empty_output_delegates_once(tmp_path: Path) -> None:
    state, events, result, workflow, before_state, before_events = _setup_six(tmp_path, "succeeded")
    expected = _six_step_outcome("succeeded")
...
def test_phase155_failed_predecessor_empty_output_message_empty_delegates_once(tmp_path: Path) -> None:
    state, events, result, workflow, before_state, before_events = _setup_six(tmp_path, "failed", message="")
    expected = _six_step_outcome("failed")
    calls: list[tuple[object, ...]] = []
...
def test_phase155_succeeded_multiple_earlier_empty_delegates_once(tmp_path: Path) -> None:
    state, events, result, workflow, before_state, before_events = _setup_six(tmp_path, "succeeded", earlier_empty=(2, 3))
    expected = _six_step_outcome("succeeded")
    calls: list[tuple[object, ...]] = []
...
def test_phase155_failed_multiple_earlier_empty_delegates_once(tmp_path: Path) -> None:
    state, events, result, workflow, before_state, before_events = _setup_six(tmp_path, "failed", earlier_empty=(2, 3))
    expected = _six_step_outcome("failed")
    calls: list[tuple[object, ...]] = []
...
def test_phase155_earlier_predecessor_none_output_remains_rejected(tmp_path: Path) -> None:
    state, events, result, workflow, *_ = _setup_six(tmp_path, "succeeded")
    lines = events.read_text(encoding="utf-8").splitlines(keepends=True)
    replacement = serialize_runtime_step_event_jsonl(_predecessor_event("two", 2, output_text=None))
