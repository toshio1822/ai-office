"""Focused Phase 119 persisted-running execution cycle handoff reentry continuation tests."""
...
def reject(values: dict[str, object], classification: str, **changes: object) -> None:
    ...
    with pytest.raises(PersistedRunningExecutionCycleHandoffReentryContinuationCompatibilityError) as caught:
        route_persisted_running_execution_cycle_handoff_reentry_continuation_boundary(**supplied)  # type: ignore[arg-type]
...
def test_default_identity_and_non_callable_rejection(tmp_path: Path) -> None:
    parameter = inspect.signature(route_persisted_running_execution_cycle_handoff_reentry_continuation_boundary).parameters["phase112_function"]
    assert parameter.default is route_persisted_running_execution_cycle_reentry_continuation_boundary
    values = setup(tmp_path)
    reject(values, "execution_inputs", phase112_function=object())
...
def test_exact_state_bytes_and_equal_path_identity_are_preserved(tmp_path: Path) -> None:
    ...
    actual = route_persisted_running_execution_cycle_handoff_reentry_continuation_boundary(**values, phase112_function=lambda *args: (seen.append(args), expected)[1])  # type: ignore[arg-type]
...
def test_valid_runtime_success_and_failure_are_returned_by_identity(tmp_path: Path, value: object) -> None:
    values = setup(tmp_path)
    actual = route_persisted_running_execution_cycle_handoff_reentry_continuation_boundary(**values, phase112_function=lambda *_: value)  # type: ignore[arg-type]
...
def test_runtime_subclasses_and_attribute_substitutes_are_rejected(tmp_path: Path, value: object) -> None:
    values = setup(tmp_path)
    with pytest.raises(PersistedRunningExecutionCycleHandoffReentryContinuationCompatibilityError) as caught:
        route_persisted_running_execution_cycle_handoff_reentry_continuation_boundary(**values, phase112_function=lambda *_: value)  # type: ignore[arg-type]
...
def test_runtime_identity_mismatch_is_rejected(tmp_path: Path, field: str) -> None:
    ...
    with pytest.raises(PersistedRunningExecutionCycleHandoffReentryContinuationCompatibilityError) as caught:
        route_persisted_running_execution_cycle_handoff_reentry_continuation_boundary(**values, phase112_function=lambda *_: bad)  # type: ignore[arg-type]
    assert caught.value.detail.classification == "runtime_contract"
...
def test_persisted_failure_stop_route_is_unchanged_and_zero_call(tmp_path: Path) -> None:
    ...
    actual = route_persisted_running_execution_cycle_handoff_reentry_continuation_boundary(result, None, values["workflow"], None, state, events, None, None, None, None, phase112_function=dependency)  # type: ignore[arg-type]
...
def test_stop_target_mismatch_and_execution_input_combination_are_rejected(tmp_path: Path) -> None:
    values = setup(tmp_path); state, events = failed_targets(tmp_path)
    result = PersistedExecutionOutcome("persisted_failure", "w", "two", 2, "e", "api_error")
    with pytest.raises(PersistedRunningExecutionCycleHandoffReentryContinuationCompatibilityError):
        route_persisted_running_execution_cycle_handoff_reentry_continuation_boundary(result, values["start"], values["workflow"], values["employee"], state, events, values["resolved_tools"], values["api_key"], values["approval"], values["transport"])  # type: ignore[arg-type]
