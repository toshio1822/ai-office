...
def real_context(
    ...
    prepared_request = ModelInvocationRequest(
        ...
        synthetic_continuation_facts(
            ...
            response_id=(
                "resp_123" if index == 11 else f"response-{wf.steps[index - 2].id}"
            ),
            request_id=(
...
def call_args(
    ...
    state: Path,
    events: Path,
    contexts: tuple[ApprovedWorkflowContinuationContext, ...],
) -> tuple[object, ...]:
...
def test_01_public_api_keeps_five_business_inputs_and_frozen_context() -> None:
...
def test_02_two_contexts_progress_to_terminal_completion(tmp_path: Path) -> None:
    ...
    assert load_workflow_execution_state(
        values["state_path"]
    ).completed_step_ids == tuple(step.id for step in wf.steps)
    assert len(values["events_path"].read_text().splitlines()) == 11


def test_03_tuple_length_is_exact_bound_and_exhaustion_preserves_identity(
    tmp_path: Path,
) -> None:
    tmp_path.mkdir(parents=True, exist_ok=True)
    values = real_setup(tmp_path, current=9, count=11)
...
def test_04_empty_tuple_returns_prepare_identity_without_dependency_or_target_inspection(
    tmp_path: Path,
) -> None:
    wf = workflow()
    state, events = targets(tmp_path)
    first = preparation(wf, 2)
    before = (state.read_bytes(), events.read_bytes())
    result = route_bounded_approved_workflow_continuation(first, wf, state, events, ())
    assert result is first and (state.read_bytes(), events.read_bytes()) == before


def test_05_terminal_inputs_ignore_operational_arguments_and_contexts(
    tmp_path: Path,
) -> None:
    wf = workflow()
    state, events = targets(tmp_path)
    ...
    for stop in (complete(wf), failure(wf)):
        result = route_bounded_approved_workflow_continuation(
            stop, wf, object(), object(), [object()]
        )
        assert result is stop
    assert calls == []


def test_06_terminal_validation_rejects_malformed_terminal_objects_before_operational_inputs() -> (
    None
):
    wf = workflow()
    bad = replace(complete(wf), current_step_id="wrong")
    missing = object.__new__(WorkflowProgressionDecision)
    missing_failure = object.__new__(PersistedExecutionOutcome)
    err(
        lambda: route_bounded_approved_workflow_continuation(
            bad, wf, object(), object(), object()
        ),
        "result_type",
    )
    err(
        lambda: route_bounded_approved_workflow_continuation(
            missing, wf, object(), object(), object()
        ),
        "result_type",
    )
    err(
        lambda: route_bounded_approved_workflow_continuation(
            missing_failure, wf, object(), object(), object()
        ),
        "result_type",
    )


def test_07_invalid_initial_result_workflow_and_targets_are_rejected(
    tmp_path: Path,
) -> None:
    wf = workflow()
    state, events = targets(tmp_path)
    ...
    err(
        lambda: route_bounded_approved_workflow_continuation(
            malformed_result, wf, state, events, ()
        ),
        "result_type",
    )
    err(
        lambda: route_bounded_approved_workflow_continuation(
            object(), wf, state, events, ()
        ),
        "result_type",
    )
    err(
        lambda: route_bounded_approved_workflow_continuation(
            first, malformed_workflow, state, events, ()
        ),
        "workflow_definition",
    )
    err(
        lambda: route_bounded_approved_workflow_continuation(
            first, object(), state, events, ()
        ),
        "workflow_definition",
    )
    err(
        lambda: route_bounded_approved_workflow_continuation(
            first, wf, object(), events, ()
        ),
        "state_target",
    )
    err(
        lambda: route_bounded_approved_workflow_continuation(
            first, wf, state, object(), ()
        ),
        "event_target",
    )
...
def test_08_context_container_must_be_exact_tuple(tmp_path: Path) -> None:
    ...
    for value in (
        [context("x")],
        TupleSubclass((context("x"),)),
        (item for item in (context("x"),)),
    ):
        err(
            lambda value=value: route_bounded_approved_workflow_continuation(
                first, wf, state, events, value
            ),
            "contexts_type",
        )
...
def test_09_context_members_must_be_exact_public_dataclass(tmp_path: Path) -> None:
    ...
    err(
        lambda: route_bounded_approved_workflow_continuation(
            first, wf, state, events, (object(),)
        ),
        "context_type",
    )
    err(
        lambda: route_bounded_approved_workflow_continuation(
            first, wf, state, events, (ContextSubclass(*astuple(context("x"))),)
        ),
        "context_type",
    )
    err(
        lambda: route_bounded_approved_workflow_continuation(
            first, wf, state, events, (malformed,)
        ),
        "context_type",
    )
...
def test_11_real_composition_first_context_failure_stops_without_retry(
    ...
    later = ApprovedWorkflowContinuationContext(
        ...
        later.employee,
        later.resolved_tools,
        later.api_key,
        later.execution_approval,
        synthetic_transport(calls),
    )
    result = route_bounded_approved_workflow_continuation(
        preparation(wf, 9),
...
def test_12_owner_error_is_not_retried_or_rolled_back(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    wf = workflow()
    state, events = targets(tmp_path)
    ...
    def fake(*args: object) -> object:
        calls.append(args)
        state.write_bytes(b"owner-owned")
        raise error
    ...
    with pytest.raises(RuntimeError) as caught:
        route_bounded_approved_workflow_continuation(
            *call_args(first, wf, state, events, (context("x"), context("later")))
        )
    assert caught.value is error
    assert len(calls) == 1
    assert (state.read_bytes(), events.read_bytes()) == (b"owner-owned", before[1])


def test_13_malformed_owner_result_stops_before_later_context(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    wf = workflow()
    state, events = targets(tmp_path)
    ...
    err(
        lambda: route_bounded_approved_workflow_continuation(
            *call_args(first, wf, state, events, (context("first"), context("later")))
        ),
        "phase190_contract",
    )
    assert len(calls) == 1
...
def test_14_malformed_owner_transition_is_rejected_before_next_context(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    wf = workflow()
    state, events = targets(tmp_path)
    ...
    err(
        lambda: route_bounded_approved_workflow_continuation(
            *call_args(first, wf, state, events, (context("first"), context("second")))
        ),
        "phase190_contract",
    )
    assert len(calls) == 1
...
def test_15_wrong_step_context_stops_before_provider_without_repair(
    tmp_path: Path,
) -> None:
    values = real_setup(tmp_path, current=9, count=11)
    wf = values["workflow"]
...
def test_terminal_result_stops_before_later_context_real_composition(
    tmp_path: Path,
) -> None:
    values = real_setup(tmp_path, current=9, count=10)
    wf = values["workflow"]
    assert isinstance(wf, WorkflowDefinition)
    calls: list[object] = []
    first = real_context(wf, 10)
    ...
    assert result.current_step_index == 10
    assert len(calls) == 1
    assert len(values["events_path"].read_text().splitlines()) == 10


def test_result_subclasses_and_discriminator_subclasses_are_rejected(
    tmp_path: Path,
) -> None:
    wf = workflow()
    state, events = targets(tmp_path)
    ...
    err(
        lambda: route_bounded_approved_workflow_continuation(
            DecisionSubclass(*astuple(first)), wf, state, events, ()
        ),
        "result_type",
    )
    err(
...
def test_real_second_context_failures_preserve_owner_semantics(
    tmp_path: Path,
) -> None:
    for mode in ("preparation", "execution"):
        (tmp_path / mode).mkdir(parents=True, exist_ok=True)
        values = real_setup(tmp_path / mode, current=9, count=11)
        wf = values["workflow"]
        assert isinstance(wf, WorkflowDefinition)
        calls: list[object] = []
        first = real_context(wf, 10)
        ...
        state = load_workflow_execution_state(values["state_path"])
        assert len(calls) == 1
        # A malformed second execution approval is rejected before running
        # persistence; no provider call is allowed.
