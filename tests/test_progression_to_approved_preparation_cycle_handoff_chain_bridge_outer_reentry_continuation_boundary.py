...
def _accumulated_stop_value137(
    tmp_path: Path,
    *,
    fail: bool,
    none_positions: tuple[int, ...] = (5, 6),
...
def test_prepare_accumulated_openai_none_request_id_narrowness137(tmp_path: Path) -> None:
    ...
    immediate_only = _accumulated_value137(tmp_path, index=7, none_positions=(6,))
    ...
    complete = _accumulated_stop_value137(tmp_path / "complete", fail=False)
    assert_rejected(complete, "terminal_contract", lambda *_: pytest.fail("called"))
    failed = _accumulated_stop_value137(tmp_path / "failed", fail=True)
    assert_rejected(failed, "terminal_contract", lambda *_: pytest.fail("called"))
