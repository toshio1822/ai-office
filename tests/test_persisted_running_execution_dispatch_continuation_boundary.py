...
def reject(values: dict[str, object], classification: str, **changes: object) -> None:
    ...
    with pytest.raises(PersistedRunningExecutionDispatchContinuationCompatibilityError) as caught:
        route_persisted_running_execution_dispatch_continuation_boundary(**supplied)  # type: ignore[arg-type]
...
def test_default_identity_and_non_callable_rejection(tmp_path: Path) -> None:
    parameter = inspect.signature(route_persisted_running_execution_dispatch_continuation_boundary).parameters["phase91_function"]
    assert parameter.default is not inspect.Parameter.empty
    assert parameter.default.__name__ == "route_persisted_running_execution_dispatch_phase_bridge_cycle_reentry_continuation"
    values = setup(tmp_path)
    reject(values, "execution_inputs", phase91_function=object())
...
def test_succeeded_predecessor_empty_output_is_accepted_and_non_string_rejected(tmp_path: Path, output: object) -> None:
    ...
    if output == "":
        ...
        reject(values, "persistence_result_contract", phase91_function=dependency)
        assert calls == 0
...
def test_exact_state_bytes_and_equal_path_identity_are_preserved(tmp_path: Path) -> None:
    ...
    actual = route_persisted_running_execution_dispatch_continuation_boundary(**values, phase91_function=lambda *args: (seen.append(args), expected)[1])  # type: ignore[arg-type]
...
def test_valid_runtime_success_and_failure_are_returned_by_identity(tmp_path: Path, value: object) -> None:
    values = setup(tmp_path)
    actual = route_persisted_running_execution_dispatch_continuation_boundary(**values, phase91_function=lambda *_: value)  # type: ignore[arg-type]
...
def test_runtime_subclasses_and_attribute_substitutes_are_rejected(tmp_path: Path, value: object) -> None:
    values = setup(tmp_path)
    with pytest.raises(PersistedRunningExecutionDispatchContinuationCompatibilityError) as caught:
        route_persisted_running_execution_dispatch_continuation_boundary(**values, phase91_function=lambda *_: value)  # type: ignore[arg-type]
...
def test_runtime_identity_mismatch_is_rejected(tmp_path: Path, field: str) -> None:
    ...
    with pytest.raises(PersistedRunningExecutionDispatchContinuationCompatibilityError) as caught:
        route_persisted_running_execution_dispatch_continuation_boundary(**values, phase91_function=lambda *_: bad)  # type: ignore[arg-type]
    assert caught.value.detail.classification == "execution_result_contract"
...
def test_stop_target_mismatch_and_execution_input_combination_are_rejected(tmp_path: Path) -> None:
    values = setup(tmp_path); state, events = failed_targets(tmp_path)
    result = PersistedExecutionOutcome("persisted_failure", "w", "two", 2, "e", "api_error")
    with pytest.raises(PersistedRunningExecutionDispatchContinuationCompatibilityError):
        route_persisted_running_execution_dispatch_continuation_boundary(result, values["start"], values["workflow"], values["employee"], state, events, values["resolved_tools"], values["api_key"], values["approval"], values["transport"])  # type: ignore[arg-type]
...
def test_safe_phase91_error_identity_is_preserved_and_no_retry(tmp_path: Path) -> None:
    values = setup(tmp_path); calls = 0
    from ai_office.engine.persisted_running_execution_dispatch_phase_bridge_cycle_reentry_continuation import PersistedRunningExecutionDispatchPhaseBridgeCycleReentryContinuationError
    supplied = PersistedRunningExecutionDispatchPhaseBridgeCycleReentryContinuationError("safe")
    def dependency(*_: object) -> object:
        nonlocal calls; calls += 1; raise supplied
    with pytest.raises(PersistedRunningExecutionDispatchPhaseBridgeCycleReentryContinuationError) as caught:
...
def test_dependency_matrix_compensates_without_retry(tmp_path: Path, mutation: str | None, error_kind: str) -> None:
    ...
    def dependency(*_: object) -> object:
        ...
        if error_kind == "safe":
            from ai_office.engine.persisted_running_execution_dispatch_phase_bridge_cycle_reentry_continuation import PersistedRunningExecutionDispatchPhaseBridgeCycleReentryContinuationError
            raise PersistedRunningExecutionDispatchPhaseBridgeCycleReentryContinuationError("safe")
        if error_kind == "unexpected": raise RuntimeError("secret")
        return object()
    with pytest.raises(Exception): route_persisted_running_execution_dispatch_continuation_boundary(**values, phase91_function=dependency)  # type: ignore[arg-type]
