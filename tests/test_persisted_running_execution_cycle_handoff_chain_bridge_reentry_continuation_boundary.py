...
def setup(tmp_path: Path, *, index: int = 4) -> dict[str, object]:
    definition = workflow()
...
def runtime_success(index: int = 4) -> StepRuntimeExecutionSuccess:
...
def runtime_failure(index: int = 4) -> StepRuntimeExecutionFailure:
    current = workflow().steps[index - 1]
...
def test_continuation_index_one_rejects_and_two_three_delegate_once(
    ...
    expected = runtime_success(index)
    seen: list[tuple[object, ...]] = []
...
# --- Phase 140/141 compatibility regressions: non-final empty success output_text ---


def _events_with_overrides(
    values: dict[str, object], **overrides: dict[str, object]
) -> None:
    """Rewrite the three predecessor events; kwargs map step_id to field overrides."""
    events_path = values["events_path"]
    defaults: dict[str, dict[str, object]] = {
        "one": {"provider": "other"},
        "two": {"provider": "other"},
        "three": {"provider": "openai"},
    }
    for step_id, fields in overrides.items():
        defaults[step_id].update(fields)
    events_path.write_text(
        "".join(
            serialize_runtime_step_event_jsonl(
                _event("step_succeeded", step_id, index, **defaults[step_id])  # type: ignore[arg-type]
            )
            for step_id, index in (("one", 1), ("two", 2), ("three", 3))
        ),
        encoding="utf-8",
    )


def test_immediate_predecessor_empty_output_text_delegates_once_in_canonical_order(
    tmp_path: Path,
) -> None:
    values = setup(tmp_path)
    _events_with_overrides(values, three={"output_text": ""})
    state, events = values["state_path"], values["events_path"]
    before = state.read_bytes(), events.read_bytes()  # type: ignore[union-attr]
    seen: list[tuple[object, ...]] = []
    expected = runtime_success()
...
def test_non_empty_predecessor_outputs_remain_accepted_with_byte_unchanged_targets(
    tmp_path: Path,
) -> None:
    values = setup(tmp_path)
    _events_with_overrides(
...
def test_predecessor_output_text_non_string_is_rejected_before_phase126(
    tmp_path: Path, output_text: object
) -> None:
    values = setup(tmp_path)
    _events_with_overrides(values, three={"output_text": output_text})
    reject_unchanged(values, "persistence_result_contract")


@pytest.mark.parametrize("response_id", ["", 4, ["x"]])
def test_predecessor_response_id_empty_and_non_string_is_rejected_before_phase126(
    tmp_path: Path, response_id: object
) -> None:
    values = setup(tmp_path)
    _events_with_overrides(values, three={"response_id": response_id})
    reject_unchanged(values, "persistence_result_contract")


@pytest.mark.parametrize("request_id", ["", 4, ["x"]])
def test_predecessor_request_id_empty_and_non_string_is_rejected_before_phase126(
    tmp_path: Path, request_id: object
) -> None:
    values = setup(tmp_path)
    _events_with_overrides(values, three={"request_id": request_id})
    reject_unchanged(values, "persistence_result_contract")
...
def test_immediate_predecessor_provider_must_remain_exact_openai_even_with_empty_output(
    ...
    values = setup(tmp_path)
    _events_with_overrides(values, three={"output_text": "", "provider": "other"})
    reject_unchanged(values, "persistence_result_contract")


def test_workflow_complete_empty_terminal_output_remains_rejected_zero_call(
    tmp_path: Path,
) -> None:
    ...
    replacement = serialize_runtime_step_event_jsonl(
        _event("step_succeeded", "four", 4, "other", output_text="")
    )
    events_path.write_text("".join(lines[:3]) + replacement, encoding="utf-8")  # type: ignore[union-attr]
    reject(values, "terminal_contract")


def test_failed_stop_history_empty_predecessor_output_remains_rejected_zero_call(
    tmp_path: Path,
) -> None:
    values, _ = stop_values(tmp_path, "failure")
    events_path = values["events_path"]
    lines = events_path.read_text(encoding="utf-8").splitlines(keepends=True)  # type: ignore[union-attr]
    replacement = serialize_runtime_step_event_jsonl(
        _event("step_succeeded", "two", 2, "other", output_text="")
    )
    events_path.write_text(lines[0] + replacement + lines[2] + lines[3], encoding="utf-8")  # type: ignore[union-attr]
    reject(values, "terminal_contract")
