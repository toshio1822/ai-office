...
def call(
    ...
    return (
        route_executed_result_transition_persistence_routing_phase_bridge_cycle_continuation(
            result, workflow_value, state, events, phase71_function=dependency
        )
...
def test_safe_error_identity_and_unexpected_error_sanitization(tmp_path: Path) -> None:
    state, events = setup(tmp_path)
    safe = ExecutedResultTransitionPersistenceRoutingPhaseBridgeContinuationError("private")
    with pytest.raises(
        ExecutedResultTransitionPersistenceRoutingPhaseBridgeContinuationError
    ) as caught:
        call(
...
def test_safe_error_identity_is_preserved_after_successful_compensation(
    ...
    state, events = setup(tmp_path)
    before = state.read_bytes(), events.read_bytes()
    safe = ExecutedResultTransitionPersistenceRoutingPhaseBridgeContinuationError(
        "private"
    )
...
def test_invalid_predecessor_output_is_rejected_before_phase71(
