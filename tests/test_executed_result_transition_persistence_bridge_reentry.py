...
def chain_setup(
    ...
    *,
    ...
    bad_output: object = _CHAIN_SENTINEL,
    bad_position: int = 2,
    ...
    for position, step_id in enumerate(_CHAIN_STEP_IDS[:5], 1):
        ...
        if position in empty_earlier or (position == 5 and immediate_empty):
            output = ""
        if bad_output is not _CHAIN_SENTINEL and position == bad_position:
            output = bad_output
...
def test_earlier_empty_output_delegates_once_to_phase43(tmp_path: Path) -> None:
    ...
    def fake(*args: object) -> object:
        ...
        expected = chain_phase43_fake(*args)  # type: ignore[arg-type]
        return expected

    returned = route_executed_result_transition_persistence_bridge_reentry(
        result, supplied_workflow, state, events, transition_routing_function=fake
    )
    assert returned is expected and len(calls) == 1
    assert (state.read_bytes(), events.read_bytes()) != (before_state, before_events)


def test_immediate_empty_output_delegates_once_to_phase43(tmp_path: Path) -> None:
    state, events, before_state, before_events = chain_setup(
...
def test_bad_predecessor_output_text_rejected_before_phase43(
    ...
    with pytest.raises(
        ...
        route_executed_result_transition_persistence_bridge_reentry(
            ...
            state,
            events,
            transition_routing_function=unexpected,
        )
