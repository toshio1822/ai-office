"""Focused tests for the Phase 114 cycle continuation boundary."""
...
def test_public_signature_and_default_identity() -> None:
    ...
    assert parameters[4].name == "phase107_function"
    assert parameters[4].default is route_persisted_transition_outcome_classification_cycle_continuation_boundary


def test_implementation_does_not_reference_phase_private_internals() -> None:
    source = _TestPath(
        "src/ai_office/engine/persisted_transition_outcome_classification_cycle_reentry_continuation_boundary.py"
    ).read_text()
    assert "route_persisted_transition_outcome_classification_cycle_continuation_boundary" in source
    assert "phase100" not in source.lower()
    assert "persisted_outcome_classification_dispatch_continuation_boundary" not in source
    assert "._top_level" not in source
...
def test_valid_routes_call_phase100_once_and_return_exact_object(tmp_path: Path, status: str) -> None:
    ...
    def fake(*args: object) -> object:
        ...
        return expected
    returned = route_persisted_transition_outcome_classification_cycle_reentry_continuation_boundary(
        result, workflow, state, events, phase107_function=fake
    )
    assert returned is expected
    assert calls == [(result, workflow, state, events)]
    assert (state.read_bytes(), events.read_bytes()) == (before_state, before_events)
...
def test_exact_result_model_and_path_identity_rejection(tmp_path: Path, bad: object) -> None:
    ...
    with pytest.raises(PersistedTransitionOutcomeClassificationCycleReentryContinuationCompatibilityError) as caught:
        route_persisted_transition_outcome_classification_cycle_reentry_continuation_boundary(bad, workflow, state, events)
    assert caught.value.detail.classification == "result_type"
    with pytest.raises(PersistedTransitionOutcomeClassificationCycleReentryContinuationCompatibilityError):
        route_persisted_transition_outcome_classification_cycle_reentry_continuation_boundary(
...
def test_stop_routes_are_unchanged_and_zero_call(tmp_path: Path, value: object) -> None:
    ...
    def fake(*_: object) -> object:
        ...
        calls += 1
        return object()
    assert route_persisted_transition_outcome_classification_cycle_reentry_continuation_boundary(
        value, workflow, state, events, phase107_function=fake
    ) is value
    assert calls == 0
    assert (state.read_bytes(), events.read_bytes()) == (before_state, before_events)
...
def test_invalid_stop_and_direct_success_are_rejected_without_call(tmp_path: Path) -> None:
    ...
    for value in (
        ...
        with pytest.raises(PersistedTransitionOutcomeClassificationCycleReentryContinuationCompatibilityError):
            route_persisted_transition_outcome_classification_cycle_reentry_continuation_boundary(value, workflow, state, events, phase107_function=fake)
    assert calls == 0
...
def test_dependency_paths_compensate_and_do_not_retry(tmp_path: Path, mutation: str, kind: str) -> None:
    ...
    calls = 0
    safe = PersistedTransitionOutcomeClassificationCycleContinuationError("dependency_error")
    def fake(*_: object) -> object:
        nonlocal calls
        calls += 1
    ...
    with pytest.raises(Exception) as caught:
        route_persisted_transition_outcome_classification_cycle_reentry_continuation_boundary(result, workflow, state, events, phase107_function=fake)
    assert calls == 1
    assert (state.read_bytes(), events.read_bytes()) == (before_state, before_events)
    if kind == "safe":
...
def test_outcome_subclass_and_substitute_rejected(tmp_path: Path) -> None:
    ...
    for value in (Sub("persisted_success", "w", "one", 1, "a", None), SimpleNamespace(outcome="persisted_success")):
        with pytest.raises(PersistedTransitionOutcomeClassificationCycleReentryContinuationCompatibilityError) as caught:
            route_persisted_transition_outcome_classification_cycle_reentry_continuation_boundary(result, workflow, state, events, phase107_function=lambda *_: value)
        assert caught.value.detail.classification == "outcome_contract"
...
def test_unrelated_event_bytes_are_not_accepted(tmp_path: Path) -> None:
    ...
    with pytest.raises(PersistedTransitionOutcomeClassificationCycleReentryContinuationCompatibilityError):
        route_persisted_transition_outcome_classification_cycle_reentry_continuation_boundary(result, workflow, state, events)

# Strict Phase 114 matrices below intentionally mirror the Phase 106/100 focused
...
def _assert_rejected(value, workflow, state, events, expected, dependency=lambda *_: pytest.fail("dependency called")):
    with pytest.raises(PersistedTransitionOutcomeClassificationCycleReentryContinuationCompatibilityError) as caught:
        route_persisted_transition_outcome_classification_cycle_reentry_continuation_boundary(
            value, workflow, state, events, phase107_function=dependency
        )
...
def test_failed_result_requires_exact_persisted_failure_all_fields(tmp_path: Path) -> None:
    ...
    assert route_persisted_transition_outcome_classification_cycle_reentry_continuation_boundary(
        result, workflow, state, events, phase107_function=lambda *_: expected
    ) is expected
    for malformed in (
...
def test_unchanged_dependency_failures_preserve_bytes_and_error_contract(
    ...
    with pytest.raises(Exception) as caught:
        route_persisted_transition_outcome_classification_cycle_reentry_continuation_boundary(
            result, workflow, state, events, phase107_function=dependency
        )
    assert caught.value is safe if kind == "safe" else isinstance(caught.value, PersistedTransitionOutcomeClassificationCycleReentryContinuationCompatibilityError)
    if kind != "safe":
