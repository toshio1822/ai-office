"""Focused Phase 105 persisted-running execution cycle continuation tests."""
...
from ai_office.engine.persisted_running_execution_dispatch_continuation_boundary import (
...
def reject(values: dict[str, object], classification: str, **changes: object) -> None:
    ...
    with pytest.raises(PersistedRunningExecutionCycleContinuationCompatibilityError) as caught:
        route_persisted_running_execution_cycle_continuation_boundary(**supplied)  # type: ignore[arg-type]
...
def test_implementation_uses_only_public_phase98_dependency() -> None:
    source = Path("src/ai_office/engine/persisted_running_execution_cycle_continuation_boundary.py").read_text()
    assert "route_persisted_running_execution_dispatch_continuation_boundary" in source
...
def test_public_signature_and_canonical_ten_argument_identity(tmp_path: Path) -> None:
    params = list(inspect.signature(route_persisted_running_execution_cycle_continuation_boundary).parameters.values())
    ...
    actual = route_persisted_running_execution_cycle_continuation_boundary(**values, phase98_function=dependency)  # type: ignore[arg-type]
...
def test_default_identity_and_non_callable_rejection(tmp_path: Path) -> None:
    parameter = inspect.signature(route_persisted_running_execution_cycle_continuation_boundary).parameters["phase98_function"]
    assert parameter.default is route_persisted_running_execution_dispatch_continuation_boundary
    values = setup(tmp_path)
    reject(values, "execution_inputs", phase98_function=object())
...
def test_succeeded_predecessor_empty_output_is_accepted_and_non_string_rejected(tmp_path: Path, output: object) -> None:
    ...
    if output == "":
        ...
        reject(values, "persistence_result_contract", phase98_function=dependency)
        assert calls == 0


def continuation(tmp_path: Path) -> dict[str, object]:
    """Running at step three with succeeded steps one and two (two predecessors)."""
    wf = WorkflowDefinition.model_validate({"id": "w", "name": "W", "description": "D", "steps": [{"id": "one", "name": "One", "employee": "e", "instructions": "a"}, {"id": "two", "name": "Two", "employee": "e", "instructions": "b"}, {"id": "three", "name": "Three", "employee": "e", "instructions": "c"}]})
    ...
    tools = (ToolDefinition("tool", "Tool", ()),)
    return {"result": RunningStatePersistenceResult(len(state_path.read_bytes())), "start": PreparedStepExecutionStart(request, state), "workflow": wf, "employee": employee(), "state_path": state_path, "events_path": events_path, "resolved_tools": tools, "api_key": OpenAIApiKey(value=SecretStr("synthetic")), "approval": approve_model_invocation_execution(request, tools, provider="openai", approved_by="test", approval_id="id"), "transport": lambda _: None}
...
def test_exact_state_bytes_and_equal_path_identity_are_preserved(tmp_path: Path) -> None:
    ...
    actual = route_persisted_running_execution_cycle_continuation_boundary(**values, phase98_function=lambda *args: (seen.append(args), expected)[1])  # type: ignore[arg-type]
...
def test_valid_runtime_success_and_failure_are_returned_by_identity(tmp_path: Path, value: object) -> None:
    values = setup(tmp_path)
    actual = route_persisted_running_execution_cycle_continuation_boundary(**values, phase98_function=lambda *_: value)  # type: ignore[arg-type]
...
def test_runtime_subclasses_and_attribute_substitutes_are_rejected(tmp_path: Path, value: object) -> None:
    values = setup(tmp_path)
    with pytest.raises(PersistedRunningExecutionCycleContinuationCompatibilityError) as caught:
        route_persisted_running_execution_cycle_continuation_boundary(**values, phase98_function=lambda *_: value)  # type: ignore[arg-type]
...
def test_runtime_identity_mismatch_is_rejected(tmp_path: Path, field: str) -> None:
    ...
    with pytest.raises(PersistedRunningExecutionCycleContinuationCompatibilityError) as caught:
        route_persisted_running_execution_cycle_continuation_boundary(**values, phase98_function=lambda *_: bad)  # type: ignore[arg-type]
    assert caught.value.detail.classification == "runtime_contract"
...
def test_persisted_failure_stop_route_is_unchanged_and_zero_call(tmp_path: Path) -> None:
    ...
    actual = route_persisted_running_execution_cycle_continuation_boundary(result, None, values["workflow"], None, state, events, None, None, None, None, phase98_function=dependency)  # type: ignore[arg-type]
...
def test_stop_target_mismatch_and_execution_input_combination_are_rejected(tmp_path: Path) -> None:
    values = setup(tmp_path); state, events = failed_targets(tmp_path)
    result = PersistedExecutionOutcome("persisted_failure", "w", "two", 2, "e", "api_error")
    with pytest.raises(PersistedRunningExecutionCycleContinuationCompatibilityError):
        route_persisted_running_execution_cycle_continuation_boundary(result, values["start"], values["workflow"], values["employee"], state, events, values["resolved_tools"], values["api_key"], values["approval"], values["transport"])  # type: ignore[arg-type]
...
def test_safe_phase98_error_identity_is_preserved_and_no_retry(tmp_path: Path) -> None:
    values = setup(tmp_path); calls = 0
    supplied = PersistedRunningExecutionDispatchContinuationError("safe")
    def dependency(*_: object) -> object:
        nonlocal calls; calls += 1; raise supplied
    with pytest.raises(PersistedRunningExecutionDispatchContinuationError) as caught:
        route_persisted_running_execution_cycle_continuation_boundary(**values, phase98_function=dependency)  # type: ignore[arg-type]
...
def test_dependency_matrix_compensates_without_retry(tmp_path: Path, mutation: str | None, error_kind: str) -> None:
    ...
    def dependency(*_: object) -> object:
        ...
        if error_kind == "safe":
            raise PersistedRunningExecutionDispatchContinuationError("safe")
        if error_kind == "unexpected": raise RuntimeError("secret")
        return object()
    with pytest.raises(Exception): route_persisted_running_execution_cycle_continuation_boundary(**values, phase98_function=dependency)  # type: ignore[arg-type]
