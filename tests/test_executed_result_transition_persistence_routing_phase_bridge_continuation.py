"""Phase 71 continuation boundary contract tests."""
...
def call(
    ...
    return (
        route_executed_result_transition_persistence_routing_phase_bridge_continuation(
            result, workflow_value, state, events, phase64_function=dependency
        )
...
def test_safe_error_identity_and_unexpected_error_sanitization(tmp_path: Path) -> None:
    state, events = setup(tmp_path)
    safe = ExecutedResultTransitionPersistenceRoutingPhaseBridgeError("private")
    with pytest.raises(
        ExecutedResultTransitionPersistenceRoutingPhaseBridgeError
    ) as caught:
        call(
