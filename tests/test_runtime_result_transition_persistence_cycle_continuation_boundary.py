"""Phase 106 runtime-result transition persistence continuation tests."""
...
def call(
    ...
    return (
        route_runtime_result_transition_persistence_cycle_continuation_boundary(
            result, workflow_value, state, events, phase99_function=dependency
        )
...
def test_public_signature_is_canonical_and_keyword_only_dependency() -> None:
    ...
    assert tuple(signature.parameters) == (
        "result", "workflow", "state_path", "events_path", "phase99_function"
    )
    assert all(
    ...
    dependency = signature.parameters["phase99_function"]
    assert dependency.kind is inspect.Parameter.KEYWORD_ONLY
    from ai_office.engine.executed_result_transition_persistence_dispatch_continuation_boundary import (
        route_executed_result_transition_persistence_dispatch_continuation_boundary,
    )
    assert dependency.default is route_executed_result_transition_persistence_dispatch_continuation_boundary


def test_production_module_references_only_the_public_phase99_dependency() -> None:
    import ai_office.engine.runtime_result_transition_persistence_cycle_continuation_boundary as module

    source = inspect.getsource(module)
    assert "executed_result_transition_persistence_dispatch_continuation_boundary" in source
    assert "phase_bridge" not in source
    assert "executed_result_transition_persistence_dispatch_continuation_boundary._" not in source
...
def test_malformed_stop_and_unsupported_progression_are_zero_call(
    ...
    with pytest.raises(RuntimeResultTransitionPersistenceCycleContinuationCompatibilityError) as caught:
        call(result, workflow(), state, events, lambda *_: pytest.fail("called"))
...
def test_safe_error_identity_and_unexpected_error_sanitization(tmp_path: Path) -> None:
    state, events = setup(tmp_path)
    safe = ExecutedResultTransitionPersistenceDispatchContinuationError("private")
    with pytest.raises(
        ExecutedResultTransitionPersistenceDispatchContinuationError
    ) as caught:
        call(
...
def test_safe_error_identity_is_preserved_after_successful_compensation(
    ...
    state, events = setup(tmp_path)
    before = state.read_bytes(), events.read_bytes()
    safe = ExecutedResultTransitionPersistenceDispatchContinuationError(
        "private"
    )
...
def test_safe_error_compensates_each_mutation_shape_and_calls_once(
    ...
    with pytest.raises(ExecutedResultTransitionPersistenceDispatchContinuationError) as caught:
        call(success(), workflow(), state, events, dependency)
    assert caught.value is safe
    assert calls == 1
    assert (state.read_bytes(), events.read_bytes()) == before
