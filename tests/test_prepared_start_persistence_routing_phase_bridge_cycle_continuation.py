...
def test_stop_routes_return_same_object_without_phase69(
    tmp_path: Path, result, status: str, index: int
) -> None:
    state, events = targets(tmp_path, status, index)
    before = state.read_bytes(), events.read_bytes()
    returned = route_prepared_start_persistence_routing_phase_bridge_cycle_continuation(
        result,
        workflow(),
        None,
        state,
        events,
        phase69_function=lambda *_: pytest.fail("must not call"),
    )
...
def test_invalid_persistence_effects_restore_both_targets(
    ...
    with pytest.raises(
        ...
        route_prepared_start_persistence_routing_phase_bridge_cycle_continuation(
            start(), workflow(), employee(), state, events, phase69_function=phase69
        )
    assert (
...
def test_malformed_return_type_and_result_subclass_are_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        ...
        route_prepared_start_persistence_routing_phase_bridge_cycle_continuation(
            ...
            state,
            events,
            phase69_function=lambda *_: ResultSubclass(1),
        )
    assert error.value.detail.classification == "persistence_contract"
...
def test_safe_error_identity_and_unexpected_error_are_safe(tmp_path: Path) -> None:
    state, events = targets(tmp_path)
    before = state.read_bytes(), events.read_bytes()
    safe = PreparedStartPersistenceRoutingPhaseBridgeContinuationError("private")

    def safe_dependency(*args: object):
        state.write_bytes(b"private")
        raise safe

    with pytest.raises(PreparedStartPersistenceRoutingPhaseBridgeContinuationError) as caught:
        route_prepared_start_persistence_routing_phase_bridge_cycle_continuation(
            start(),
            workflow(),
            employee(),
            state,
            events,
            phase69_function=safe_dependency,
        )
    ...
    with pytest.raises(
        ...
        route_prepared_start_persistence_routing_phase_bridge_cycle_continuation(
            ...
            state,
            events,
            phase69_function=lambda *_: (_ for _ in ()).throw(RuntimeError("private")),
        )
    assert (
