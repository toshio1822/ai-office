"""Focused Phase 86 boundary tests."""
...
def six_step_definition() -> WorkflowDefinition:
...
def test_phase155_succeeded_multiple_earlier_empty_delegates_once(tmp_path: Path) -> None:
...
def test_phase155_failed_multiple_earlier_empty_delegates_once(tmp_path: Path) -> None:
...
def test_phase155_earlier_predecessor_none_output_remains_rejected(tmp_path: Path) -> None:
...
def test_phase155_immediate_predecessor_non_string_output_remains_rejected(tmp_path: Path) -> None:
    state, events, result, workflow, *_ = setup_six(tmp_path, "succeeded")
    lines = events.read_text(encoding="utf-8").splitlines(keepends=True)
...
def error() -> PersistedOutcomeClassificationRoutingPhaseBridgeCycleContinuationError:
...
def test_valid_persistence_result_delegates_once_and_returns_exact_phase79_object(tmp_path: Path, status: str) -> None:
...
def test_phase79_receives_each_exact_object_identity(tmp_path: Path) -> None:
    state, events, result, workflow, _, _ = setup(tmp_path)
    received: list[object] = []
    expected = PersistedExecutionOutcome("persisted_success", "w", "one", 1, "a", None)

    def phase79(received_result: object, received_workflow: object, received_state: object, received_events: object) -> object:
...
def test_persistence_result_each_field_requires_exact_type_and_value(
    ...
    with pytest.raises(PersistedOutcomeClassificationRoutingPhaseBridgeCycleReentryContinuationCompatibilityError) as caught:
        route_persisted_outcome_classification_routing_phase_bridge_cycle_reentry_continuation(bad, workflow, state, events, phase79_function=lambda *_: object())
...
def test_outcome_subclass_and_substitute_are_rejected(tmp_path: Path, bad: object) -> None:
    ...
    with pytest.raises(PersistedOutcomeClassificationRoutingPhaseBridgeCycleReentryContinuationCompatibilityError) as caught:
        route_persisted_outcome_classification_routing_phase_bridge_cycle_reentry_continuation(result, workflow, state, events, phase79_function=lambda *_: bad)
...
def test_unsupported_or_malformed_completion_stop_route_is_rejected(tmp_path: Path, field: str) -> None:
    ...
    with pytest.raises(PersistedOutcomeClassificationRoutingPhaseBridgeCycleReentryContinuationCompatibilityError) as caught:
        route_persisted_outcome_classification_routing_phase_bridge_cycle_reentry_continuation(replace(completion(), **{field: values[field]}), workflow, state, events)
...
def test_malformed_persisted_failure_stop_route_is_rejected(tmp_path: Path, field: str) -> None:
    ...
    with pytest.raises(PersistedOutcomeClassificationRoutingPhaseBridgeCycleReentryContinuationCompatibilityError) as caught:
        route_persisted_outcome_classification_routing_phase_bridge_cycle_reentry_continuation(replace(failure(), **{field: values[field]}), workflow, state, events)
...
def test_missing_and_non_regular_targets_are_rejected(tmp_path: Path, kind: str) -> None:
    ...
    if kind == "missing":
        ...
        events.mkdir()
    with pytest.raises(PersistedOutcomeClassificationRoutingPhaseBridgeCycleReentryContinuationCompatibilityError) as caught:
        route_persisted_outcome_classification_routing_phase_bridge_cycle_reentry_continuation(result, workflow, state, events)
...
def test_valid_stop_routes_return_same_object_without_phase79(tmp_path: Path, value: object) -> None:
    ...
    assert route_persisted_outcome_classification_routing_phase_bridge_cycle_reentry_continuation(value, workflow, state, events, phase79_function=phase79) is value
...
def test_direct_persisted_success_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(PersistedOutcomeClassificationRoutingPhaseBridgeCycleReentryContinuationCompatibilityError) as caught:
        route_persisted_outcome_classification_routing_phase_bridge_cycle_reentry_continuation(supplied, workflow, state, events)
    assert caught.value.detail.classification == "failure_contract"
...
def test_exact_input_models_and_path_identity_are_required(tmp_path: Path) -> None:
    ...
    for supplied, supplied_workflow, supplied_state, supplied_events in cases:
        with pytest.raises(PersistedOutcomeClassificationRoutingPhaseBridgeCycleReentryContinuationCompatibilityError):
            route_persisted_outcome_classification_routing_phase_bridge_cycle_reentry_continuation(supplied, supplied_workflow, supplied_state, supplied_events, phase79_function=lambda *_: object())
...
def test_persistence_byte_counts_are_exact(tmp_path: Path, field: str) -> None:
    ...
    with pytest.raises(PersistedOutcomeClassificationRoutingPhaseBridgeCycleReentryContinuationCompatibilityError) as caught:
        route_persisted_outcome_classification_routing_phase_bridge_cycle_reentry_continuation(bad, workflow, state, events)
    assert caught.value.detail.classification == "persistence_contract"
...
def test_safe_error_identity_and_unexpected_error_sanitization(tmp_path: Path, kind: str) -> None:
    ...
    expected = PersistedOutcomeClassificationRoutingPhaseBridgeCycleContinuationError if kind == "safe" else PersistedOutcomeClassificationRoutingPhaseBridgeCycleReentryContinuationCompatibilityError
...
def test_malformed_return_is_compensated(tmp_path: Path) -> None:
    state, events, result, workflow, before_state, before_events = setup(tmp_path)
    with pytest.raises(PersistedOutcomeClassificationRoutingPhaseBridgeCycleReentryContinuationCompatibilityError):
        route_persisted_outcome_classification_routing_phase_bridge_cycle_reentry_continuation(result, workflow, state, events, phase79_function=lambda *_: object())
    assert (state.read_bytes(), events.read_bytes()) == (before_state, before_events)
...
def test_malformed_returns_call_phase79_once_and_never_retry(tmp_path: Path, bad: object) -> None:
    state, events, result, workflow, before_state, before_events = setup(tmp_path)
    calls = 0
