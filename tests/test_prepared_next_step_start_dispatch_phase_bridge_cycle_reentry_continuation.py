...
def invoke(result: object, person: object | None, state: Path, events: Path, function=None):
    kwargs = {} if function is None else {"phase82_function": function}
    return route_prepared_next_step_start_dispatch_phase_bridge_cycle_reentry_continuation(result, workflow(), person, state, events, **kwargs)


def test_public_signature_and_phase82_signature_are_canonical() -> None:
    parameters = tuple(inspect.signature(route_prepared_next_step_start_dispatch_phase_bridge_cycle_reentry_continuation).parameters)
    assert parameters[:5] == ("result", "workflow", "employee", "state_path", "events_path")
    assert tuple(inspect.signature(route_prepared_next_step_start_routing_phase_bridge_cycle_reentry_continuation).parameters)[:5] == parameters[:5]
...
def test_prepared_route_delegates_exact_five_arguments_once_and_returns_identity(tmp_path: Path) -> None:
    ...
    def phase82(*args: object) -> object:
        received.extend(args)
        return started()

    result = route_prepared_next_step_start_dispatch_phase_bridge_cycle_reentry_continuation(*supplied, phase82_function=phase82)
...
def test_stop_routes_are_zero_call_and_preserve_identity(tmp_path: Path, result: object, status: str, index: int) -> None:
    ...
    def phase82(*_: object) -> object:
        nonlocal calls
        calls += 1
        raise AssertionError

    assert invoke(result, None, state, events, phase82) is result and calls == 0
...
def test_stop_routes_reject_employee(tmp_path: Path, result: object) -> None:
    ...
    with pytest.raises(PreparedNextStepStartDispatchPhaseBridgeCycleReentryContinuationCompatibilityError) as caught:
...
def test_direct_success_and_terminal_predecessor_mismatch_are_rejected(tmp_path: Path) -> None:
    state, events = targets(tmp_path)
    with pytest.raises(PreparedNextStepStartDispatchPhaseBridgeCycleReentryContinuationCompatibilityError):
...
def test_safe_error_identity_and_compensation(tmp_path: Path, mutation: str) -> None:
    ...
    safe = PreparedNextStepStartRoutingPhaseBridgeCycleReentryContinuationError("safe")
    ...
    with pytest.raises(PreparedNextStepStartRoutingPhaseBridgeCycleReentryContinuationError) as caught:
        invoke(prepared(), employee(), state, events, phase82)
...
def test_missing_target_and_target_conflict_are_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(PreparedNextStepStartDispatchPhaseBridgeCycleReentryContinuationCompatibilityError) as caught:
        route_prepared_next_step_start_dispatch_phase_bridge_cycle_reentry_continuation(prepared(), workflow(), employee(), state, state)
