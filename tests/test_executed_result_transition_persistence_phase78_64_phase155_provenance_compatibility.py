"""Real Phase 78 -> 71 -> 64 segment with Phase-155 provenance and synthetic Phase 57 seam."""
...
def run_chain(
    ...
    calls = {"phase78": 0, "phase71": 0, "phase64": 0, "seam": 0}
    ...
    def phase64(result: object, workflow: object, state: object, events: object) -> object:
        calls["phase64"] += 1
        handoffs.append(("phase64", (result, workflow, state, events)))
        return route_executed_result_transition_persistence_routing_phase_bridge_reentry(
            result, workflow, state, events, phase57_function=seam  # type: ignore[arg-type]
        )

    def phase71(result: object, workflow: object, state: object, events: object) -> object:
        calls["phase71"] += 1
        handoffs.append(("phase71", (result, workflow, state, events)))
        return route_executed_result_transition_persistence_routing_phase_bridge_continuation(
            result, workflow, state, events, phase64_function=phase64  # type: ignore[arg-type]
        )

    def phase78(result: object, workflow: object, state: object, events: object) -> object:
        calls["phase78"] += 1
        handoffs.append(("phase78", (result, workflow, state, events)))
        return route_executed_result_transition_persistence_routing_phase_bridge_cycle_continuation(
            result, workflow, state, events, phase71_function=phase71  # type: ignore[arg-type]
        )
...
def assert_chain_ok(
    ...
    assert calls == {"phase78": 1, "phase71": 1, "phase64": 1, "seam": 1}
    assert out is seam_values[0]
    expected = tuple(
        values[key] for key in ("result", "workflow", "state_path", "events_path")
    )
    assert [name for name, _ in handoffs] == ["phase78", "phase71", "phase64", "seam"]
...
def test_earlier_predecessor_none_output_rejected_before_phase71_64_seam(
    tmp_path: Path,
) -> None:
    values = setup(tmp_path)
    ...
    calls = {"phase71": 0, "phase64": 0, "seam": 0}

    def fail(*_: object) -> object:
        calls["phase71"] += 1
        pytest.fail("Phase 71 must not be called")

    with pytest.raises(ExecutedResultTransitionPersistenceRoutingPhaseBridgeCycleContinuationCompatibilityError) as caught:
        route_executed_result_transition_persistence_routing_phase_bridge_cycle_continuation(
            ...
            phase71_function=fail,  # type: ignore[arg-type]
        )
    assert caught.value.detail.classification == "runtime_contract"
    assert calls["phase71"] == 0 and calls["phase64"] == 0 and calls["seam"] == 0
    assert (values["state_path"].read_bytes(), events.read_bytes()) == before  # type: ignore[union-attr]


def test_immediate_predecessor_none_output_rejected_before_phase71_64_seam(
    tmp_path: Path,
) -> None:
    values = setup(tmp_path)
    events = values["events_path"]
