"""Focused tests for the Phase 115 classified persisted-outcome progression reentry boundary."""
...
def classification(error: pytest.ExceptionInfo[ClassifiedPersistedOutcomeProgressionCycleReentryContinuationCompatibilityError]) -> str:
    return error.value.detail.classification


def test_public_signature_and_default_identity() -> None:
    signature = inspect.signature(route_classified_persisted_outcome_progression_cycle_reentry_continuation_boundary)
    parameters = list(signature.parameters.values())
    assert [parameter.name for parameter in parameters] == ["result", "workflow", "state_path", "events_path", "phase108_function"]
    ...
    assert parameters[4].default is route_classified_persisted_outcome_progression_cycle_continuation_boundary
...
def test_persisted_success_delegates_once_in_exact_four_argument_order_and_returns_identity(tmp_path: Path, single: bool) -> None:
    ...
    returned = route_classified_persisted_outcome_progression_cycle_reentry_continuation_boundary(result, definition, state, events, phase108_function=dependency)
...
def test_stop_routes_return_exact_identity_without_dependency_call(tmp_path: Path, route: str) -> None:
    ...
    returned = route_classified_persisted_outcome_progression_cycle_reentry_continuation_boundary(result, definition, state, events, phase108_function=forbidden)
...
def test_exact_result_models_reject_subclasses_and_substitutes_without_call(tmp_path: Path, result_factory) -> None:
    ...
    def forbidden(*args: object) -> object:
        ...
        calls += 1
        return args[0]
    with pytest.raises(ClassifiedPersistedOutcomeProgressionCycleReentryContinuationCompatibilityError) as caught:
        route_classified_persisted_outcome_progression_cycle_reentry_continuation_boundary(result_factory(), definition, state, events, phase108_function=forbidden)
...
def test_exact_path_models_existing_files_and_distinct_targets_are_required(tmp_path: Path, target: str, expected: str) -> None:
    ...
    with pytest.raises(ClassifiedPersistedOutcomeProgressionCycleReentryContinuationCompatibilityError) as caught:
        route_classified_persisted_outcome_progression_cycle_reentry_continuation_boundary(result, definition, supplied_state, supplied_events)
...
def test_terminal_history_missing_duplicate_reordered_unrelated_and_malformed_are_rejected(tmp_path: Path, mode: str) -> None:
    ...
    with pytest.raises(ClassifiedPersistedOutcomeProgressionCycleReentryContinuationCompatibilityError) as caught:
        route_classified_persisted_outcome_progression_cycle_reentry_continuation_boundary(result, definition, state, events)
...
def test_workflow_and_step_models_and_builtin_field_types_are_exact(tmp_path: Path, model: str) -> None:
    ...
    with pytest.raises(ClassifiedPersistedOutcomeProgressionCycleReentryContinuationCompatibilityError) as caught:
        route_classified_persisted_outcome_progression_cycle_reentry_continuation_boundary(result, supplied, state, events)
...
def test_success_contract_revalidates_fields_and_exact_types(tmp_path: Path, field: str, value: object, expected: str) -> None:
    result, definition, state, events = setup(tmp_path)
    with pytest.raises(ClassifiedPersistedOutcomeProgressionCycleReentryContinuationCompatibilityError) as caught:
        route_classified_persisted_outcome_progression_cycle_reentry_continuation_boundary(replace(result, **{field: value}), definition, state, events)
    assert classification(caught) == expected
...
def test_dependency_decision_accepts_exact_prepare_next_step_and_workflow_complete_fields(tmp_path: Path, final: bool) -> None:
    ...
    assert route_classified_persisted_outcome_progression_cycle_reentry_continuation_boundary(result, definition, state, events, phase108_function=lambda *args: decision) is decision
...
def test_prepare_next_step_dependency_decision_revalidates_all_fields(tmp_path: Path, field: str, value: object) -> None:
    result, definition, state, events = setup(tmp_path)
    with pytest.raises(ClassifiedPersistedOutcomeProgressionCycleReentryContinuationCompatibilityError) as caught:
        route_classified_persisted_outcome_progression_cycle_reentry_continuation_boundary(result, definition, state, events, phase108_function=lambda *args: replace(expected_decision(), **{field: value}))
...
def test_safe_dependency_error_restores_mutations_and_is_not_retried(tmp_path: Path, mutation: str) -> None:
    ...
    def dependency(*args: object) -> object:
        ...
        raise ClassifiedPersistedOutcomeProgressionCycleContinuationCompatibilityError("result_type")
    with pytest.raises(ClassifiedPersistedOutcomeProgressionCycleContinuationCompatibilityError):
        route_classified_persisted_outcome_progression_cycle_reentry_continuation_boundary(result, definition, state, events, phase108_function=dependency)
    assert calls == 1
...
def test_malformed_dependency_return_without_mutation_is_progression_contract(tmp_path: Path) -> None:
    result, definition, state, events = setup(tmp_path)
    with pytest.raises(ClassifiedPersistedOutcomeProgressionCycleReentryContinuationCompatibilityError) as caught:
        route_classified_persisted_outcome_progression_cycle_reentry_continuation_boundary(result, definition, state, events, phase108_function=lambda *args: object())
    assert classification(caught) == "progression_contract"
