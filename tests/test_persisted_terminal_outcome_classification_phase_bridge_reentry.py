...
@pytest.mark.parametrize("status", ["succeeded", "failed"])
def test_empty_predecessor_output_is_accepted_and_delegates_once(
    tmp_path: Path, status: str
) -> None:
    values = setup_six(
        tmp_path,
...
@pytest.mark.parametrize("status", ["succeeded", "failed"])
def test_multiple_earlier_empty_outputs_are_accepted_and_delegate_once(
    tmp_path: Path, status: str
) -> None:
    values = setup_six(
        tmp_path,
...
def test_none_predecessor_output_is_rejected_before_phase51(tmp_path: Path) -> None:
...
def test_non_string_predecessor_output_is_rejected_before_phase51(
    tmp_path: Path,
) -> None:
    values = setup_six(tmp_path, "succeeded")
    events_path = values["events_path"]
