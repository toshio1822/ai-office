"""Focused tests for the Phase 121 cycle handoff reentry continuation boundary."""
...
def _setup(tmp_path: Path, status: str = "succeeded"):
    ...
    state_model = WorkflowExecutionState(
        "w", status, "two", 2, "b", ("one", "two") if status == "succeeded" else ("one",),
        None if status == "succeeded" else "api_error",
    )
    prior = RuntimeStepEvent(
        "step_succeeded", "w", "one", 1, "a", "running", "succeeded", "openai",
        None, "response-one", "request-one", "out-one", None,
    )
    event = RuntimeStepEvent(
        "step_succeeded" if status == "succeeded" else "step_failed", "w", "two", 2,
        "b", "running", status, "openai", None if status == "succeeded" else "api_error",
        "response-two" if status == "succeeded" else None, "request-two",
        "out-two" if status == "succeeded" else None, None if status == "succeeded" else "safe",
    )
    state_bytes = serialize_workflow_execution_state_json(state_model).encode()
    event_bytes = (serialize_runtime_step_event_jsonl(prior) +
    ...
    result = WorkflowExecutionPersistenceResult(
        state, events, len(state_bytes), len(serialize_runtime_step_event_jsonl(event).encode())
    )
...
def _failure() -> PersistedExecutionOutcome:
    return PersistedExecutionOutcome("persisted_failure", "w", "two", 2, "b", "api_error")
    ...
    *,
    ...
    events = [
        RuntimeStepEvent("step_succeeded", "w", step_id, position, step_id[0], "running", "succeeded",
                         "other", None, f"response-{step_id}", f"request-{step_id}",
                         "" if position in earlier_empty else "output", None)
        for position, step_id in enumerate(_SIX_STEP_IDS[: step_index - 1], 1)
    ]
    if step_index > 1:
        immediate = step_index - 1
        events[immediate - 1] = RuntimeStepEvent(
            "step_succeeded", "w", _SIX_STEP_IDS[immediate - 1], immediate,
            _SIX_STEP_IDS[immediate - 1][0], "running", "succeeded",
            "openai", None, f"response-{_SIX_STEP_IDS[immediate - 1]}", None, "", None)
    if status == "succeeded":
...
def test_phase155_six_step_history_fallback_accepts_and_delegates_once(
    ...
    returned = route_persisted_transition_outcome_classification_cycle_handoff_reentry_continuation_boundary(
        result, workflow, state, events, phase114_function=seam
    )
...
def test_phase155_fallback_rejects_none_predecessor_output_before_phase114(tmp_path: Path) -> None:
    ...
    def seam(*_: object) -> object:
        ...
        calls += 1
        return object()
    with pytest.raises(PersistedTransitionOutcomeClassificationCycleHandoffReentryContinuationCompatibilityError) as caught:
        route_persisted_transition_outcome_classification_cycle_handoff_reentry_continuation_boundary(
            result, workflow, state, events, phase114_function=seam
        )
...
@pytest.mark.parametrize("status", ["succeeded", "failed"])
def test_phase155_fallback_multiple_earlier_empty_accepts_and_delegates_once(
    tmp_path: Path, status: str
) -> None:
    state, events, result, workflow, before_state, before_events = _setup_phase155(
        tmp_path, status, earlier_empty=(2, 3)
    )
    expected = PersistedExecutionOutcome(
        "persisted_success" if status == "succeeded" else "persisted_failure",
        "w", "six", 6, "s", None if status == "succeeded" else "api_error",
...
def test_public_signature_and_default_identity() -> None:
    signature = inspect.signature(route_persisted_transition_outcome_classification_cycle_handoff_reentry_continuation_boundary)
...
def test_implementation_does_not_reference_phase_private_internals() -> None:
    source = _TestPath(
        "src/ai_office/engine/persisted_transition_outcome_classification_cycle_handoff_reentry_continuation_boundary.py"
    ).read_text()
    assert "route_persisted_transition_outcome_classification_cycle_reentry_continuation_boundary" in source
    assert "phase107" not in source.lower()
    assert "persisted_transition_outcome_classification_cycle_continuation_boundary" not in source
    assert "._validate_" not in source
    assert "._top" not in source
    assert "._persistence" not in source
...
def test_exact_result_model_and_path_identity_rejection(tmp_path: Path, bad: object) -> None:
    ...
    with pytest.raises(PersistedTransitionOutcomeClassificationCycleHandoffReentryContinuationCompatibilityError) as caught:
        route_persisted_transition_outcome_classification_cycle_handoff_reentry_continuation_boundary(bad, workflow, state, events)
    assert caught.value.detail.classification == "result_type"
    with pytest.raises(PersistedTransitionOutcomeClassificationCycleHandoffReentryContinuationCompatibilityError):
        route_persisted_transition_outcome_classification_cycle_handoff_reentry_continuation_boundary(
...
def test_stop_routes_are_unchanged_and_zero_call(tmp_path: Path, value: object) -> None:
    ...
    def fake(*_: object) -> object:
        ...
        calls += 1
        return object()
    assert route_persisted_transition_outcome_classification_cycle_handoff_reentry_continuation_boundary(
        value, workflow, state, events, phase114_function=fake
    ) is value
    assert calls == 0
    assert (state.read_bytes(), events.read_bytes()) == (before_state, before_events)
...
def test_invalid_stop_and_direct_success_are_rejected_without_call(tmp_path: Path) -> None:
    ...
    def fake(*_: object) -> object:
        ...
        return _failure()
    for value in (
        replace(_completion(), current_step_index=3),
        replace(_failure(), failure_category="wrong"),
        PersistedExecutionOutcome("persisted_success", "w", "two", 2, "b", None),
        WorkflowProgressionDecision("next_step", "w", "two", 2, "b", "two", 2, "b", "x"),
    ):
        with pytest.raises(PersistedTransitionOutcomeClassificationCycleHandoffReentryContinuationCompatibilityError):
            route_persisted_transition_outcome_classification_cycle_handoff_reentry_continuation_boundary(value, workflow, state, events, phase114_function=fake)
    assert calls == 0
...
def test_dependency_paths_compensate_and_do_not_retry(tmp_path: Path, mutation: str, kind: str) -> None:
    ...
    calls = 0
    safe = PersistedTransitionOutcomeClassificationCycleReentryContinuationError("dependency_error")
    def fake(*_: object) -> object:
        nonlocal calls
        calls += 1
    ...
    with pytest.raises(Exception) as caught:
        route_persisted_transition_outcome_classification_cycle_handoff_reentry_continuation_boundary(result, workflow, state, events, phase114_function=fake)
    assert calls == 1
    assert (state.read_bytes(), events.read_bytes()) == (before_state, before_events)
    if kind == "safe":
...
def test_outcome_subclass_and_substitute_rejected(tmp_path: Path) -> None:
    ...
    class Sub(PersistedExecutionOutcome):
        pass
    for value in (Sub("persisted_success", "w", "two", 2, "b", None), SimpleNamespace(outcome="persisted_success")):
        with pytest.raises(PersistedTransitionOutcomeClassificationCycleHandoffReentryContinuationCompatibilityError) as caught:
            route_persisted_transition_outcome_classification_cycle_handoff_reentry_continuation_boundary(result, workflow, state, events, phase114_function=lambda *_: value)
        assert caught.value.detail.classification == "outcome_contract"
...
def test_unrelated_event_bytes_are_not_accepted(tmp_path: Path) -> None:
    ...
    events.write_text("\n".join(lines[:-1] + [json.dumps(payload, separators=(",", ":"))]) + "\n")
    with pytest.raises(PersistedTransitionOutcomeClassificationCycleHandoffReentryContinuationCompatibilityError):
        route_persisted_transition_outcome_classification_cycle_handoff_reentry_continuation_boundary(result, workflow, state, events)

# Strict Phase 114 matrices below intentionally mirror the Phase 121/114 focused
...
def _assert_rejected(value, workflow, state, events, expected, dependency=lambda *_: pytest.fail("dependency called")):
    with pytest.raises(PersistedTransitionOutcomeClassificationCycleHandoffReentryContinuationCompatibilityError) as caught:
        route_persisted_transition_outcome_classification_cycle_handoff_reentry_continuation_boundary(
            value, workflow, state, events, phase114_function=dependency
        )
...
def test_terminal_history_strict_matrix_rejects_before_dependency(tmp_path: Path, mode: str) -> None:
    ...
    if mode == "missing":
        ...
        payload = json.loads(line.decode().splitlines()[-1])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("current_step_index", 3), ("current_step_index", True),
...
def test_every_dependency_outcome_field_type_and_value_is_matched(
    ...
    state, events, result, workflow, *_ = _setup(tmp_path)
    valid = PersistedExecutionOutcome("persisted_success", "w", "two", 2, "b", None)
...
def test_failed_result_requires_exact_persisted_failure_all_fields(tmp_path: Path) -> None:
    ...
    expected = PersistedExecutionOutcome("persisted_failure", "w", "two", 2, "b", "api_error")
    assert route_persisted_transition_outcome_classification_cycle_handoff_reentry_continuation_boundary(
        result, workflow, state, events, phase114_function=lambda *_: expected
    ) is expected
    for malformed in (
...
def test_unchanged_dependency_failures_preserve_bytes_and_error_contract(
    ...
    with pytest.raises(Exception) as caught:
        route_persisted_transition_outcome_classification_cycle_handoff_reentry_continuation_boundary(
            result, workflow, state, events, phase114_function=dependency
        )
    assert caught.value is safe if kind == "safe" else isinstance(caught.value, PersistedTransitionOutcomeClassificationCycleHandoffReentryContinuationCompatibilityError)
    if kind != "safe":
...
def test_persistence_result_subclass_and_attribute_substitute_reject_before_phase114(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("current_step_index", True), ("current_step_index", 3),
