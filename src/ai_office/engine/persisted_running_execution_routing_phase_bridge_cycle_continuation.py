"""Continue exact Phase 76 persistence results through the Phase 70 boundary."""
...
from ai_office.engine.persisted_running_execution_routing_phase_bridge_continuation import (
...
class PersistedRunningExecutionRoutingPhaseBridgeCycleContinuationFailureDetail:
    classification: Classification


class PersistedRunningExecutionRoutingPhaseBridgeCycleContinuationError(ValueError):
    """Raised when Phase 70 cannot safely continue its supplied result."""


class PersistedRunningExecutionRoutingPhaseBridgeCycleContinuationCompatibilityError(
    PersistedRunningExecutionRoutingPhaseBridgeCycleContinuationError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__(
            "persisted-running execution routing phase bridge continuation inputs are incompatible"
        )
        self.detail = (
...
def route_persisted_running_execution_routing_phase_bridge_cycle_continuation(
    result: object,
    start: object | None,
    workflow: object,
    ...
    *,
    phase70_function: Phase70Function = route_persisted_running_execution_routing_phase_bridge_continuation,
) -> (
    StepRuntimeExecutionSuccess
    | StepRuntimeExecutionFailure
    | WorkflowProgressionDecision
    ...
    _top_level(result, workflow, state_path, events_path, phase70_function)
    ...
    except PersistedRunningExecutionRoutingPhaseBridgeContinuationError as error:
...
def _raise(classification: Classification) -> None:
    raise PersistedRunningExecutionRoutingPhaseBridgeCycleContinuationCompatibilityError(
        classification
    ) from None
