"""Dispatch one exact Phase 88 result through the Phase 82 boundary."""
...
from ai_office.engine.prepared_next_step_start_routing_phase_bridge_cycle_reentry_continuation import (
...
Phase82Function = Callable[..., PreparedStepExecutionStart | WorkflowProgressionDecision | PersistedExecutionOutcome]
_FAILURE_CATEGORIES = frozenset(get_args(ModelInvocationFailureCategory))
_PATH_TYPE = type(Path())


@dataclass(frozen=True)
class PreparedNextStepStartDispatchPhaseBridgeCycleReentryContinuationFailureDetail:
    classification: Classification


class PreparedNextStepStartDispatchPhaseBridgeCycleReentryContinuationError(ValueError):
    """Raised when Phase 89 cannot safely dispatch its supplied result."""


class PreparedNextStepStartDispatchPhaseBridgeCycleReentryContinuationCompatibilityError(
    PreparedNextStepStartDispatchPhaseBridgeCycleReentryContinuationError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__("prepared next-step start dispatch phase bridge cycle reentry continuation inputs are incompatible")
        self.detail = PreparedNextStepStartDispatchPhaseBridgeCycleReentryContinuationFailureDetail(classification)


def route_prepared_next_step_start_dispatch_phase_bridge_cycle_reentry_continuation(
    result: object,
    workflow: object,
    ...
    events_path: object,
    *,
    phase82_function: Phase82Function = route_prepared_next_step_start_routing_phase_bridge_cycle_reentry_continuation,
) -> PreparedStepExecutionStart | WorkflowProgressionDecision | PersistedExecutionOutcome:
    _validate_inputs(result, workflow, state_path, events_path, phase82_function)
    ...
    try:
        value = phase82_function(result, workflow, employee, state_path, events_path)
    except PreparedNextStepStartRoutingPhaseBridgeCycleReentryContinuationError as error:
    ...
    except PreparedNextStepStartDispatchPhaseBridgeCycleReentryContinuationCompatibilityError as error:
...
def _raise(classification: Classification) -> None:
    raise PreparedNextStepStartDispatchPhaseBridgeCycleReentryContinuationCompatibilityError(classification) from None
