...
Phase97Function = Callable[[object, object, object, object, object], RunningStatePersistenceResult | WorkflowProgressionDecision | PersistedExecutionOutcome]
_FAILURE_CATEGORIES = frozenset(get_args(ModelInvocationFailureCategory))
_PATH_TYPE = type(Path())


@dataclass(frozen=True)
class PreparedStartPersistenceCycleContinuationFailureDetail:
    classification: Classification


class PreparedStartPersistenceCycleContinuationError(ValueError):
    """Raised when Phase 104 cannot safely continue its supplied result."""


class PreparedStartPersistenceCycleContinuationCompatibilityError(
    PreparedStartPersistenceCycleContinuationError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__("prepared-start persistence cycle continuation inputs are incompatible")
        self.detail = PreparedStartPersistenceCycleContinuationFailureDetail(classification)


def route_prepared_start_persistence_cycle_continuation_boundary(
    result: object,
    workflow: object,
    ...
    events_path: object,
    *,
    phase97_function: Phase97Function = route_prepared_start_persistence_dispatch_continuation_boundary,
) -> RunningStatePersistenceResult | WorkflowProgressionDecision | PersistedExecutionOutcome:
    _inputs(result, workflow, state_path, events_path, phase97_function)
    ...
    try:
        value = phase97_function(result, workflow, employee, state_path, events_path)
    except PreparedStartPersistenceDispatchContinuationError as error:
    ...
    except PreparedStartPersistenceCycleContinuationCompatibilityError as error:
...
def _raise(classification: Classification) -> None:
    raise PreparedStartPersistenceCycleContinuationCompatibilityError(classification) from None
