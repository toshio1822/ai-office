"""Phase 114 persisted-transition outcome classification cycle continuation boundary."""
...
Phase107Function = Callable[..., PersistedExecutionOutcome]
_PATH_TYPE = type(Path())
_FAILURE_CATEGORIES = frozenset(get_args(ModelInvocationFailureCategory))


@dataclass(frozen=True)
class PersistedTransitionOutcomeClassificationCycleReentryContinuationFailureDetail:
    classification: Classification


class PersistedTransitionOutcomeClassificationCycleReentryContinuationError(ValueError):
    """Raised when Phase 114 cannot safely classify a persisted transition."""


class PersistedTransitionOutcomeClassificationCycleReentryContinuationCompatibilityError(PersistedTransitionOutcomeClassificationCycleReentryContinuationError):
    def __init__(self, classification: Classification) -> None:
        super().__init__("persisted transition outcome classification cycle continuation inputs are incompatible")
        self.detail = PersistedTransitionOutcomeClassificationCycleReentryContinuationFailureDetail(classification)


def route_persisted_transition_outcome_classification_cycle_reentry_continuation_boundary(
    result: object, workflow: object, state_path: object, events_path: object, *,
    phase107_function: Phase107Function = route_persisted_transition_outcome_classification_cycle_continuation_boundary,
) -> PersistedExecutionOutcome | WorkflowProgressionDecision:
    _top_level(result, workflow, state_path, events_path, phase107_function)
    ...
    try:
        value = phase107_function(result, workflow, state_path, events_path)
    except PersistedTransitionOutcomeClassificationCycleContinuationError as error:
    ...
    except PersistedTransitionOutcomeClassificationCycleReentryContinuationCompatibilityError:
        raise
    except Exception:
        _restore_if_changed(state_path, events_path, original)
        _raise("outcome_contract")
...
def _raise(classification: Classification) -> None:
    raise PersistedTransitionOutcomeClassificationCycleReentryContinuationCompatibilityError(classification) from None
