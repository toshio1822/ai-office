"""Continue exact Phase 67 results through the existing Phase 61 boundary."""
...
class PreparedNextStepStartRoutingPhaseBridgeContinuationFailureDetail:
    classification: Classification


class PreparedNextStepStartRoutingPhaseBridgeContinuationError(ValueError):
    """Raised when Phase 68 cannot safely continue its supplied result."""


class PreparedNextStepStartRoutingPhaseBridgeContinuationCompatibilityError(
    PreparedNextStepStartRoutingPhaseBridgeContinuationError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__(
            "prepared next-step start routing phase bridge continuation "
            "inputs are incompatible"
        )
        self.detail = PreparedNextStepStartRoutingPhaseBridgeContinuationFailureDetail(
            classification
        )


def route_prepared_next_step_start_routing_phase_bridge_continuation(
    result: object,
    workflow: object,
    ...
    events_path: object,
    *,
    phase61_function: Phase61Function = (
        route_prepared_next_step_start_routing_phase_bridge_reentry
    ),
) -> (
    PreparedStepExecutionStart | WorkflowProgressionDecision | PersistedExecutionOutcome
):
    _validate_inputs(result, workflow, state_path, events_path, phase61_function)
    ...
    try:
        started = phase61_function(result, workflow, employee, state_path, events_path)
    except PreparedNextStepStartRoutingPhaseBridgeError as error:
...
def _validate_prepared(
    ...
    if not (
        ...
        and _exact_str(employee.id, value.employee_id)
    ):
        _raise("prepared_step_contract")
...
def _raise(classification: Classification) -> None:
    raise PreparedNextStepStartRoutingPhaseBridgeContinuationCompatibilityError(
        classification
    ) from None
