...
from ai_office.engine.executed_result_transition_persistence_routing_phase_bridge_cycle_continuation import (
...
class ExecutedResultTransitionPersistenceRoutingPhaseBridgeCycleReentryContinuationFailureDetail:
    classification: Classification


class ExecutedResultTransitionPersistenceRoutingPhaseBridgeCycleReentryContinuationError(
    ValueError
):
    """Raised when Phase 78 cannot safely continue the supplied result."""


class ExecutedResultTransitionPersistenceRoutingPhaseBridgeCycleReentryContinuationCompatibilityError(
    ...
    def __init__(self, classification: Classification) -> None:
        ...
        self.detail = ExecutedResultTransitionPersistenceRoutingPhaseBridgeCycleReentryContinuationFailureDetail(
            classification
        )


def route_executed_result_transition_persistence_routing_phase_bridge_cycle_reentry_continuation(
    result: object,
    workflow: object,
    state_path: object,
    events_path: object,
    *,
    phase78_function: Phase78Function = route_executed_result_transition_persistence_routing_phase_bridge_cycle_continuation,
) -> (
    WorkflowExecutionPersistenceResult
    | WorkflowProgressionDecision
    | PersistedExecutionOutcome
):
    _top_level(result, workflow, state_path, events_path, phase78_function)
    ...
    try:
        value = phase78_function(result, workflow, state_path, events_path)
    except ExecutedResultTransitionPersistenceRoutingPhaseBridgeCycleContinuationError as error:
    ...
    except ExecutedResultTransitionPersistenceRoutingPhaseBridgeCycleReentryContinuationCompatibilityError as error:
...
def _raise(classification: Classification) -> None:
    raise ExecutedResultTransitionPersistenceRoutingPhaseBridgeCycleReentryContinuationCompatibilityError(
        classification
    ) from None
