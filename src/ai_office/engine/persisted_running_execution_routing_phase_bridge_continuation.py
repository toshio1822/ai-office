"""Continue exact Phase 69 persistence results through the Phase 63 boundary."""
...
class PersistedRunningExecutionRoutingPhaseBridgeContinuationFailureDetail:
    classification: Classification


class PersistedRunningExecutionRoutingPhaseBridgeContinuationError(ValueError):
    """Raised when Phase 70 cannot safely continue its supplied result."""


class PersistedRunningExecutionRoutingPhaseBridgeContinuationCompatibilityError(
...
def route_persisted_running_execution_routing_phase_bridge_continuation(
    result: object,
    start: object | None,
    workflow: object,
    ...
    *,
    phase63_function: Phase63Function = route_persisted_running_execution_routing_phase_bridge_reentry,
) -> (
    StepRuntimeExecutionSuccess
    | StepRuntimeExecutionFailure
    | WorkflowProgressionDecision
    ...
    _top_level(result, workflow, state_path, events_path, phase63_function)
    ...
    except PersistedRunningExecutionRoutingPhaseBridgeError as error:
...
def _raise(classification: Classification) -> None:
    raise PersistedRunningExecutionRoutingPhaseBridgeContinuationCompatibilityError(
        classification
    ) from None
