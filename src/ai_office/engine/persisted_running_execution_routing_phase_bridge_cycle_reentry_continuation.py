...
from ai_office.engine.persisted_running_execution_routing_phase_bridge_cycle_continuation import (
...
class PersistedRunningExecutionRoutingPhaseBridgeCycleReentryContinuationError(ValueError):
    """Raised when Phase 77 cannot safely continue its supplied result."""


class PersistedRunningExecutionRoutingPhaseBridgeCycleReentryContinuationCompatibilityError(
...
def route_persisted_running_execution_routing_phase_bridge_cycle_reentry_continuation(
    ...
    *,
    phase77_function: Phase77Function = route_persisted_running_execution_routing_phase_bridge_cycle_continuation,
) -> (
    StepRuntimeExecutionSuccess
    | StepRuntimeExecutionFailure
    | WorkflowProgressionDecision
    ...
    _top_level(result, workflow, state_path, events_path, phase77_function)
    ...
    except PersistedRunningExecutionRoutingPhaseBridgeCycleContinuationError as error:
...
def _raise(classification: Classification) -> None:
    raise PersistedRunningExecutionRoutingPhaseBridgeCycleReentryContinuationCompatibilityError(
        classification
    ) from None
