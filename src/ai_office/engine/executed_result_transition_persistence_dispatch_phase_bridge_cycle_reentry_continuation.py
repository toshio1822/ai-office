...
from ai_office.engine.executed_result_transition_persistence_routing_phase_bridge_cycle_reentry_continuation import (
...
class ExecutedResultTransitionPersistenceDispatchPhaseBridgeCycleReentryContinuationFailureDetail:
...
def route_executed_result_transition_persistence_dispatch_phase_bridge_cycle_reentry_continuation(
    result: object,
    workflow: object,
    state_path: object,
    events_path: object,
    *,
    phase85_function: Phase85Function = route_executed_result_transition_persistence_routing_phase_bridge_cycle_reentry_continuation,
) -> (
    WorkflowExecutionPersistenceResult
    | WorkflowProgressionDecision
    | PersistedExecutionOutcome
):
    _top_level(result, workflow, state_path, events_path, phase85_function)
    ...
    try:
        value = phase85_function(result, workflow, state_path, events_path)
    except ExecutedResultTransitionPersistenceRoutingPhaseBridgeCycleReentryContinuationError as error:
    ...
    except ExecutedResultTransitionPersistenceDispatchPhaseBridgeCycleReentryContinuationCompatibilityError as error:
...
def _raise(classification: Classification) -> None:
    raise ExecutedResultTransitionPersistenceDispatchPhaseBridgeCycleReentryContinuationCompatibilityError(
        classification
    ) from None
