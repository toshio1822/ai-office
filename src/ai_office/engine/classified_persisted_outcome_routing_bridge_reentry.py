"""Bridge exact Phase 51 outcomes to the Phase 45 routing boundary."""
...
_ERROR_MESSAGE = "classified persisted outcome routing bridge inputs are incompatible"
_FAILURE_CATEGORIES = frozenset(get_args(ModelInvocationFailureCategory))
Phase45Function = Callable[..., WorkflowProgressionDecision | PersistedExecutionOutcome]


@dataclass(frozen=True)
class ClassifiedPersistedOutcomeRoutingBridgeFailureDetail:
    classification: Classification


class ClassifiedPersistedOutcomeRoutingBridgeError(ValueError):
    """Raised when the Phase 52 bridge cannot safely proceed."""


class ClassifiedPersistedOutcomeRoutingBridgeCompatibilityError(
    ClassifiedPersistedOutcomeRoutingBridgeError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__(_ERROR_MESSAGE)
        self.detail = ClassifiedPersistedOutcomeRoutingBridgeFailureDetail(
            classification
        )


def route_classified_persisted_outcome_bridge_reentry(
    result: object,
    workflow: object,
    state_path: object,
    events_path: object,
    *,
    routing_function: Phase45Function = route_classified_persisted_outcome_reentry,
) -> WorkflowProgressionDecision | PersistedExecutionOutcome:
    """Route one exact Phase 51 result through Phase 45 once, or stop completion."""
    ...
    try:
        _require_unchanged(state_path, events_path, original)
        _validate_return(value, result, workflow, state)
    except ClassifiedPersistedOutcomeRoutingBridgeCompatibilityError:
        _restore_changed(state_path, events_path, original)
        raise
    return value
...
def _validate_terminal(
    ...
    try:
        ...
        if isinstance(error.__cause__, WorkflowExecutionLoadError):
            ...
            # terminal_contract: it is not a strict-contract rejection that the
            ...
            _raise("terminal_contract")
        state, _ = _load_phase155_terminal_history(
    ...
    if not (
        ...
        and state.current_step_id == result.current_step_id
        and state.current_step_index == result.current_step_index
        and state.current_employee_id == result.current_employee_id
    ):
        _raise("terminal_contract")
    if type(result) is WorkflowProgressionDecision:
        ...
        state.status
        != ("succeeded" if result.outcome == "persisted_success" else "failed")
...
def _validate_return(
    ...
    if state.current_step_index == len(workflow.steps):
        ...
        valid = (
            ...
            and value.next_step_index == state.current_step_index + 1
...
def _raise(classification: Classification) -> None:
    raise ClassifiedPersistedOutcomeRoutingBridgeCompatibilityError(
        classification
    ) from None
