...
Phase104Function = Callable[[object, object, object, object, object], RunningStatePersistenceResult | WorkflowProgressionDecision | PersistedExecutionOutcome]
_FAILURE_CATEGORIES = frozenset(get_args(ModelInvocationFailureCategory))
_PATH_TYPE = type(Path())


@dataclass(frozen=True)
class PreparedStartPersistenceCycleReentryContinuationFailureDetail:
    classification: Classification


class PreparedStartPersistenceCycleReentryContinuationError(ValueError):
    """Raised when Phase 104 cannot safely continue its supplied result."""


class PreparedStartPersistenceCycleReentryContinuationCompatibilityError(
    ...
    def __init__(self, classification: Classification) -> None:
        ...
        self.detail = PreparedStartPersistenceCycleReentryContinuationFailureDetail(classification)


def route_prepared_start_persistence_cycle_reentry_continuation_boundary(
    result: object,
    workflow: object,
    ...
    events_path: object,
    *,
    phase104_function: Phase104Function = route_prepared_start_persistence_cycle_continuation_boundary,
) -> RunningStatePersistenceResult | WorkflowProgressionDecision | PersistedExecutionOutcome:
    _inputs(result, workflow, state_path, events_path, phase104_function)
    ...
    try:
        value = phase104_function(result, workflow, employee, state_path, events_path)
    except PreparedStartPersistenceCycleContinuationError as error:
    ...
    except PreparedStartPersistenceCycleReentryContinuationCompatibilityError as error:
...
def _raise(classification: Classification) -> None:
    raise PreparedStartPersistenceCycleReentryContinuationCompatibilityError(classification) from None
