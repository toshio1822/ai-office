...
Phase96Function = Callable[..., PreparedStepExecutionStart | WorkflowProgressionDecision | PersistedExecutionOutcome]
_FAILURE_CATEGORIES = frozenset(get_args(ModelInvocationFailureCategory))
_PATH_TYPE = type(Path())


@dataclass(frozen=True)
class PreparedStepStartCycleContinuationFailureDetail:
    classification: Classification


class PreparedStepStartCycleContinuationError(ValueError):
    """Raised when Phase 103 cannot safely continue its supplied result."""


class PreparedStepStartCycleContinuationCompatibilityError(
    ...
    def __init__(self, classification: Classification) -> None:
        ...
        self.detail = PreparedStepStartCycleContinuationFailureDetail(classification)


def route_prepared_step_start_cycle_continuation_boundary(
    result: object,
    workflow: object,
    ...
    events_path: object,
    *,
    phase96_function: Phase96Function = route_prepared_next_step_start_dispatch_continuation_boundary,
) -> PreparedStepExecutionStart | WorkflowProgressionDecision | PersistedExecutionOutcome:
    _validate_inputs(result, workflow, state_path, events_path, phase96_function)
    ...
    try:
        value = phase96_function(result, workflow, employee, state_path, events_path)
    except PreparedNextStepStartDispatchContinuationError as error:
    ...
    except PreparedStepStartCycleContinuationCompatibilityError as error:
...
def _raise(classification: Classification) -> None:
    raise PreparedStepStartCycleContinuationCompatibilityError(classification) from None
