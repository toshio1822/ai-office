...
Phase110Function = Callable[..., PreparedStepExecutionStart | WorkflowProgressionDecision | PersistedExecutionOutcome]
_FAILURE_CATEGORIES = frozenset(get_args(ModelInvocationFailureCategory))
_PATH_TYPE = type(Path())


@dataclass(frozen=True)
class PreparedStepStartCycleHandoffReentryContinuationFailureDetail:
    classification: Classification


class PreparedStepStartCycleHandoffReentryContinuationError(ValueError):
    """Raised when Phase 117 cannot safely hand off one prepared-step result."""


class PreparedStepStartCycleHandoffReentryContinuationCompatibilityError(
    PreparedStepStartCycleHandoffReentryContinuationError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__("prepared-step start cycle handoff reentry continuation inputs are incompatible")
        self.detail = PreparedStepStartCycleHandoffReentryContinuationFailureDetail(classification)


def route_prepared_step_start_cycle_handoff_reentry_continuation_boundary(
    result: object,
    workflow: object,
    employee: object,
    state_path: object,
    events_path: object,
    *,
    phase110_function: Phase110Function = route_prepared_step_start_cycle_reentry_continuation_boundary,
) -> PreparedStepExecutionStart | WorkflowProgressionDecision | PersistedExecutionOutcome:
    _validate_inputs(result, workflow, state_path, events_path, phase110_function)
    ...
    try:
        value = phase110_function(result, workflow, employee, state_path, events_path)
    except (
        PreparedStepStartCycleHandoffReentryContinuationError,
        PreparedStepStartCycleReentryContinuationError,
    ) as error:
        _compensate_dependency_failure(state_path, events_path, original)
        raise error
    except Exception:
        _compensate_dependency_failure(state_path, events_path, original)
    ...
    except PreparedStepStartCycleHandoffReentryContinuationCompatibilityError as error:
...
def _compensate_dependency_failure(
    state: Path,
    events: Path,
...
def _raise(classification: Classification) -> None:
    raise PreparedStepStartCycleHandoffReentryContinuationCompatibilityError(classification) from None
