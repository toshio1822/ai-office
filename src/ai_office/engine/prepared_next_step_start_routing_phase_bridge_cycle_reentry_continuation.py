"""Continue one exact Phase 81 result through the Phase 75 boundary."""
...
Classification = Literal[
    ...
    "target_conflict", "terminal_contract", "start_contract", "dependency_error",
    "dependency_rollback",
]
Phase75Function = Callable[..., PreparedStepExecutionStart | WorkflowProgressionDecision | PersistedExecutionOutcome]
_FAILURE_CATEGORIES = frozenset(get_args(ModelInvocationFailureCategory))
_PATH_TYPE = type(Path())


@dataclass(frozen=True)
class PreparedNextStepStartRoutingPhaseBridgeCycleReentryContinuationFailureDetail:
    classification: Classification


class PreparedNextStepStartRoutingPhaseBridgeCycleReentryContinuationError(ValueError):
    """Raised when Phase 82 cannot safely continue its supplied result."""


class PreparedNextStepStartRoutingPhaseBridgeCycleReentryContinuationCompatibilityError(
    PreparedNextStepStartRoutingPhaseBridgeCycleReentryContinuationError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__("prepared next-step start routing phase bridge cycle reentry continuation inputs are incompatible")
        self.detail = PreparedNextStepStartRoutingPhaseBridgeCycleReentryContinuationFailureDetail(classification)


def route_prepared_next_step_start_routing_phase_bridge_cycle_reentry_continuation(
    result: object,
    workflow: object,
    ...
    events_path: object,
    *,
    phase75_function: Phase75Function = route_prepared_next_step_start_routing_phase_bridge_cycle_continuation,
) -> PreparedStepExecutionStart | WorkflowProgressionDecision | PersistedExecutionOutcome:
    _validate_inputs(result, workflow, state_path, events_path, phase75_function)
    ...
    if type(result) is PreparedWorkflowStep:
        ...
        if not (
            ...
            _raise("employee_contract")
        if not _exact_str(employee.id, result.employee_id):
            _raise("employee_contract")
    ...
    try:
        started = phase75_function(result, workflow, employee, state_path, events_path)
    except PreparedNextStepStartRoutingPhaseBridgeCycleContinuationError as error:
    ...
    except PreparedNextStepStartRoutingPhaseBridgeCycleReentryContinuationCompatibilityError as error:
...
def _validate_prepared(value: PreparedWorkflowStep, workflow: WorkflowDefinition, employee: EmployeeDefinition) -> None:
...
def _validate_completion(value: WorkflowProgressionDecision, workflow: WorkflowDefinition, employee: object | None) -> None:
...
def _validate_failure(value: PersistedExecutionOutcome, workflow: WorkflowDefinition, employee: object | None) -> None:
    if not (
        employee is None
...
def _validate_terminal(value: WorkflowProgressionDecision | PersistedExecutionOutcome, workflow: WorkflowDefinition, state: Path, events: Path, status: Literal["succeeded", "failed"]) -> None:
    try:
...
def _validate_predecessor(prepared: PreparedWorkflowStep, workflow: WorkflowDefinition, state: Path, events: Path) -> WorkflowExecutionState:
    try:
        persisted, _ = load_strict_terminal_history(workflow, state, events)
    except (OSError, TerminalHistoryContractError):
        _raise("terminal_contract")
...
def _validate_start(value: object, prepared: PreparedWorkflowStep, state: WorkflowExecutionState) -> None:
...
def _raise(classification: Classification) -> None:
    raise PreparedNextStepStartRoutingPhaseBridgeCycleReentryContinuationCompatibilityError(classification) from None
