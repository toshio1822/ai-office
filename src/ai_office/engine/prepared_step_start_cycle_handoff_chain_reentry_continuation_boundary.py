"""Phase 124 prepared-step start cycle handoff chain boundary."""
...
from ai_office.engine.prepared_step_start_cycle_handoff_reentry_continuation_boundary import (
    PreparedStepStartCycleHandoffReentryContinuationError as Phase117Error,
)
from ai_office.engine.prepared_step_start_cycle_handoff_reentry_continuation_boundary import (
...
class PreparedStepStartCycleHandoffChainReentryContinuationFailureDetail:
    classification: Classification


class PreparedStepStartCycleHandoffChainReentryContinuationError(ValueError):
    """Raised when the Phase 124 boundary cannot safely hand off one result."""


class PreparedStepStartCycleHandoffChainReentryContinuationCompatibilityError(
    PreparedStepStartCycleHandoffChainReentryContinuationError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__("prepared-step start cycle handoff chain inputs are incompatible")
        self.detail = PreparedStepStartCycleHandoffChainReentryContinuationFailureDetail(
            classification
        )


def route_prepared_step_start_cycle_handoff_chain_reentry_continuation_boundary(
    result: object,
    workflow: object,
    employee: object,
    state_path: object,
    events_path: object,
    *,
    phase117_function: Phase117Function = (
        route_prepared_step_start_cycle_handoff_reentry_continuation_boundary
    ),
) -> PreparedStepExecutionStart | WorkflowProgressionDecision | PersistedExecutionOutcome:
    """Route one exact Phase 123 result through the public Phase 117 boundary once."""
    _check_inputs(result, workflow, state_path, events_path, phase117_function)
    ...
    try:
        value = phase117_function(result, workflow, employee, state_path, events_path)
    except Phase117Error as error:
...
def _check_employee(value: object | None, prepared: PreparedWorkflowStep) -> None:
    ...
    if not _employee_fields(value) or value.id != prepared.employee_id:
        _fail("employee_contract")
...
def _check_prepared(
    ...
    if not (
        ...
        and value.employee_id == step.employee
...
def _check_start(
    ...
    if not (
        ...
        and type(running.workflow_id) is str
...
def _compensate_or_raise(
    ...
    try:
        unchanged = state.read_bytes() == original[0] and events.read_bytes() == original[1]
    except OSError:
        unchanged = False
    if not unchanged:
...
def _fail(classification: Classification) -> None:
    raise PreparedStepStartCycleHandoffChainReentryContinuationCompatibilityError(
        classification
    ) from None
