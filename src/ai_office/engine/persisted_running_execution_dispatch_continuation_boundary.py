"""Phase 98 read-only continuation boundary for persisted running executions."""
...
from ai_office.engine.persisted_running_execution_dispatch_phase_bridge_cycle_reentry_continuation import (
...
Classification = Literal[
    ...
    "event_target", "target_conflict", "terminal_contract", "execution_result_contract",
    "dependency_error", "dependency_rollback",
]
Phase91Function = Callable[..., StepRuntimeExecutionSuccess | StepRuntimeExecutionFailure | WorkflowProgressionDecision | PersistedExecutionOutcome]
_PATH_TYPE = type(Path())
_FAILURES = frozenset(get_args(ModelInvocationFailureCategory))


@dataclass(frozen=True)
class PersistedRunningExecutionDispatchContinuationFailureDetail:
    classification: Classification


class PersistedRunningExecutionDispatchContinuationError(ValueError):
    """Base error for the Phase 98 boundary."""


class PersistedRunningExecutionDispatchContinuationCompatibilityError(
    PersistedRunningExecutionDispatchContinuationError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__("persisted-running execution dispatch continuation boundary inputs are incompatible")
        self.detail = PersistedRunningExecutionDispatchContinuationFailureDetail(classification)


def route_persisted_running_execution_dispatch_continuation_boundary(
    result: object,
    start: object | None,
    workflow: object,
    ...
    *,
    phase91_function: Phase91Function = route_persisted_running_execution_dispatch_phase_bridge_cycle_reentry_continuation,
) -> StepRuntimeExecutionSuccess | StepRuntimeExecutionFailure | WorkflowProgressionDecision | PersistedExecutionOutcome:
    _top(result, workflow, state_path, events_path, phase91_function)
    ...
    try:
        value = phase91_function(result, start, workflow, employee, state_path, events_path, resolved_tools, api_key, approval, transport)
    except PersistedRunningExecutionDispatchPhaseBridgeCycleReentryContinuationError as error:
    ...
    try:
        _unchanged(state_path, events_path, original, "execution_result_contract")
    ...
    except PersistedRunningExecutionDispatchContinuationCompatibilityError as error:
...
def _top(result: object, workflow: object, state: object, events: object, function: object) -> None:
    if type(result) not in (RunningStatePersistenceResult, WorkflowProgressionDecision, PersistedExecutionOutcome): _raise("result_type")
    if type(workflow) is not WorkflowDefinition or not _workflow(workflow): _raise("workflow_definition")
    ...
    if not callable(function): _raise("execution_inputs")


def _none(*values: object | None) -> None:
    if any(value is not None for value in values): _raise("execution_inputs")
...
def _execution_inputs(result: RunningStatePersistenceResult, start: object | None, workflow: WorkflowDefinition, employee: object | None, tools: object | None, key: object | None, approval: object | None, transport: object | None) -> None:
    ...
    if type(running.current_step_index) is not int or not 1 <= running.current_step_index <= len(workflow.steps): _raise("start_contract")
...
def _predecessor(start: PreparedStepExecutionStart, workflow: WorkflowDefinition, state: Path, events: Path) -> None:
    ...
    except (OSError, WorkflowExecutionDataError, WorkflowExecutionLoadError): _raise("persistence_result_contract")
    running = start.running_state
    if loaded != running: _raise("persistence_result_contract")
...
def _raise(classification: Classification) -> None:
    raise PersistedRunningExecutionDispatchContinuationCompatibilityError(classification) from None
