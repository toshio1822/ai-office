"""Dispatch exact Phase 90 persistence results through the Phase 84 execution boundary."""
...
from ai_office.engine.persisted_running_execution_routing_phase_bridge_cycle_reentry_continuation import (
...
class PersistedRunningExecutionDispatchPhaseBridgeCycleReentryContinuationFailureDetail:
    classification: Classification


class PersistedRunningExecutionDispatchPhaseBridgeCycleReentryContinuationError(ValueError):
    """Raised when Phase 84 cannot safely continue its supplied result."""


class PersistedRunningExecutionDispatchPhaseBridgeCycleReentryContinuationCompatibilityError(
    PersistedRunningExecutionDispatchPhaseBridgeCycleReentryContinuationError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__(
            "persisted-running execution dispatch phase bridge cycle reentry continuation inputs are incompatible"
        )
        self.detail = (
...
def route_persisted_running_execution_dispatch_phase_bridge_cycle_reentry_continuation(
    result: object,
    start: object | None,
    workflow: object,
    ...
    *,
    phase84_function: Phase84Function = route_persisted_running_execution_routing_phase_bridge_cycle_reentry_continuation,
) -> (
    StepRuntimeExecutionSuccess
    | StepRuntimeExecutionFailure
    | WorkflowProgressionDecision
    ...
    _top_level(result, workflow, state_path, events_path, phase84_function)
    ...
    except PersistedRunningExecutionRoutingPhaseBridgeCycleReentryContinuationError as error:
...
def _validate_execution_inputs(
    ...
    if not (
        ...
        and _exact_str(employee.id, running.current_employee_id)
...
def _raise(classification: Classification) -> None:
    raise PersistedRunningExecutionDispatchPhaseBridgeCycleReentryContinuationCompatibilityError(
        classification
    ) from None
