...
Phase108Function = Callable[..., WorkflowProgressionDecision | PersistedExecutionOutcome]
_PATH_TYPE = type(Path())
_FAILURE_CATEGORIES = frozenset(get_args(ModelInvocationFailureCategory))


@dataclass(frozen=True)
class ClassifiedPersistedOutcomeProgressionCycleReentryContinuationFailureDetail:
    classification: Classification


class ClassifiedPersistedOutcomeProgressionCycleReentryContinuationError(ValueError):
    """Raised when Phase 115 cannot safely progress one classified persisted outcome."""


class ClassifiedPersistedOutcomeProgressionCycleReentryContinuationCompatibilityError(
    ...
    def __init__(self, classification: Classification) -> None:
        ...
        self.detail = ClassifiedPersistedOutcomeProgressionCycleReentryContinuationFailureDetail(
            classification
        )


def route_classified_persisted_outcome_progression_cycle_reentry_continuation_boundary(
    result: object,
    workflow: object,
    state_path: object,
    events_path: object,
    *,
    ...
    try:
        progressed = phase108_function(result, workflow, state_path, events_path)
    except ClassifiedPersistedOutcomeProgressionCycleContinuationError as error:
...
def _raise(classification: Classification) -> None:
    raise ClassifiedPersistedOutcomeProgressionCycleReentryContinuationCompatibilityError(
        classification
    )
...
def _compensate_dependency_error(
    ...
    events_path: Path,
    original: tuple[bytes, bytes],
    safe_error: ClassifiedPersistedOutcomeProgressionCycleContinuationError | None,
) -> None:
    if _changed(state_path, events_path, original):
        _restore_or_raise(state_path, events_path, original)
