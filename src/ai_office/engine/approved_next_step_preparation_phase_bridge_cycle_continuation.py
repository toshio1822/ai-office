"""Continue exactly one Phase 73 result through the Phase 67 boundary."""
...
Classification = Literal[
    "result_type", "workflow_definition", "decision_contract", "completion_contract",
    "failure_contract", "approval_contract", "employee_contract", "state_target",
    "event_target", "target_conflict", "terminal_contract", "preparation_contract",
    "dependency_error", "dependency_rollback",
]
Phase67Function = Callable[
...
class ApprovedNextStepPreparationPhaseBridgeCycleContinuationFailureDetail:
    classification: Classification


class ApprovedNextStepPreparationPhaseBridgeCycleContinuationError(ValueError):
    """Raised when Phase 74 cannot safely continue its supplied result."""


class ApprovedNextStepPreparationPhaseBridgeCycleContinuationCompatibilityError(
    ApprovedNextStepPreparationPhaseBridgeCycleContinuationError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__(
            "approved next-step preparation phase bridge cycle continuation "
            "inputs are incompatible"
        )
        self.detail = ApprovedNextStepPreparationPhaseBridgeCycleContinuationFailureDetail(
            classification
        )


def route_approved_next_step_preparation_phase_bridge_cycle_continuation(
    result: object,
    workflow: object,
    approval: object,
    employee: object,
    ...
    events_path: object,
    *,
    phase67_function: Phase67Function = (
        route_approved_next_step_preparation_phase_bridge_continuation
    ),
) -> PreparedWorkflowStep | WorkflowProgressionDecision | PersistedExecutionOutcome:
    _validate_inputs(result, workflow, approval, employee, state_path, events_path, phase67_function)
    ...
    except ApprovedNextStepPreparationPhaseBridgeCycleContinuationCompatibilityError as error:
...
def _validate_prepare(value: WorkflowProgressionDecision, workflow: WorkflowDefinition,
    ...
    next_step = (workflow.steps[value.next_step_index - 1] if type(value.next_step_index) is int
                 and 1 <= value.next_step_index <= len(workflow.steps) else None)
    if current is None or next_step is None or not (
        _exact_str(value.decision, "prepare_next_step") and _exact_str(value.workflow_id, workflow.id)
        and type(value.current_step_index) is int and 1 <= value.current_step_index < len(workflow.steps)
        and _exact_str(value.current_step_id, current.id) and _exact_str(value.current_employee_id, current.employee)
        and type(value.next_step_index) is int and value.next_step_index == value.current_step_index + 1
        and _exact_str(value.next_step_id, next_step.id) and _exact_str(value.next_employee_id, next_step.employee)
...
def _validate_completion(value: WorkflowProgressionDecision, workflow: WorkflowDefinition,
    ...
    if not (approval is None and employee is None and _exact_str(value.decision, "workflow_complete")
            and _exact_str(value.workflow_id, workflow.id) and type(value.current_step_index) is int
            and value.current_step_index == len(workflow.steps) and _exact_str(value.current_step_id, final.id)
            and _exact_str(value.current_employee_id, final.employee) and value.next_step_id is None
...
def _validate_failure(value: PersistedExecutionOutcome, workflow: WorkflowDefinition,
                      approval: object, employee: object) -> None:
    if not (approval is None and employee is None and _exact_str(value.outcome, "persisted_failure")
            and _exact_str(value.workflow_id, workflow.id) and type(value.current_step_index) is int
            and 1 <= value.current_step_index <= len(workflow.steps)
            and _exact_str(value.current_step_id, workflow.steps[value.current_step_index - 1].id)
            and _exact_str(value.current_employee_id, workflow.steps[value.current_step_index - 1].employee)
...
def _validate_prepared(value: object, decision: WorkflowProgressionDecision,
    ...
    if not (_exact_str(value.workflow_id, workflow.id) and _exact_str(value.step_id, next_step.id)
...
def _require_unchanged(state: Path, events: Path, original: tuple[bytes, bytes],
...
def _exact_tuple(value: object, expected: tuple[str, ...]) -> bool:
    return type(value) is tuple and all(type(item) is str for item in value) and value == expected


def _raise(classification: Classification) -> None:
    raise ApprovedNextStepPreparationPhaseBridgeCycleContinuationCompatibilityError(classification) from None
