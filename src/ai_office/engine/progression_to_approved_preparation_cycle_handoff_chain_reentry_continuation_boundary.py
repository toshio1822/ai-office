"""Phase 123 progression-to-approved-preparation handoff chain reentry continuation boundary."""
...
from ai_office.engine.progression_to_approved_preparation_cycle_handoff_reentry_continuation_boundary import (
    ProgressionToApprovedPreparationCycleHandoffReentryContinuationError as Phase116Error,
)
from ai_office.engine.progression_to_approved_preparation_cycle_handoff_reentry_continuation_boundary import (
...
Classification = Literal[
    ...
    "event_target", "target_conflict", "terminal_contract", "prepared_contract",
    "dependency_error", "dependency_rollback",
]
Phase116Function = Callable[
    ...,
...
class ProgressionToApprovedPreparationCycleHandoffChainReentryContinuationFailureDetail:
    classification: Classification


class ProgressionToApprovedPreparationCycleHandoffChainReentryContinuationError(ValueError):
    """Raised when Phase 116 cannot safely hand off one progression result."""


class ProgressionToApprovedPreparationCycleHandoffChainReentryContinuationCompatibilityError(
    ProgressionToApprovedPreparationCycleHandoffChainReentryContinuationError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__("progression to approved preparation cycle reentry continuation inputs are incompatible")
        self.detail = ProgressionToApprovedPreparationCycleHandoffChainReentryContinuationFailureDetail(classification)


def route_progression_to_approved_preparation_cycle_handoff_chain_reentry_continuation_boundary(
    result: object,
    workflow: object,
    approval: object,
    employee: object,
    ...
    events_path: object,
    *,
    phase116_function: Phase116Function = (
        route_progression_to_approved_preparation_cycle_handoff_reentry_continuation_boundary
    ),
) -> PreparedWorkflowStep | WorkflowProgressionDecision | PersistedExecutionOutcome:
    """Hand off one exact Phase 116 result through Phase 116 at most once."""
    _validate_inputs(result, workflow, state_path, events_path, phase116_function)
    ...
    if type(result) is PersistedExecutionOutcome or result.decision == "workflow_complete":
        return result
    ...
    _validate_prepared(value, result, workflow, approval, employee)
    return value
...
def _validate_prepare(
    ...
    if type(employee) is not EmployeeDefinition or not _employee_fields(employee):
    ...
    if not (
        ...
        and type(approval.current_step_id) is str
        and approval.current_step_id == value.current_step_id
        and type(approval.current_step_index) is int
        and approval.current_step_index == value.current_step_index
        and type(approval.next_step_id) is str
        and approval.next_step_id == value.next_step_id
        and type(approval.next_step_index) is int
        and approval.next_step_index == value.next_step_index
        and type(approval.next_employee_id) is str
        and approval.next_employee_id == value.next_employee_id
    ):
        _raise("approval_contract")
...
def _validate_prepared(
    ...
    if type(value) is not PreparedWorkflowStep:
        _raise("prepared_contract")
    assert type(decision.next_step_index) is int
    step = workflow.steps[decision.next_step_index - 1]
    if not (
        approval.approved is True
        and type(value.workflow_id) is str
        and value.workflow_id == workflow.id == approval.workflow_id
        and type(value.step_id) is str
        and value.step_id == step.id == approval.next_step_id
        and type(value.step_index) is int
        and value.step_index == decision.next_step_index == approval.next_step_index
        and type(value.employee_id) is str
        and value.employee_id == employee.id == approval.next_employee_id
        and type(value.employee_instructions) is str
        and value.employee_instructions == employee.instructions
        and type(value.step_instructions) is str
...
def _raise(classification: Classification) -> None:
    raise ProgressionToApprovedPreparationCycleHandoffChainReentryContinuationCompatibilityError(classification) from None
