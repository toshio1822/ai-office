"""Continue one exact Phase 74 result through the existing Phase 68 boundary."""
...
class PreparedNextStepStartRoutingPhaseBridgeCycleContinuationFailureDetail:
    classification: Classification


class PreparedNextStepStartRoutingPhaseBridgeCycleContinuationError(ValueError):
    """Raised when Phase 75 cannot safely continue its supplied result."""


class PreparedNextStepStartRoutingPhaseBridgeCycleContinuationCompatibilityError(
    PreparedNextStepStartRoutingPhaseBridgeCycleContinuationError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__(
            "prepared next-step start routing phase bridge cycle continuation "
            "inputs are incompatible"
        )
        self.detail = PreparedNextStepStartRoutingPhaseBridgeCycleContinuationFailureDetail(
            classification
        )


def route_prepared_next_step_start_routing_phase_bridge_cycle_continuation(
    result: object,
    workflow: object,
    ...
    events_path: object,
    *,
    phase68_function: Phase68Function = (
        route_prepared_next_step_start_routing_phase_bridge_continuation
    ),
) -> PreparedStepExecutionStart | WorkflowProgressionDecision | PersistedExecutionOutcome:
    _validate_inputs(result, workflow, state_path, events_path, phase68_function)
    ...
    try:
        started = phase68_function(result, workflow, employee, state_path, events_path)
    except Phase68Error as error:
    ...
    except PreparedNextStepStartRoutingPhaseBridgeCycleContinuationCompatibilityError as error:
...
def _validate_terminal(value: WorkflowProgressionDecision | PersistedExecutionOutcome, workflow: WorkflowDefinition,
    ...
    if persisted.status != status or (
        persisted.workflow_id, persisted.current_step_id, persisted.current_step_index, persisted.current_employee_id
...
def _raise(classification: Classification) -> None:
    raise PreparedNextStepStartRoutingPhaseBridgeCycleContinuationCompatibilityError(classification) from None
