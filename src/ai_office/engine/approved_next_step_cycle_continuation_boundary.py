...
class ApprovedNextStepCycleContinuationFailureDetail:
    classification: Classification


class ApprovedNextStepCycleContinuationError(ValueError):
    """Raised when Phase 102 cannot safely continue one approved next step."""


class ApprovedNextStepCycleContinuationCompatibilityError(
    ApprovedNextStepCycleContinuationError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__("approved next-step cycle continuation inputs are incompatible")
        self.detail = ApprovedNextStepCycleContinuationFailureDetail(classification)


def route_approved_next_step_cycle_continuation_boundary(
    result: object,
    workflow: object,
    approval: object,
    employee: object,
    ...
    events_path: object,
    *,
    phase95_function: Phase95Function = (
        route_approved_next_step_preparation_dispatch_phase_bridge_cycle_reentry_continuation
    ),
) -> PreparedWorkflowStep | WorkflowProgressionDecision | PersistedExecutionOutcome:
    """Continue one exact Phase 101 result through Phase 95 at most once."""
    _validate_inputs(result, workflow, state_path, events_path, phase95_function)
    ...
    try:
        value = phase95_function(
            result, workflow, approval, employee, state_path, events_path
        )
    except ApprovedNextStepPreparationDispatchPhaseBridgeCycleReentryContinuationError as error:
...
def _raise(classification: Classification) -> None:
    raise ApprovedNextStepCycleContinuationCompatibilityError(classification) from None
