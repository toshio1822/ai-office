...
Phase103Function = Callable[..., PreparedStepExecutionStart | WorkflowProgressionDecision | PersistedExecutionOutcome]
_FAILURE_CATEGORIES = frozenset(get_args(ModelInvocationFailureCategory))
_PATH_TYPE = type(Path())


@dataclass(frozen=True)
class PreparedStepStartCycleReentryContinuationFailureDetail:
    classification: Classification


class PreparedStepStartCycleReentryContinuationError(ValueError):
    """Raised when Phase 103 cannot safely continue its supplied result."""


class PreparedStepStartCycleReentryContinuationCompatibilityError(
    PreparedStepStartCycleReentryContinuationError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__("prepared-step start cycle continuation inputs are incompatible")
        self.detail = PreparedStepStartCycleReentryContinuationFailureDetail(classification)


def route_prepared_step_start_cycle_reentry_continuation_boundary(
    result: object,
    workflow: object,
    employee: object,
    state_path: object,
    events_path: object,
    *,
    phase103_function: Phase103Function = route_prepared_step_start_cycle_continuation_boundary,
) -> PreparedStepExecutionStart | WorkflowProgressionDecision | PersistedExecutionOutcome:
    _validate_inputs(result, workflow, state_path, events_path, phase103_function)
    ...
    try:
        value = phase103_function(result, workflow, employee, state_path, events_path)
    except PreparedStepStartCycleContinuationError as error:
    ...
    except PreparedStepStartCycleReentryContinuationCompatibilityError as error:
...
def _raise(classification: Classification) -> None:
    raise PreparedStepStartCycleReentryContinuationCompatibilityError(classification) from None
