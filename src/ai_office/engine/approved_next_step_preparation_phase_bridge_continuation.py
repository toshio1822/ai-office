"""Continue exact Phase 66 results through the existing Phase 60 boundary."""
...
class ApprovedNextStepPreparationPhaseBridgeContinuationFailureDetail:
    classification: Classification


class ApprovedNextStepPreparationPhaseBridgeContinuationError(ValueError):
    """Raised when Phase 67 cannot safely continue its supplied result."""


class ApprovedNextStepPreparationPhaseBridgeContinuationCompatibilityError(
    ApprovedNextStepPreparationPhaseBridgeContinuationError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__(
            "approved next-step preparation phase bridge continuation "
            "inputs are incompatible"
        )
        self.detail = ApprovedNextStepPreparationPhaseBridgeContinuationFailureDetail(
            classification
        )


def route_approved_next_step_preparation_phase_bridge_continuation(
    result: object,
    workflow: object,
    state_path: object,
    events_path: object,
    ...
    *,
    phase60_function: Phase60Function = (
        route_approved_next_step_preparation_phase_bridge_reentry
    ),
) -> PreparedWorkflowStep | WorkflowProgressionDecision | PersistedExecutionOutcome:
    _validate_inputs(result, workflow, state_path, events_path, phase60_function)
...
def _validate_prepare(
    ...
    current = (
        ...
        if type(value.current_step_index) is int
        and 1 <= value.current_step_index <= len(workflow.steps)
        else None
    )
    next_step = (
        workflow.steps[value.next_step_index - 1]
        if type(value.next_step_index) is int
        and 1 <= value.next_step_index <= len(workflow.steps)
        else None
    )
    if (
        current is None
        ...
        or not (
            ...
            and 1 <= value.current_step_index < len(workflow.steps)
            and _exact_str(value.current_step_id, current.id)
            and _exact_str(value.current_employee_id, current.employee)
            and type(value.next_step_index) is int
            and value.next_step_index == value.current_step_index + 1
            and _exact_str(value.next_step_id, next_step.id)
            and _exact_str(value.next_employee_id, next_step.employee)
...
def _validate_prepared(
    ...
    if not (
        type(value) is PreparedWorkflowStep and type(decision.next_step_index) is int
    ):
        _raise("preparation_contract")
...
def _raise(classification: Classification) -> None:
    raise ApprovedNextStepPreparationPhaseBridgeContinuationCompatibilityError(
        classification
    ) from None
