"""Dispatch one Phase 96 prepared start through the Phase 90 persistence boundary."""
...
Phase90Function = Callable[[object, object, object, object, object], RunningStatePersistenceResult | WorkflowProgressionDecision | PersistedExecutionOutcome]
_FAILURE_CATEGORIES = frozenset(get_args(ModelInvocationFailureCategory))
_PATH_TYPE = type(Path())


@dataclass(frozen=True)
class PreparedStartPersistenceDispatchContinuationFailureDetail:
    classification: Classification


class PreparedStartPersistenceDispatchContinuationError(ValueError):
    """Raised when Phase 90 cannot safely dispatch its supplied result."""


class PreparedStartPersistenceDispatchContinuationCompatibilityError(
    PreparedStartPersistenceDispatchContinuationError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__("prepared-start persistence dispatch continuation boundary inputs are incompatible")
        self.detail = PreparedStartPersistenceDispatchContinuationFailureDetail(classification)


def route_prepared_start_persistence_dispatch_continuation_boundary(
    result: object,
    workflow: object,
    ...
    events_path: object,
    *,
    phase90_function: Phase90Function = route_prepared_start_persistence_dispatch_phase_bridge_cycle_reentry_continuation,
) -> RunningStatePersistenceResult | WorkflowProgressionDecision | PersistedExecutionOutcome:
    _inputs(result, workflow, state_path, events_path, phase90_function)
    ...
    try:
        value = phase90_function(result, workflow, employee, state_path, events_path)
    except PreparedStartPersistenceDispatchPhaseBridgeCycleReentryContinuationError as error:
    ...
    except PreparedStartPersistenceDispatchContinuationCompatibilityError as error:
...
def _raise(classification: Classification) -> None:
    raise PreparedStartPersistenceDispatchContinuationCompatibilityError(classification) from None
