"""Continue one exact Phase 79 classified success through Phase 73."""
...
Phase73Function = Callable[..., WorkflowProgressionDecision | PersistedExecutionOutcome]
_FAILURE_CATEGORIES = frozenset(get_args(ModelInvocationFailureCategory))
_PATH_TYPE = type(Path())


@dataclass(frozen=True)
class ClassifiedOutcomeRoutingPhaseBridgeCycleContinuationFailureDetail:
    classification: Classification


class ClassifiedOutcomeRoutingPhaseBridgeCycleContinuationError(ValueError):
    """Raised when Phase 80 cannot safely continue the supplied result."""


class ClassifiedOutcomeRoutingPhaseBridgeCycleContinuationCompatibilityError(
    ClassifiedOutcomeRoutingPhaseBridgeCycleContinuationError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__("classified outcome routing phase bridge cycle continuation inputs are incompatible")
        self.detail = ClassifiedOutcomeRoutingPhaseBridgeCycleContinuationFailureDetail(classification)


def route_classified_outcome_routing_phase_bridge_cycle_continuation(
    result: object,
    workflow: object,
    state_path: object,
    events_path: object,
    *,
    phase73_function: Phase73Function = route_classified_outcome_routing_phase_bridge_continuation,
) -> WorkflowProgressionDecision | PersistedExecutionOutcome:
    _validate_inputs(result, workflow, state_path, events_path, phase73_function)
    ...
    if result.outcome == "persisted_failure":
        ...
        return result
    try:
        value = phase73_function(result, workflow, state_path, events_path)
    ...
    except ClassifiedOutcomeRoutingPhaseBridgeCycleContinuationCompatibilityError as error:
...
def _validate_success(value: PersistedExecutionOutcome, workflow: WorkflowDefinition) -> None:
    ...
    _validate_identity(value, workflow, "success_contract")


def _validate_failure(value: PersistedExecutionOutcome, workflow: WorkflowDefinition) -> None:
    if not (type(value.outcome) is str and value.outcome == "persisted_failure" and type(value.failure_category) is str and value.failure_category in _FAILURE_CATEGORIES):
        _raise("failure_contract")
    _validate_identity(value, workflow, "failure_contract")


def _validate_identity(value: PersistedExecutionOutcome, workflow: WorkflowDefinition, classification: Classification) -> None:
    if not (type(value.workflow_id) is str and value.workflow_id == workflow.id and type(value.current_step_index) is int and 1 <= value.current_step_index <= len(workflow.steps)):
        _raise(classification)
    step = workflow.steps[value.current_step_index - 1]
    if not (type(value.current_step_id) is str and value.current_step_id == step.id and type(value.current_employee_id) is str and value.current_employee_id == step.employee):
        _raise(classification)


def _validate_completion(value: WorkflowProgressionDecision, workflow: WorkflowDefinition) -> None:
    final = workflow.steps[-1]
    if not (type(value.decision) is str and value.decision == "workflow_complete" and type(value.workflow_id) is str and value.workflow_id == workflow.id and type(value.current_step_id) is str and value.current_step_id == final.id and type(value.current_step_index) is int and value.current_step_index == len(workflow.steps) and type(value.current_employee_id) is str and value.current_employee_id == final.employee and value.next_step_id is None and value.next_step_index is None and value.next_employee_id is None and type(value.reason) is str and value.reason == "last_step_succeeded"):
...
def _validate_terminal(value: object, workflow: WorkflowDefinition, state: Path, events: Path, status: str) -> None:
    try:
...
def _validate_return(value: object, outcome: PersistedExecutionOutcome, workflow: WorkflowDefinition) -> None:
    if type(value) is not WorkflowProgressionDecision:
        _raise("progression_contract")
    if not (type(value.decision) is str and type(value.workflow_id) is str and type(value.current_step_id) is str and type(value.current_step_index) is int and type(value.current_employee_id) is str and type(value.reason) is str and (value.workflow_id, value.current_step_id, value.current_step_index, value.current_employee_id) == (outcome.workflow_id, outcome.current_step_id, outcome.current_step_index, outcome.current_employee_id)):
        _raise("progression_contract")
    if outcome.current_step_index == len(workflow.steps):
        valid = value.decision == "workflow_complete" and value.next_step_id is None and value.next_step_index is None and value.next_employee_id is None and value.reason == "last_step_succeeded"
    else:
        step = workflow.steps[outcome.current_step_index]
        valid = value.decision == "prepare_next_step" and type(value.next_step_id) is str and value.next_step_id == step.id and type(value.next_step_index) is int and value.next_step_index == outcome.current_step_index + 1 and type(value.next_employee_id) is str and value.next_employee_id == step.employee and value.reason == "next_step_available"
    if not valid:
        _raise("progression_contract")
...
def _raise(classification: Classification) -> None:
    raise ClassifiedOutcomeRoutingPhaseBridgeCycleContinuationCompatibilityError(classification) from None
