"""Continue one exact Phase 72 classified success through Phase 59."""
...
class ClassifiedOutcomeRoutingPhaseBridgeContinuationFailureDetail:
    classification: Classification


class ClassifiedOutcomeRoutingPhaseBridgeContinuationError(ValueError):
    """Raised when Phase 73 cannot safely continue a result."""


class ClassifiedOutcomeRoutingPhaseBridgeContinuationCompatibilityError(
    ClassifiedOutcomeRoutingPhaseBridgeContinuationError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__(
            "classified outcome routing phase bridge continuation inputs are "
            "incompatible"
        )
        self.detail = ClassifiedOutcomeRoutingPhaseBridgeContinuationFailureDetail(
            classification
        )


def route_classified_outcome_routing_phase_bridge_continuation(
    result: object,
    workflow: object,
    state_path: object,
    events_path: object,
    *,
    ...
    if type(result) is WorkflowProgressionDecision:
        ...
        _validate_success(
            result, workflow
        ) if result.outcome == "persisted_success" else _validate_failure(
            result, workflow
        )
    ...
    expected = "succeeded" if result.outcome == "persisted_success" else "failed"
    _validate_terminal(result, workflow, state_path, events_path, expected)
    if result.outcome == "persisted_failure":
        _require_unchanged(state_path, events_path, original, "terminal_contract")
        return result
    try:
        value = phase59_function(result, workflow, state_path, events_path)
    except ClassifiedPersistedOutcomeRoutingPhaseBridgeError as error:
    ...
    except ClassifiedOutcomeRoutingPhaseBridgeContinuationCompatibilityError as error:
...
def _validate_failure(
    ...
    if (
        type(value.outcome) is not str
        or value.outcome != "persisted_failure"
        or type(value.failure_category) is not str
...
def _validate_terminal(
    value: object, workflow: WorkflowDefinition, state: Path, events: Path, status: str
) -> None:
    try:
...
def _validate_return(
    ...
    if outcome.current_step_index == len(workflow.steps):
        ...
        valid = (
            ...
            and value.next_step_id == step.id
            ...
            and value.next_employee_id == step.employee
...
def _raise(classification: Classification) -> None:
    raise ClassifiedOutcomeRoutingPhaseBridgeContinuationCompatibilityError(
        classification
    ) from None
