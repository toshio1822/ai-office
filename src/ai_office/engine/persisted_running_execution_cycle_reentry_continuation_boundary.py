...
Phase105Function = Callable[..., StepRuntimeExecutionSuccess | StepRuntimeExecutionFailure | WorkflowProgressionDecision | PersistedExecutionOutcome]
_PATH_TYPE = type(Path())
_FAILURES = frozenset(get_args(ModelInvocationFailureCategory))


@dataclass(frozen=True)
class PersistedRunningExecutionCycleReentryContinuationFailureDetail:
    classification: Classification


class PersistedRunningExecutionCycleReentryContinuationError(ValueError):
    """Base error for the Phase 112 boundary."""


class PersistedRunningExecutionCycleReentryContinuationCompatibilityError(
    PersistedRunningExecutionCycleReentryContinuationError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__("persisted-running execution cycle reentry continuation inputs are incompatible")
        self.detail = PersistedRunningExecutionCycleReentryContinuationFailureDetail(classification)


def route_persisted_running_execution_cycle_reentry_continuation_boundary(
    result: object,
    start: object | None,
    workflow: object,
    ...
    *,
    phase105_function: Phase105Function = route_persisted_running_execution_cycle_continuation_boundary,
) -> StepRuntimeExecutionSuccess | StepRuntimeExecutionFailure | WorkflowProgressionDecision | PersistedExecutionOutcome:
    _top(result, workflow, state_path, events_path, phase105_function)
    ...
    try:
        value = phase105_function(result, start, workflow, employee, state_path, events_path, resolved_tools, api_key, approval, transport)
    ...
    except PersistedRunningExecutionCycleReentryContinuationCompatibilityError as error:
...
def _raise(classification: Classification) -> None:
    raise PersistedRunningExecutionCycleReentryContinuationCompatibilityError(classification) from None
