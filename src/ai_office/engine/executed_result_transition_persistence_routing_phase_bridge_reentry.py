"""Bridge exact Phase 63 results to the existing Phase 57 boundary."""
...
class ExecutedResultTransitionPersistenceRoutingPhaseBridgeFailureDetail:
    classification: Classification


class ExecutedResultTransitionPersistenceRoutingPhaseBridgeError(ValueError):
    """Raised when Phase 64 cannot safely route its supplied result."""


class ExecutedResultTransitionPersistenceRoutingPhaseBridgeCompatibilityError(
    ExecutedResultTransitionPersistenceRoutingPhaseBridgeError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__(
            "executed-result transition persistence routing phase bridge "
...
def route_executed_result_transition_persistence_routing_phase_bridge_reentry(
    result: object,
    workflow: object,
    state_path: object,
    events_path: object,
    *,
    phase57_function: Phase57Function = (
        route_executed_result_transition_persistence_phase_bridge_reentry
    ),
) -> (
    WorkflowExecutionPersistenceResult
    ...
    _top_level(result, workflow, state_path, events_path, phase57_function)
    ...
    try:
        value = phase57_function(result, workflow, state_path, events_path)
    except ExecutedResultTransitionPersistencePhaseBridgeError as error:
    ...
    except ExecutedResultTransitionPersistenceRoutingPhaseBridgeCompatibilityError:
        _restore_if_changed(state_path, events_path, original)
        raise
...
def _raise(classification: Classification) -> None:
    raise ExecutedResultTransitionPersistenceRoutingPhaseBridgeCompatibilityError(
        classification
    ) from None
