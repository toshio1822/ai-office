"""Phase 99 boundary dispatching runtime results through Phase 92."""
...
from ai_office.engine.executed_result_transition_persistence_dispatch_phase_bridge_cycle_reentry_continuation import (
...
class ExecutedResultTransitionPersistenceDispatchContinuationFailureDetail:
    classification: Classification


class ExecutedResultTransitionPersistenceDispatchContinuationError(
    ValueError
):
    """Raised when Phase 92 cannot safely dispatch the supplied result."""


class ExecutedResultTransitionPersistenceDispatchContinuationCompatibilityError(
    ...
    def __init__(self, classification: Classification) -> None:
        ...
        self.detail = ExecutedResultTransitionPersistenceDispatchContinuationFailureDetail(
            classification
        )


def route_executed_result_transition_persistence_dispatch_continuation_boundary(
    result: object,
    workflow: object,
    state_path: object,
    events_path: object,
    *,
    phase92_function: Phase92Function = route_executed_result_transition_persistence_dispatch_phase_bridge_cycle_reentry_continuation,
) -> (
    WorkflowExecutionPersistenceResult
    | WorkflowProgressionDecision
    | PersistedExecutionOutcome
):
    _top_level(result, workflow, state_path, events_path, phase92_function)
    ...
    try:
        value = phase92_function(result, workflow, state_path, events_path)
    except ExecutedResultTransitionPersistenceDispatchPhaseBridgeCycleReentryContinuationError as error:
    ...
    except ExecutedResultTransitionPersistenceDispatchContinuationCompatibilityError as error:
...
def _raise(classification: Classification) -> None:
    raise ExecutedResultTransitionPersistenceDispatchContinuationCompatibilityError(
        classification
    ) from None
