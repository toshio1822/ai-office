...
Phase98Function = Callable[..., StepRuntimeExecutionSuccess | StepRuntimeExecutionFailure | WorkflowProgressionDecision | PersistedExecutionOutcome]
_PATH_TYPE = type(Path())
_FAILURES = frozenset(get_args(ModelInvocationFailureCategory))


@dataclass(frozen=True)
class PersistedRunningExecutionCycleContinuationFailureDetail:
    classification: Classification


class PersistedRunningExecutionCycleContinuationError(ValueError):
    """Base error for the Phase 105 boundary."""


class PersistedRunningExecutionCycleContinuationCompatibilityError(
    PersistedRunningExecutionCycleContinuationError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__("persisted-running execution cycle continuation inputs are incompatible")
        self.detail = PersistedRunningExecutionCycleContinuationFailureDetail(classification)


def route_persisted_running_execution_cycle_continuation_boundary(
    result: object,
    start: object | None,
    workflow: object,
    ...
    *,
    phase98_function: Phase98Function = route_persisted_running_execution_dispatch_continuation_boundary,
) -> StepRuntimeExecutionSuccess | StepRuntimeExecutionFailure | WorkflowProgressionDecision | PersistedExecutionOutcome:
    _top(result, workflow, state_path, events_path, phase98_function)
    ...
    try:
        value = phase98_function(result, start, workflow, employee, state_path, events_path, resolved_tools, api_key, approval, transport)
    except PersistedRunningExecutionDispatchContinuationError as error:
    ...
    except PersistedRunningExecutionCycleContinuationCompatibilityError as error:
...
def _raise(classification: Classification) -> None:
    raise PersistedRunningExecutionCycleContinuationCompatibilityError(classification) from None
