"""Continue one exact Phase 87 result through the Phase 81 boundary."""
...
Phase81Function = Callable[..., PreparedWorkflowStep | WorkflowProgressionDecision | PersistedExecutionOutcome]
_FAILURE_CATEGORIES = frozenset(get_args(ModelInvocationFailureCategory))
_PATH_TYPE = type(Path())


@dataclass(frozen=True)
class ApprovedNextStepPreparationRoutingPhaseBridgeCycleReentryContinuationFailureDetail:
    classification: Classification


class ApprovedNextStepPreparationRoutingPhaseBridgeCycleReentryContinuationError(ValueError):
    """Raised when Phase 88 cannot safely continue its supplied result."""


class ApprovedNextStepPreparationRoutingPhaseBridgeCycleReentryContinuationCompatibilityError(
    ApprovedNextStepPreparationRoutingPhaseBridgeCycleReentryContinuationError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__("approved next-step preparation routing phase bridge cycle reentry continuation inputs are incompatible")
        self.detail = ApprovedNextStepPreparationRoutingPhaseBridgeCycleReentryContinuationFailureDetail(classification)


def route_approved_next_step_preparation_routing_phase_bridge_cycle_reentry_continuation(
    result: object,
    workflow: object,
    approval: object,
    employee: object,
    ...
    events_path: object,
    *,
    phase81_function: Phase81Function = route_approved_next_step_preparation_phase_bridge_cycle_reentry_continuation,
) -> PreparedWorkflowStep | WorkflowProgressionDecision | PersistedExecutionOutcome:
    _validate_inputs(result, workflow, approval, employee, state_path, events_path, phase81_function)
    ...
    _validate_terminal(result, workflow, state_path, events_path, "succeeded")
    try:
        value = phase81_function(result, workflow, approval, employee, state_path, events_path)
    except ApprovedNextStepPreparationPhaseBridgeCycleReentryContinuationError as error:
    ...
    try:
        _unchanged(state_path, events_path, original, "preparation_contract")
        _validate_prepared(value, result, workflow, employee)
    except ApprovedNextStepPreparationRoutingPhaseBridgeCycleReentryContinuationCompatibilityError as error:
...
def _validate_inputs(result: object, workflow: object, approval: object, employee: object,
                     state: object, events: object, function: object) -> None:
...
def _validate_prepare(value: WorkflowProgressionDecision, workflow: WorkflowDefinition,
                      approval: object, employee: object) -> None:
    ...
    if not (type(employee.id) is str and type(employee.name) is str and type(employee.role) is str
            and type(employee.instructions) is str and type(employee.model) is str
            and type(employee.allowed_tools) is list and all(type(item) is str for item in employee.allowed_tools)):
        _raise("employee_contract")
    ...
    if current is None or next_step is None or not (
        type(value.decision) is str and value.decision == "prepare_next_step"
        ...
        and type(value.next_step_index) is int and value.next_step_index == value.current_step_index + 1
        and type(value.next_step_id) is str and value.next_step_id == next_step.id
        and type(value.next_employee_id) is str and value.next_employee_id == next_step.employee
    ...
    if not (type(employee.id) is str and employee.id == value.next_employee_id):
        _raise("employee_contract")
    if not (approval.approved is True and type(approval.workflow_id) is str and approval.workflow_id == value.workflow_id
...
def _validate_failure(value: PersistedExecutionOutcome, workflow: WorkflowDefinition,
    ...
    if not (approval is None and employee is None and type(value.outcome) is str and value.outcome == "persisted_failure"
            and type(value.workflow_id) is str and value.workflow_id == workflow.id
            and type(value.current_step_index) is int and 1 <= value.current_step_index <= len(workflow.steps)
            and type(value.current_step_id) is str and value.current_step_id == workflow.steps[value.current_step_index - 1].id
            and type(value.current_employee_id) is str and value.current_employee_id == workflow.steps[value.current_step_index - 1].employee
...
def _validate_terminal(value: object, workflow: WorkflowDefinition, state: Path, events: Path,
...
def _validate_prepared(value: object, decision: WorkflowProgressionDecision,
                       workflow: WorkflowDefinition, employee: EmployeeDefinition) -> None:
    ...
    if not (type(value.workflow_id) is str and value.workflow_id == workflow.id
            and type(value.step_id) is str and value.step_id == next_step.id
            and type(value.step_index) is int and value.step_index == decision.next_step_index
            and type(value.employee_id) is str and value.employee_id == employee.id
            and type(value.employee_instructions) is str and value.employee_instructions == employee.instructions
            and type(value.step_instructions) is str and value.step_instructions == next_step.instructions
            and type(value.model) is str and value.model == employee.model
            and type(value.allowed_tool_names) is tuple and all(type(item) is str for item in value.allowed_tool_names)
...
def _raise(classification: Classification) -> None:
    raise ApprovedNextStepPreparationRoutingPhaseBridgeCycleReentryContinuationCompatibilityError(classification) from None
