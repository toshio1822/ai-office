...
def _check_terminal(
    ...
    if not _valid_terminal_history(
        state, history, workflow, expected_status, expected_failure
    ):
        _fail("terminal_contract")
    if (
        state.workflow_id,
...
def _valid_terminal_history(
    ...
    expected_completed = (
        ...
        else tuple(step.id for step in prior_steps)
    )
...
def _valid_event_shape(event: object) -> bool:
    return (
        type(event) is RuntimeStepEvent
        ...
        and event.previous_status in {"ready", "running", "succeeded", "failed"}
        ...
        and _optional_nonempty_string(event.request_id)
