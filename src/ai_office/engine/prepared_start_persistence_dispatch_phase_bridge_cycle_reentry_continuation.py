"""Dispatch one Phase 89 prepared start through the Phase 83 persistence boundary."""
...
Phase83Function = Callable[[object, object, object, object, object], RunningStatePersistenceResult | WorkflowProgressionDecision | PersistedExecutionOutcome]
_FAILURE_CATEGORIES = frozenset(get_args(ModelInvocationFailureCategory))
_PATH_TYPE = type(Path())


@dataclass(frozen=True)
class PreparedStartPersistenceDispatchPhaseBridgeCycleReentryContinuationFailureDetail:
    classification: Classification


class PreparedStartPersistenceDispatchPhaseBridgeCycleReentryContinuationError(ValueError):
    """Raised when Phase 90 cannot safely dispatch its supplied result."""


class PreparedStartPersistenceDispatchPhaseBridgeCycleReentryContinuationCompatibilityError(
    PreparedStartPersistenceDispatchPhaseBridgeCycleReentryContinuationError
):
    def __init__(self, classification: Classification) -> None:
        super().__init__("prepared-start persistence dispatch phase bridge cycle reentry continuation inputs are incompatible")
        self.detail = PreparedStartPersistenceDispatchPhaseBridgeCycleReentryContinuationFailureDetail(classification)


def route_prepared_start_persistence_dispatch_phase_bridge_cycle_reentry_continuation(
    result: object,
    workflow: object,
    ...
    events_path: object,
    *,
    phase83_function: Phase83Function = route_prepared_start_persistence_routing_phase_bridge_cycle_reentry_continuation,
) -> RunningStatePersistenceResult | WorkflowProgressionDecision | PersistedExecutionOutcome:
    _inputs(result, workflow, state_path, events_path, phase83_function)
    ...
    try:
        value = phase83_function(result, workflow, employee, state_path, events_path)
    except PreparedStartPersistenceRoutingPhaseBridgeCycleReentryContinuationError as error:
    ...
    except PreparedStartPersistenceDispatchPhaseBridgeCycleReentryContinuationCompatibilityError as error:
...
def _raise(classification: Classification) -> None:
    raise PreparedStartPersistenceDispatchPhaseBridgeCycleReentryContinuationCompatibilityError(classification) from None
